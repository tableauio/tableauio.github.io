...
**workbook**、**worksheet**、**column** 和 **struct**（message）的所有名称，均应使用 `PascalCase`（首字母大写）命名规范。tableau 解析器会将 **worksheet** 名称作为 protoconf 的 message 名称，并自动将 `PascalCase` 转换为 `snake_case`，用于 protobuf 的 [字段名](https://protobuf.dev/programming-guides/style/#field-names)和文件名，以符合 [Protocol Buffers 风格指南](https://protobuf.dev/programming-guides/style)。
