...
`}).add({id:141,href:"/docs/excel/enum/#use-predefined-enum-type",title:"Enum / Use predefined enum type ",description:"Excel enum guide.",content:` [!NOTE] See basics/enum for more details.
...
MODE_ENUM_TYPE: define single enum type in a sheet. MODE_ENUM_TYPE_MULTI: define multiple enum types in a sheet. `}).add({id:143,href:"/docs/excel/enum/#single-enum-type-in-sheet",title:"Enum / Single enum type in sheet ",description:"Excel enum guide.",content:` [!Note]
The Number column is optional, used to specify enum value numbers. If omitted, they auto-increment starting from 1. If the default enum value 0 is not explicitly defined, it will be auto-generated with the name pattern {ENUM_TYPE}_INVALID. You should specify Mode option to MODE_ENUM_TYPE in metasheet @TABLEAU.
For example, a worksheet ItemType in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemType @TABLEAU Number Name Alias 1 ITEM_TYPE_FRUIT Fruit 2 ITEM_TYPE_EQUIP Equip 3 ITEM_TYPE_BOX Box Sheet Mode ItemType MODE_ENUM_TYPE Generated:
...
HelloWorld.xlsx&nbsp; Enum @TABLEAU CatType CatType note Number Name Alias 1 CAT_TYPE_RAGDOLL Ragdoll 2 CAT_TYPE_PERSIAN Persian 3 CAT_TYPE_SPHYNX Sphynx DogType DogType note Number Name Alias 1 DOG_TYPE_POODLE Poodle 2 DOG_TYPE_BULLDOG Bulldog 3 DOG_TYPE_DACHSHUND Dachshund BirdType BirdType note Number Name Alias 1 BIRD_TYPE_CANARY Canary 2 BIRD_TYPE_WOODPECKER Woodpecker 3 BIRD_TYPE_OWL Owl Sheet Mode Enum MODE_ENUM_TYPE_MULTI Generated:
hello_world.proto `}).add({id:145,href:"/docs/excel/enum/#define-and-use-enum-type-in-sheet",title:"Enum / Define and use enum type in sheet ",description:"Excel enum guide.",content:` For example, two worksheets ItemType and ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:146,href:"/docs/excel/struct/#cross-cell-struct",title:"Struct / Cross-cell struct ",description:"Excel struct guide.",content:` Syntax: &lt;StructType&gt;ColumnType.
...
Generated:
hello_world.proto ItemConf.json `}).add({id:147,href:"/docs/excel/struct/#note",title:"Struct / Note ",description:"Excel struct guide.",content:` Cross-cell struct is usually used together with:
cross-cell horizontal/vertical map, as map value type. Map cross-cell horizontal/vertical list, as list element type. List `}).add({id:148,href:"/docs/excel/struct/#incell-struct",title:"Struct / Incell struct ",description:"Excel struct guide.",content:` Each field type of the struct should be scalar type.
For example, a worksheet ItemConf in HelloWorld.xlsx:
...
Generated:
hello_world.proto ItemConf.json `}).add({id:149,href:"/docs/excel/struct/#predefined-struct",title:"Struct / Predefined struct ",description:"Excel struct guide.",content:` For example, struct type Prop in common.proto is defined as:
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:150,href:"/docs/excel/struct/#predefined-incell-struct",title:"Struct / Predefined incell struct ",description:"Excel struct guide.",content:` Each field type of the predefined struct should be scalar type.
...
Generated:
hello_world.proto ItemConf.json `}).add({id:152,href:"/docs/excel/struct/#advanced-predefined-incell-struct",title:"Struct / Advanced predefined incell struct ",description:"Excel struct guide.",content:` In some situations, you may want to configure any complex struct in a cell, so tableau support two kinds of protobuf serialized formats: text format, and JSON format.
...
hello_world.proto ItemConf.json `}).add({id:153,href:"/docs/excel/struct/#define-struct-type-in-sheet",title:"Struct / Define struct type in sheet ",description:"Excel struct guide.",content:` There are two kinds of Mode (in metasheet @TABLEAU) to define struct types in a sheet:
MODE_STRUCT_TYPE: define single struct type in a sheet. MODE_STRUCT_TYPE_MULTI: define multiple struct types in a sheet. `}).add({id:154,href:"/docs/excel/struct/#single-struct-type-in-sheet",title:"Struct / Single struct type in sheet ",description:"Excel struct guide.",content:` [!Note] The Number column is optional, used to specify field numbers. If omitted, they auto-increment starting from 1.
You should specify Mode option to MODE_STRUCT_TYPE in metasheet @TABLEAU.
For example, a worksheet Item in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; Item @TABLEAU Number Name Type 1 ID uint32 2 Num int32 3 FruitType enum&lt;.FruitType&gt; 4 Feature []int32 5 Prop map&lt;int32, string&gt; 6 Detail {enum&lt;.ItemType&gt; Type, string Name, string Desc}Detail Sheet Mode Item MODE_STRUCT_TYPE Generated:
hello_world.proto `}).add({id:155,href:"/docs/excel/struct/#multiple-struct-types-in-sheet",title:"Struct / Multiple struct types in sheet ",description:"Excel struct guide.",content:` [!IMPORTANT] Each struct type is defined by a block — a series of contiguous non-empty rows. Blocks are separated by one or more empty rows.
...
HelloWorld.xlsx&nbsp; Struct @TABLEAU Tree Tree note Number Name Type 1 ID uint32 2 Num int32 Pet Pet note Number Name Type 1 Kind int32 2 Tip []string FruitShop FruitShop note Number Name Type 1 FruitType enum&lt;.FruitType&gt; 2 Prop map&lt;int32, string&gt; Sheet Mode Struct MODE_STRUCT_TYPE_MULTI Generated:
hello_world.proto `}).add({id:156,href:"/docs/excel/union/#theory",title:"Union / Theory ",description:"Excel union guide.",content:` In protoconf, union type means the tagged union: a data structure used to hold a value that could take on several different, but fixed, types. Only one of the types can be in use at any one time, and a tag field explicitly indicates which one is in use. More details can be learned from wikipedia Tagged union.
...
`}).add({id:157,href:"/docs/excel/union/#union-definition",title:"Union / Union definition ",description:"Excel union guide.",content:` For example, union type Target in common.proto is predefined as:
`}).add({id:158,href:"/docs/excel/union/#predefined-union-in-list",title:"Union / Predefined union in list ",description:"Excel union guide.",content:` [!NOTE] Based on predefined union type Target.
...
hello_world.proto TaskConf.json TaskConf.txt `}).add({id:159,href:"/docs/excel/union/#predefined-union-in-map",title:"Union / Predefined union in map ",description:"Excel union guide.",content:` [!NOTE] Based on predefined union type Target.
A worksheet TaskConf in HelloWorld.xlsx:
...
hello_world.proto TaskConf.json TaskConf.txt `}).add({id:160,href:"/docs/excel/union/#predefined-incell-union-in-map",title:"Union / Predefined incell union in map ",description:"Excel union guide.",content:` [!NOTE] Based on predefined union type Target.
A worksheet TaskConf in HelloWorld.xlsx:
...
hello_world.proto TaskConf.json `}).add({id:161,href:"/docs/excel/union/#define-union-type-in-sheet",title:"Union / Define union type in sheet ",description:"Excel union guide.",content:` There are two kinds of Mode (in metasheet @TABLEAU) to define union types in a sheet:
...
Scalar Enum Wellknown types Incell struct Incell list Incell map `}).add({id:162,href:"/docs/excel/union/#single-union-type-in-sheet",title:"Union / Single union type in sheet ",description:"Excel union guide.",content:` [!Note] The Number column is optional, used to specify field numbers and their corresponding enum value numbers. If omitted, they auto-increment starting from 1.
...
Note Type
enum&lt;.FruitType&gt;
Note 2 PVE AliasPVE Hero
[]uint32
Note Dungeon
map&lt;int32, int64&gt;
Note 3 Skill AliasSkill StartTime
datetime
Note Duration
duration
Note Sheet Mode Target MODE_UNION_TYPE Generated:
hello_world.proto `}).add({id:163,href:"/docs/excel/union/#multiple-union-types-in-sheet",title:"Union / Multiple union types in sheet ",description:"Excel union guide.",content:` [!IMPORTANT] Each union type is defined by a block — a series of contiguous non-empty rows. Blocks are separated by one or more empty rows.
...
HelloWorld.xlsx&nbsp; Union @TABLEAU HeroTarget HeroTarget note Number Name Alias Field1 Field2 Field3 1 StarUp HeroStarUp ID
uint32
Note Star
int32 Note 2 LevelUp HeroLevelUp ID
uint32
Note Level
int32 Note Super
bool
Note BattleTarget BattleTarget note Number Name Alias Field1 Field2 Field3 1 PVP BattlePVP BattleID
int32
Note Damage
int64
Note 2 PVE BattlePVE HeroID
[]int32
Note Dungeon
map&lt;int32, int64&gt;
Note Boss
{uint32 ID, int64 Damage}Boss
Note Sheet Mode Union MODE_UNION_TYPE_MULTI Generated:
hello_world.proto `}).add({id:164,href:"/docs/excel/union/#specify-type-column",title:"Union / Specify Type column ",description:"Excel union guide.",content:` By default, each union&rsquo;s oneof field is a message type with the name specified by Name column. Now, you can add Type column and specify custom oneof field type:
...
hello_world.proto `}).add({id:165,href:"/docs/excel/union/#complex-union-type-in-sheet",title:"Union / Complex union type in sheet ",description:"Excel union guide.",content:` For example, two worksheets Target and TaskConf in HelloWorld.xlsx:
...
hello_world.proto TaskConf.json `}).add({id:166,href:"/docs/excel/list/#horizontal-list",title:"List / Horizontal list ",description:"Excel list guide.",content:` [!IMPORTANT] In a horizontal list, the list element name in each column name MUST carry a digit suffix starting at 1.
...
List element type Syntax example scalar []uint32 enum []enum&lt;.FruitType&gt; struct [Item]uint32 predefined struct [.Item]uint32 incell struct []{uint32 ID, string Num}Item incell predefined struct []{.Item} `}).add({id:167,href:"/docs/excel/list/#horizontal-scalar-list",title:"List / Horizontal scalar list ",description:"Excel list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:168,href:"/docs/excel/list/#horizontal-enum-list",title:"List / Horizontal enum list ",description:"Excel list guide.",content:` FruitType in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:169,href:"/docs/excel/list/#horizontal-struct-list",title:"List / Horizontal struct list ",description:"Excel list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:170,href:"/docs/excel/list/#horizontal-predefined-struct-list",title:"List / Horizontal predefined-struct list ",description:"Excel list guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:171,href:"/docs/excel/list/#horizontal-incell-struct-list",title:"List / Horizontal incell-struct list ",description:"Excel list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:172,href:"/docs/excel/list/#horizontal-incell-predefined-struct-list",title:"List / Horizontal incell-predefined-struct list ",description:"Excel list guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:173,href:"/docs/excel/list/#vertical-list",title:"List / Vertical list ",description:"Excel list guide.",content:` Overview of vertical list syntax:
List element type Syntax example scalar []uint32 enum []enum&lt;.FruitType&gt; struct [Item]uint32 predefined struct [.Item]int32 incell struct []{int32 ID,int32 Num}Item incell predefined struct []{.Item} `}).add({id:174,href:"/docs/excel/list/#vertical-scalar-list",title:"List / Vertical scalar list ",description:"Excel list guide.",content:` [!NOTE] It&rsquo;s defined same as Incell scalar list, but will aggregate multiple rows if provided.
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []uint32 ID 1,2,3 1,2 1 Generated:
hello_world.proto ItemConf.json `}).add({id:175,href:"/docs/excel/list/#vertical-enum-list",title:"List / Vertical enum list ",description:"Excel list guide.",content:` [!NOTE] It&rsquo;s defined same as Incell enum list, but will aggregate multiple rows if provided.
...
hello_world.proto ItemConf.json `}).add({id:176,href:"/docs/excel/list/#vertical-struct-list",title:"List / Vertical struct list ",description:"Excel list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:177,href:"/docs/excel/list/#vertical-predefined-struct-list",title:"List / Vertical predefined-struct list ",description:"Excel list guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Num [.Item]int32 int32 Item&rsquo;s ID Item&rsquo;s num 1 100 2 200 3 300 Generated:
hello_world.proto ItemConf.json `}).add({id:178,href:"/docs/excel/list/#vertical-incell-struct-list",title:"List / Vertical incell-struct list ",description:"Excel list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{int32 ID,int32 Num}Item Item list 1:100 2:200,3:300 Generated:
hello_world.proto ItemConf.json `}).add({id:179,href:"/docs/excel/list/#vertical-incell-predefined-struct-list",title:"List / Vertical incell-predefined-struct list ",description:"Excel list guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{.Item} Item&rsquo;s info 1:100 2:200,3:300 Generated:
hello_world.proto ItemConf.json `}).add({id:180,href:"/docs/excel/list/#incell-list",title:"List / Incell list ",description:"Excel list guide.",content:` Overview of incell list syntax:
List element type Syntax example scalar []int32 enum []enum&lt;.FruitType&gt; incell struct []{int32 ID,int32 Num}Item incell predefined struct []{.Item} `}).add({id:181,href:"/docs/excel/list/#incell-scalar-list",title:"List / Incell scalar list ",description:"Excel list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
Generated:
hello_world.proto ItemConf.json `}).add({id:182,href:"/docs/excel/list/#incell-enum-list",title:"List / Incell enum list ",description:"Excel list guide.",content:` FruitType in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
...
Generated:
hello_world.proto ItemConf.json `}).add({id:183,href:"/docs/excel/list/#incell-struct-list",title:"List / Incell struct list ",description:"Excel list guide.",content:` For more advanced incell data parsing, see Advanced predefined incell struct.
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{int32 ID,int32 Num}Item Item&rsquo;s info 1:100,2:200,3:300 Generated:
hello_world.proto ItemConf.json `}).add({id:184,href:"/docs/excel/list/#incell-predefined-struct-list",title:"List / Incell predefined-struct list ",description:"Excel list guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{.Item} Item&rsquo;s info 1:100,2:200,3:300 Generated:
hello_world.proto ItemConf.json `}).add({id:185,href:"/docs/excel/list/#horizontal-list-size",title:"List / Horizontal list size ",description:"Excel list guide.",content:`
`}).add({id:186,href:"/docs/excel/list/#dynamic-size",title:"List / Dynamic size ",description:"Excel list guide.",content:` By default, all lists are Dynamically Sized Types. List elements should be present continuously, otherwise an error is reported if an empty element is existed in between.
`}).add({id:187,href:"/docs/excel/list/#fixed-size",title:"List / Fixed size ",description:"Excel list guide.",content:` Implicit fixed size # The list size is auto resolved by the max present list elements in name row.
...
hello_world.proto ItemConf.json `}).add({id:188,href:"/docs/excel/list/#advanced-features",title:"List / Advanced features ",description:"Excel list guide.",content:`
`}).add({id:189,href:"/docs/excel/list/#horizontal-column-skipped-list",title:"List / Horizontal column-skipped list ",description:"Excel list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto HeroConf.json `}).add({id:190,href:"/docs/excel/map/#horizontal-map",title:"Map / Horizontal map ",description:"Excel map guide.",content:` [!IMPORTANT] In a horizontal map, the map element name in each column name MUST carry a digit suffix starting at 1.
For example: Item1ID, Item1Name, Item2ID, Item2Name (struct map, element name: Item).
There are some kinds of horizontal map:
Horizontal scalar map, as map value type is scalar. E.g: map&lt;int32, int32&gt;. Horizontal struct map, as map value type is struct. E.g: map&lt;int32, Item&gt;. Horizontal predefined-struct map, as map value type is predefined struct. E.g: map&lt;int32, .Item&gt;. `}).add({id:191,href:"/docs/excel/map/#horizontal-scalar-map",title:"Map / Horizontal scalar map ",description:"Excel map guide.",content:` No need to support, use this instead: map&lt;int32, Item&gt;.
`}).add({id:192,href:"/docs/excel/map/#horizontal-struct-map",title:"Map / Horizontal struct map ",description:"Excel map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:193,href:"/docs/excel/map/#horizontal-predefined-struct-map",title:"Map / Horizontal predefined-struct map ",description:"Excel map guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:194,href:"/docs/excel/map/#vertical-map",title:"Map / Vertical map ",description:"Excel map guide.",content:` There are some kinds of vertical map:
Vertical scalar map, as map value type is scalar. E.g: map&lt;int32, int32&gt;. Vertical struct map, as map value type is struct. E.g: map&lt;int32, Item&gt;. Vertical predefined-struct map, as map value type is predefined struct. E.g: map&lt;int32, .Item&gt;. `}).add({id:195,href:"/docs/excel/map/#vertical-scalar-map",title:"Map / Vertical scalar map ",description:"Excel map guide.",content:` No need to support, use map&lt;int32, Item&gt; instead.
`}).add({id:196,href:"/docs/excel/map/#vertical-struct-map",title:"Map / Vertical struct map ",description:"Excel map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:197,href:"/docs/excel/map/#vertical-predefined-struct-map",title:"Map / Vertical predefined-struct map ",description:"Excel map guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:198,href:"/docs/excel/map/#incell-map",title:"Map / Incell map ",description:"Excel map guide.",content:` There are some kinds of in-cell map:
in-cell scalar map, as map value type is scalar. E.g: map&lt;int32, int32&gt;. in-cell struct map, as map value type is struct. E.g: map&lt;int32, Item&gt;. `}).add({id:199,href:"/docs/excel/map/#incell-scalar-map",title:"Map / Incell scalar map ",description:"Excel map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
Generated:
hello_world.proto ItemConf.json `}).add({id:200,href:"/docs/excel/map/#incell-enum-map",title:"Map / Incell enum map ",description:"Excel map guide.",content:` For incell map, both the key and value can be enum types.
...
hello_world.proto ItemConf.json `}).add({id:201,href:"/docs/excel/map/#empty-key-map",title:"Map / Empty key map ",description:"Excel map guide.",content:` If map key is not configured, then it will be treated as default value of map key type. Default value is illustrated at Scalar types.
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:202,href:"/docs/excel/map/#enum-key-map",title:"Map / Enum key map ",description:"Excel map guide.",content:` As the protobuf documents the restrictions of map key type:
...
hello_world.proto ItemConf.json `}).add({id:203,href:"/docs/excel/map/#horizontal-map-size",title:"Map / Horizontal map size ",description:"Excel map guide.",content:`
`}).add({id:204,href:"/docs/excel/map/#dynamic-size",title:"Map / Dynamic size ",description:"Excel map guide.",content:` By default, all maps are Dynamically Sized Types. Map items should be present continuously, otherwise an error is reported if an empty item is existed in between.
`}).add({id:205,href:"/docs/excel/map/#fixed-size",title:"Map / Fixed size ",description:"Excel map guide.",content:` Implicit fixed size # The map size is auto resolved by the max map items present in name row.
...
[!CAUTION] If multiple empty map items are inserted, only one is actually generated — because all empty map items share the same key. This differs from list behavior, so pay special attention.
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:206,href:"/docs/excel/map/#advanced-features",title:"Map / Advanced features ",description:"Excel map guide.",content:`
`}).add({id:207,href:"/docs/excel/map/#horizontal-column-skipped-map",title:"Map / Horizontal column-skipped map ",description:"Excel map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto HeroConf.json `}).add({id:208,href:"/docs/excel/keyedlist/#syntax",title:"KeyedList / Syntax ",description:"Excel keyed list guide.",content:` Keyed list is same as normal list, except that ColumnType (first field type) is surrounded by angle brackets &lt;&gt;, and is treated as map key.
Syntax: [ElemType]&lt;ColumnType&gt;
`}).add({id:209,href:"/docs/excel/keyedlist/#horizontal-list",title:"KeyedList / Horizontal list ",description:"Excel keyed list guide.",content:` TODO&hellip;
`}).add({id:210,href:"/docs/excel/keyedlist/#vertical-keyedlist",title:"KeyedList / Vertical KeyedList ",description:"Excel keyed list guide.",content:`
`}).add({id:211,href:"/docs/excel/keyedlist/#vertical-scalar-keyedlist",title:"KeyedList / Vertical scalar KeyedList ",description:"Excel keyed list guide.",content:` [!NOTE] It&rsquo;s defined same as Incell scalar KeyedList, but will aggregate multiple rows if provided.
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []&lt;uint32&gt; ID 1,2,3 4,5 6 Generated:
hello_world.proto ItemConf.json `}).add({id:212,href:"/docs/excel/keyedlist/#vertical-enum-keyedlist",title:"KeyedList / Vertical enum KeyedList ",description:"Excel keyed list guide.",content:` It&rsquo;s defined same as Incell enum keyedList, but will aggregate multiple rows if provided.
FruitType in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Type []&lt;enum&lt;.FruitType&gt;&gt; Type Apple,Orange FRUIT_TYPE_BANANA 0 Generated:
hello_world.proto ItemConf.json `}).add({id:213,href:"/docs/excel/keyedlist/#vertical-struct-keyedlist",title:"KeyedList / Vertical struct KeyedList ",description:"Excel keyed list guide.",content:` For example, a worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:214,href:"/docs/excel/keyedlist/#incell-keyedlist",title:"KeyedList / Incell KeyedList ",description:"Excel keyed list guide.",content:`
`}).add({id:215,href:"/docs/excel/keyedlist/#incell-scalar-keyedlist",title:"KeyedList / Incell scalar KeyedList ",description:"Excel keyed list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []&lt;uint32&gt; ID list 1,2,3 Generated:
hello_world.proto ItemConf.json `}).add({id:216,href:"/docs/excel/keyedlist/#incell-enum-keyedlist",title:"KeyedList / Incell enum KeyedList ",description:"Excel keyed list guide.",content:` FruitType in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Param []enum&lt;.FruitType&gt; Param list 1,FRUIT_TYPE_ORANGE,Banana The Param column&rsquo;s type is incell list []enum&lt;.FruitType&gt;, as the list element is the predefined enum type FruitType.
Generated:
hello_world.proto ItemConf.json `}).add({id:217,href:"/docs/excel/wellknown-types/#datetime",title:"Wellknown types / Datetime ",description:"Wellknown guide.",content:`
`}).add({id:218,href:"/docs/excel/wellknown-types/#datetime-1",title:"Wellknown types / Datetime ",description:"Wellknown guide.",content:` [!NOTE] See Basics: Datetime for more details.
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:219,href:"/docs/excel/wellknown-types/#date",title:"Wellknown types / Date ",description:"Wellknown guide.",content:` [!NOTE] See Basics: Datetime for more details.
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:220,href:"/docs/excel/wellknown-types/#time",title:"Wellknown types / Time ",description:"Wellknown guide.",content:` [!NOTE] See Basics: Datetime for more details.
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:221,href:"/docs/excel/wellknown-types/#duration",title:"Wellknown types / Duration ",description:"Wellknown guide.",content:` [!NOTE] See Basics: Duration for more details.
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:222,href:"/docs/excel/wellknown-types/#fraction",title:"Wellknown types / Fraction ",description:"Wellknown guide.",content:` [!NOTE] See Basics: Fraction for more details.
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:223,href:"/docs/excel/wellknown-types/#comparator",title:"Wellknown types / Comparator ",description:"Wellknown guide.",content:` [!NOTE] See Basics: Comparator for more details.
A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:224,href:"/docs/excel/wellknown-types/#version",title:"Wellknown types / Version ",description:"Wellknown guide.",content:` [!NOTE] See Basics: Version for more details.
...
hello_world.proto ItemConf.json `}).add({id:225,href:"/docs/excel/struct-in-struct/#struct-in-struct",title:"Struct in struct / Struct in struct ",description:"Excel struct in struct guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:226,href:"/docs/excel/struct-in-struct/#predefined-struct-in-struct",title:"Struct in struct / Predefined-struct in struct ",description:"Excel struct in struct guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU RewardID RewardItemID RewardItemNum {Reward}int32 {.Item}int32 int32 Reward&rsquo;s ID Item&rsquo;s ID Item&rsquo;s num 1 1 10 Generated:
hello_world.proto ItemConf.json `}).add({id:227,href:"/docs/excel/struct-in-struct/#incell-struct-in-struct",title:"Struct in struct / Incell-struct in struct ",description:"Excel struct in struct guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:228,href:"/docs/excel/struct-in-list/#nested-in-vertical-list",title:"Struct in list / Nested in vertical-list ",description:"Excel struct in list guide.",content:`
`}).add({id:229,href:"/docs/excel/struct-in-list/#struct-in-vertical-list",title:"Struct in list / Struct in vertical-list ",description:"Excel struct in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:230,href:"/docs/excel/struct-in-list/#incell-struct-in-vertical-list",title:"Struct in list / Incell-struct in vertical-list ",description:"Excel struct in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:231,href:"/docs/excel/struct-in-list/#first-field-in-horizontal-list",title:"Struct in list / First-field in horizontal-list ",description:"Excel struct in list guide.",content:`
`}).add({id:232,href:"/docs/excel/struct-in-list/#struct-in-horizontal-list",title:"Struct in list / Struct in horizontal-list ",description:"Excel struct in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:233,href:"/docs/excel/struct-in-list/#predefined-struct-in-horizontal-list",title:"Struct in list / Predefined-struct in horizontal-list ",description:"Excel struct in list guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Reward1ItemID Reward1ItemNum Reward1Name Reward2ItemID Reward2ItemNum Reward2Name [Reward]{.Item}int32 int32 string int32 int32 string Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name 1 10 Lotto 10 100 Super Lotto Generated:
hello_world.proto ItemConf.json `}).add({id:234,href:"/docs/excel/struct-in-list/#incell-struct-in-horizontal-list",title:"Struct in list / Incell-struct in horizontal-list ",description:"Excel struct in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:235,href:"/docs/excel/struct-in-map/#nested-in-vertical-map",title:"Struct in map / Nested in vertical-map ",description:"Excel struct in map guide.",content:`
`}).add({id:236,href:"/docs/excel/struct-in-map/#struct-in-vertical-map",title:"Struct in map / Struct in vertical-map ",description:"Excel struct in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:237,href:"/docs/excel/struct-in-map/#predefined-struct-in-vertical-map",title:"Struct in map / Predefined-struct in vertical-map ",description:"Excel struct in map guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID ItemID ItemNum map&lt;int32, Reward&gt; {.Item}int32 int32 Reward&rsquo;s ID Item&rsquo;s ID Item&rsquo;s Num 1 1 10 2 2 20 3 Generated:
hello_world.proto ItemConf.json `}).add({id:238,href:"/docs/excel/struct-in-map/#incell-struct-in-vertical-map",title:"Struct in map / Incell-struct in vertical-map ",description:"Excel struct in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:239,href:"/docs/excel/list-in-list/#nested-in-vertical-list",title:"List in list / Nested in vertical-list ",description:"Excel list in list guide.",content:`
`}).add({id:240,href:"/docs/excel/list-in-list/#horizontal-list-in-vertical-list",title:"List in list / Horizontal-list in vertical-list ",description:"Excel list in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:241,href:"/docs/excel/list-in-list/#vertical-list-in-vertical-keyed-list",title:"List in list / Vertical-list in vertical-keyed-list ",description:"Excel list in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:242,href:"/docs/excel/list-in-list/#vertical-keyed-list-in-vertical-keyed-list",title:"List in list / Vertical-keyed-list in vertical-keyed-list ",description:"Excel list in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:243,href:"/docs/excel/list-in-list/#incell-list-in-vertical-keyed-list",title:"List in list / Incell-list in vertical-keyed-list ",description:"Excel list in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:244,href:"/docs/excel/list-in-list/#incell-keyed-list-in-vertical-keyed-list",title:"List in list / Incell-keyed-list in vertical-keyed-list ",description:"Excel list in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
Generated:
hello_world.proto ItemConf.json `}).add({id:245,href:"/docs/excel/list-in-list/#nested-in-horizontal-list",title:"List in list / Nested in horizontal-list ",description:"Excel list in list guide.",content:`
`}).add({id:246,href:"/docs/excel/list-in-list/#horizontal-list-in-horizontal-list",title:"List in list / Horizontal-list in horizontal-list ",description:"Excel list in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:247,href:"/docs/excel/list-in-list/#predefined-struct-list-in-horizontal-list",title:"List in list / Predefined-struct-list in horizontal-list ",description:"Excel list in list guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Reward1Item1ID Reward1Item1Num Reward1Item2ID Reward1Item2Num Reward1Name Reward2Item1ID Reward2Item1Num Reward2Name [Reward][.Item]int32 int32 int32 int32 string int32 int32 string Item1&rsquo;s ID Item1&rsquo;s num Item2&rsquo;s ID Item2&rsquo;s num Reward&rsquo;s name Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name 1 10 2 20 Lotto 10 100 Super Lotto Generated:
hello_world.proto ItemConf.json `}).add({id:248,href:"/docs/excel/list-in-list/#incell-list-in-horizontal-list",title:"List in list / Incell-list in horizontal-list ",description:"Excel list in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:249,href:"/docs/excel/list-in-map/#nested-in-vertical-map",title:"List in map / Nested in vertical-map ",description:"Excel list in map guide.",content:`
`}).add({id:250,href:"/docs/excel/list-in-map/#horizontal-list-in-vertical-map",title:"List in map / Horizontal-list in vertical-map ",description:"Excel list in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:251,href:"/docs/excel/list-in-map/#vertical-list-in-vertical-map",title:"List in map / Vertical-list in vertical-map ",description:"Excel list in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:252,href:"/docs/excel/list-in-map/#incell-list-in-vertical-map",title:"List in map / Incell-list in vertical-map ",description:"Excel list in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:253,href:"/docs/excel/list-in-map/#incell-struct-list-in-vertical-map",title:"List in map / Incell-struct-list in vertical-map ",description:"Excel list in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:254,href:"/docs/excel/list-in-map/#nested-in-horizontal-map",title:"List in map / Nested in horizontal-map ",description:"Excel list in map guide.",content:`
`}).add({id:255,href:"/docs/excel/list-in-map/#horizontal-list-in-horizontal-map",title:"List in map / Horizontal-list in horizontal-map ",description:"Excel list in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:256,href:"/docs/excel/list-in-map/#incell-list-in-horizontal-map",title:"List in map / Incell-list in horizontal-map ",description:"Excel list in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
Generated:
hello_world.proto ItemConf.json `}).add({id:257,href:"/docs/excel/map-in-list/#nested-in-vertical-list",title:"Map in list / Nested in vertical-list ",description:"Excel map in list guide.",content:`
`}).add({id:258,href:"/docs/excel/map-in-list/#horizontal-map-in-vertical-list",title:"Map in list / Horizontal-map in vertical-list ",description:"Excel map in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:259,href:"/docs/excel/map-in-list/#vertical-map-in-vertical-keyed-list",title:"Map in list / Vertical-map in vertical-keyed-list ",description:"Excel map in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:260,href:"/docs/excel/map-in-list/#incell-map-in-vertical-list",title:"Map in list / Incell-map in vertical-list ",description:"Excel map in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:261,href:"/docs/excel/map-in-list/#first-field-in-horizontal-list",title:"Map in list / First-field in horizontal-list ",description:"Excel map in list guide.",content:`
`}).add({id:262,href:"/docs/excel/map-in-list/#horizontal-map-in-horizontal-list",title:"Map in list / Horizontal-map in horizontal-list ",description:"Excel map in list guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:263,href:"/docs/excel/map-in-list/#predefined-struct-map-in-horizontal-list",title:"Map in list / Predefined-struct-map in horizontal-list ",description:"Excel map in list guide.",content:` Item in common.proto is predefined as:
A worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Reward1Item1ID Reward1Item1Num Reward1Item2ID Reward1Item2Num Reward1Name Reward2Item1ID Reward2Item1Num Reward2Name [Reward]map&lt;int32, .Item&gt; int32 int32 int32 string int32 int32 string Item1&rsquo;s ID Item1&rsquo;s num Item2&rsquo;s ID Item2&rsquo;s num Reward&rsquo;s name Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name 1 10 2 20 Lotto 10 100 Super Lotto Generated:
hello_world.proto ItemConf.json `}).add({id:264,href:"/docs/excel/map-in-map/#nested-in-vertical-map",title:"Map in map / Nested in vertical-map ",description:"Excel map in map guide.",content:`
`}).add({id:265,href:"/docs/excel/map-in-map/#horizontal-map-in-vertical-map",title:"Map in map / Horizontal-map in vertical-map ",description:"Excel map in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:266,href:"/docs/excel/map-in-map/#vertical-map-in-vertical-map",title:"Map in map / Vertical-map in vertical-map ",description:"Excel map in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:267,href:"/docs/excel/map-in-map/#incell-map-in-vertical-map",title:"Map in map / Incell-map in vertical-map ",description:"Excel map in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:268,href:"/docs/excel/map-in-map/#nested-in-horizontal-map",title:"Map in map / Nested in horizontal-map ",description:"Excel map in map guide.",content:`
`}).add({id:269,href:"/docs/excel/map-in-map/#horizontal-map-in-horizontal-map",title:"Map in map / Horizontal-map in horizontal-map ",description:"Excel map in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:270,href:"/docs/excel/map-in-map/#incell-map-in-horizontal-map",title:"Map in map / Incell-map in horizontal-map ",description:"Excel map in map guide.",content:` A worksheet ItemConf in HelloWorld.xlsx:
...
hello_world.proto ItemConf.json `}).add({id:271,href:"/docs/excel/infinite-nesting/#overview",title:"Infinite nesting / Overview ",description:"Excel infinite nesting guide.",content:` Now, the horizontal/vertical list element&rsquo;s first field can be any type, even as struct, list, and map.
...
`}).add({id:272,href:"/docs/excel/infinite-nesting/#nested-naming",title:"Infinite nesting / Nested naming ",description:"Excel infinite nesting guide.",content:` Predefined types in &ldquo;common.proto&rdquo;:
...
hello_world.proto loader_conf.json `}).add({id:273,href:"/docs/excel/field-property/#overview",title:"Field property / Overview ",description:"Tableau field property guide.",content:` Option Type Description unique bool Check field uniqueness. Default: false. Specially for map (or KeyedList) key, default will be auto deduced. range string Format: &quot;left,right&quot;. E.g.: &quot;1,10&quot;, &quot;1,~&quot;, &quot;~,10&quot;. Different interpretations of range: - number: value range. - string: count of utf-8 code point. refer string Format: &quot;SheetName(SheetAlias).ColumnName&quot;.
Ensure this field is in another sheet&rsquo;s column value space. Multiple refers are comma-separated. sequence int64 Ensure this field&rsquo;s value is a sequence and begins with this value. default string Use this default value if cell is empty (not present). fixed bool Auto-detected fixed size of horizontal list/map. Default: false. size uint32 Specify fixed size of horizontal list/map. form Form Specify cell data form of incell struct.
- FORM_TEXT
- FORM_JSON json_name string Specify field&rsquo;s custom JSON name instead of lowerCamelCase name of proto field name. present bool Must fill cell data explicitly if present is true. Default: false. optional bool Whether this field is optional (field name existence). patch Patch Field patch type. - PATCH_REPLACE - PATCH_MERGE sep string Field-level separator. subsep string Field-level subseparator. cross int32 Specify count of crossed nodes/cells/fields of composite types with cardinality, such as list and map. pattern string Specify the pattern of scalar, list element, and map value. `}).add({id:274,href:"/docs/excel/field-property/#option-unique",title:"Field property / Option unique ",description:"Tableau field property guide.",content:` Option unique can be specified as true or false in the field property. It can check the uniqueness of any scalar field in list/map element.
If you set unique to true explicitly, tableau will report an error if a duplicate key appears. If you set unique to false explicitly, no check will be performed. `}).add({id:275,href:"/docs/excel/field-property/#map-or-keyedlist-key",title:"Field property / Map (or KeyedList) key ",description:"Tableau field property guide.",content:` Tableau will auto deduce the map (or KeyedList) key&rsquo;s unique as true or not.
...
`}).add({id:276,href:"/docs/excel/field-property/#general-scalar-field",title:"Field property / General scalar field ",description:"Tableau field property guide.",content:` If you specify a general scalar field&rsquo;s property unique as true, then tableau will check the field&rsquo;s uniquness in map or list.
`}).add({id:277,href:"/docs/excel/field-property/#option-range",title:"Field property / Option range ",description:"Tableau field property guide.",content:` [!WARNING] This check option is skipped when cell data is empty (not present). To enforce the check even on empty cells, set option present to true.
Option range can be specified as format: &quot;left,right&quot; (left and right are both inclusive).
Different interpretations of range:
number: value range, e.g.: &quot;1,10&quot;, &quot;1,~&quot;, &quot;~,10&quot;. string: count of utf-8 code point. list: length of list. map: length of map. `}).add({id:278,href:"/docs/excel/field-property/#option-refer",title:"Field property / Option refer ",description:"Tableau field property guide.",content:` Option refer is similar to the FOREIGN KEY constraint in SQL, preventing actions that would break links between tables. Unlike SQL foreign keys, it can reference any sheet&rsquo;s column — not just map key columns — and supports multiple refers (comma-separated). It ensures this field&rsquo;s value exists in at least one of the referenced columns (i.e., the field value space of another sheet).
Format: &quot;SheetName(SheetAlias).ColumnName[,SheetName(SheetAlias).ColumnName]...&quot;.
For example:
map&lt;uint32, Reward&gt;|{refer:&quot;ItemConf.ID&quot;}: single-refer without alias, so sheet name is just the generated protobuf message name. map&lt;uint32, Reward&gt;|{refer:&quot;ItemConf.ID,EquipConf.ID&quot;}: multi-refer without alias, then sheet alias is the generated protobuf message name. map&lt;uint32, Reward&gt;|{refer:&quot;Sheet1(ItemConf).ID&quot;}: single-refer with alias, then sheet alias is the generated protobuf message name. `}).add({id:279,href:"/docs/excel/field-property/#option-sequence",title:"Field property / Option sequence ",description:"Tableau field property guide.",content:` Option sequence is used to ensure this field’s value is a sequence and begins with this value. It can be used for any fields even in nested list/map.
For example:
map&lt;uint32, Item&gt;|{sequence:1}: this map key must follow the sequence rule which begins with value 1. int32|{sequence:1}: the parent list/map elements must follow the sequence rule which begins with value 1. `}).add({id:280,href:"/docs/excel/field-property/#option-default",title:"Field property / Option default ",description:"Tableau field property guide.",content:` If option default is set, then use it as default value if cell is empty (not present).
`}).add({id:281,href:"/docs/excel/field-property/#option-fixed",title:"Field property / Option fixed ",description:"Tableau field property guide.",content:` If option fixed is set as true, then auto-detect fixed size of horizontal list/map.
For example:
List: implicit fixed size Map: implicit fixed size `}).add({id:282,href:"/docs/excel/field-property/#option-size",title:"Field property / Option size ",description:"Tableau field property guide.",content:` Option size is used to specify fixed size of horizontal list/map.
For example:
List: explicit fixed size Map: explicit fixed size `}).add({id:283,href:"/docs/excel/field-property/#option-form",title:"Field property / Option form ",description:"Tableau field property guide.",content:` Option form is used to specify cell data form of incell struct.
...
`}).add({id:284,href:"/docs/excel/field-property/#option-json_name",title:"Field property / Option json_name ",description:"Tableau field property guide.",content:` By default, JSON name is deduced from the field&rsquo;s proto name by converting it to camelCase. Now you can explicitly specify it by json_name prop option.
For example, a worksheet ItemConf in HelloWorld.xlsx:
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Rarity_1 SpecialEffect_2 map&lt;int32, Item&gt; int32|{json_name:&ldquo;rarity_1&rdquo;} int32|{json_name:&ldquo;specialEffect_2&rdquo;} Item&rsquo;s ID Item&rsquo;s rarity. Item&rsquo;s special effect. 1 10 101 2 20 102 3 30 103 `}).add({id:285,href:"/docs/excel/field-property/#option-present",title:"Field property / Option present ",description:"Tableau field property guide.",content:` If option present is set as true, then cell data cannot be empty and must be filled explicitly. Otherwise an error will be reported.
`}).add({id:286,href:"/docs/excel/field-property/#option-optional",title:"Field property / Option optional ",description:"Tableau field property guide.",content:` Specify whether this field is optional (field name existence).
If set to true, then:
table formats (Excel/CSV): field&rsquo;s column can be absent. document formats (XML/YAML): field&rsquo;s name can be absent. `}).add({id:287,href:"/docs/excel/field-property/#option-patch",title:"Field property / Option patch ",description:"Tableau field property guide.",content:` See field-level patch in Option Patch.
`}).add({id:288,href:"/docs/excel/field-property/#option-sep",title:"Field property / Option sep ",description:"Tableau field property guide.",content:` Field-level separator for separating:
incell list elements (scalar or struct). incell map items. If not set, it will use sheet-level seq in metasheet.
`}).add({id:289,href:"/docs/excel/field-property/#option-subsep",title:"Field property / Option subsep ",description:"Tableau field property guide.",content:` Field-level subseparator for separating:
...
`}).add({id:290,href:"/docs/excel/field-property/#option-cross",title:"Field property / Option cross ",description:"Tableau field property guide.",content:` Specify count of crossed nodes/cells/fields of composite types with cardinality, such as list and map.
`}).add({id:291,href:"/docs/excel/field-property/#union-list-field",title:"Field property / union list field ",description:"Tableau field property guide.",content:` TODO: example illustrated.
...
Value 0 means it is an incell list. Value &gt; 0 means it is a horizontal list occupying N fields. Value &lt; 0 means it is a horizontal list occupying all following fields. `}).add({id:292,href:"/docs/excel/field-property/#option-pattern",title:"Field property / Option pattern ",description:"Tableau field property guide.",content:` Specify the pattern of scalar field, list element, and map value.
`}).add({id:293,href:"/docs/excel/field-property/#wellknown-version-field",title:"Field property / Wellknown version field ",description:"Tableau field property guide.",content:` For use cases, see Wellknown types: Version
...
`}).add({id:294,href:"/docs/excel/metasheet/#overview",title:"Metasheet / Overview ",description:"Excel metasheet @TABLEAU guide.",content:` Options below can be specified in the metasheet @TABLEAU to affect the corresponding worksheet&rsquo;s layout, ability, loader and so on.
...
- Multi-column OrderedIndex format: (Column1,Column2,...)&lt;ColumnX,ColumnY,...&gt;@IndexName. LangOptions map&lt;string, string&gt; Specify loader language options. Valid keys are: OrderedMap, Index. Different kvs must be seperated by , and one key value must be seperated by :. If one key doesn&rsquo;t exist in map, it means that this loader option is supported in all languages. Valid values are all combinations of cpp, go with space as seperator. Examples: - OrderedMap:cpp,Index:cpp go // ordered map supported in cpp, index supported in cpp and go - OrderedMap:cpp // ordered map supported in cpp, index supported in all languages `}).add({id:295,href:"/docs/excel/metasheet/#empty-tableau",title:"Metasheet / Empty @TABLEAU ",description:"Excel metasheet @TABLEAU guide.",content:` If metasheet @TABLEAU is empty, then all other worksheets in the same workbook will be processed.
`}).add({id:296,href:"/docs/excel/metasheet/#a-simple-example",title:"Metasheet / A simple example ",description:"Excel metasheet @TABLEAU guide.",content:` There is a worksheet Sheet1 in HelloWorld.xlsx, we want to rename sheet to ItemConf, define custom seperator as |, and generate ordered map accessers.
So the metasheet @TABLEAU in HelloWorld.xlsx should be configured as:
HelloWorld.xlsx&nbsp; Sheet1 @TABLEAU ID Name map&lt;uint32, Item&gt; string Item&rsquo;s ID Item&rsquo;s Name 1 Apple 2 Orange 3 Banana Sheet Alias Sep OrderedMap Sheet1 ItemConf | true `}).add({id:297,href:"/docs/excel/metasheet/#workbook-alias",title:"Metasheet / Workbook Alias ",description:"Excel metasheet @TABLEAU guide.",content:` The generated proto file name is the snake case of input file name. For example, if you have a workbook named HelloWorld.xlsx, the generated proto file name is hello_world.proto. If you want to manually specify a name for the generated proto file, you can also use the Alias option. In this scenario, # refers to the workbook name.
A worksheet ItemConf in HelloWorld.xlsx:
...
custom_conf.proto `}).add({id:298,href:"/docs/excel/metasheet/#option-mode",title:"Metasheet / Option Mode ",description:"Excel metasheet @TABLEAU guide.",content:` Sheet mode defines how tableauc (protogen) parses the sheet: data or types.
Available modes:
MODE_DEFAULT: Default mode, which defines sheet&rsquo;s data structure. MODE_ENUM_TYPE: Define single enum type in a sheet, see Example. MODE_ENUM_TYPE_MULTI: Define multiple enum types in a sheet, see Example. MODE_STRUCT_TYPE: Define single struct type in a sheet, see Example. MODE_STRUCT_TYPE_MULTI: Define multiple struct types in a sheet, see Example. MODE_UNION_TYPE: Define single union type in a sheet, see Example. MODE_UNION_TYPE_MULTI: Define multiple union types in a sheet, see Example. `}).add({id:299,href:"/docs/excel/metasheet/#option-transpose",title:"Metasheet / Option Transpose ",description:"Excel metasheet @TABLEAU guide.",content:` In linear algebra, transpose of a matrix is an operator which flips a matrix over its diagonal. Likewise, transpose of a sheet (2D matrix) means interchanging its rows into columns or vice versa.
...
hello_world.proto HeroConf.json `}).add({id:300,href:"/docs/excel/metasheet/#option-merger",title:"Metasheet / Option Merger ",description:"Excel metasheet @TABLEAU guide.",content:` Option Merger is used to merge multiple sheets (comma-separated) with same schema to one.
Each element can be:
just a workbook file path or Glob path (relative to this workbook): &lt;Workbook&gt;, then the sheet name is the same as this sheet. a workbook file path (relative to this workbook) with a worksheet name: &lt;Workbook&gt;#&lt;Worksheet&gt;. [!NOTE] The glob pattern should not match the main workbook. If it does, tableauc will automatically exclude it.
`}).add({id:301,href:"/docs/excel/metasheet/#merging-multiple-workbooks",title:"Metasheet / Merging multiple workbooks ",description:"Excel metasheet @TABLEAU guide.",content:` For example, there are three workbooks, each containing a worksheet ZoneConf with the same schema:
...
merger_main.proto HeroConf.json `}).add({id:302,href:"/docs/excel/metasheet/#merging-multiple-sheets-in-same-workbook",title:"Metasheet / Merging multiple sheets in same workbook ",description:"Excel metasheet @TABLEAU guide.",content:` For example, there are three worksheets with the same schema in the same workbook Merger.xlsx:
...
Merger.xlsx&nbsp; ZoneConf ZoneConf2 ZoneConf3 @TABLEAU ID Name Difficulty map&lt;uint32, Zone&gt; string int32 Zone&rsquo;s ID Zone&rsquo;s name Zone&rsquo;s difficulty 1 Infinity 100 ID Name Difficulty map&lt;uint32, Zone&gt; string int32 Zone&rsquo;s ID Zone&rsquo;s name Zone&rsquo;s difficulty 2 Desert 200 ID Name Difficulty map&lt;uint32, Zone&gt; string int32 Zone&rsquo;s ID Zone&rsquo;s name Zone&rsquo;s difficulty 3 Snowfield 300 Sheet Merger ZoneConf Merger.xlsx#ZoneConf2,Merger.xlsx#ZoneConf3 [!NOTE] Use &lt;Workbook&gt;#&lt;Worksheet&gt; to refer to a specific sheet in a workbook.
Generated:
merger_same.proto ZoneConf.json `}).add({id:303,href:"/docs/excel/metasheet/#option-scatter",title:"Metasheet / Option Scatter ",description:"Excel metasheet @TABLEAU guide.",content:` Option Scatter is used to scatter multiple sheets (comma-separated) with same schema to different generated config files.
Each element can be:
just a workbook file path or Glob path (relative to this workbook): &lt;Workbook&gt;, then the sheet name is the same as this sheet. a workbook file path (relative to this workbook) with a worksheet name: &lt;Workbook&gt;#&lt;Worksheet&gt;. [!NOTE] The glob pattern should not match the main workbook. If it does, tableauc will automatically exclude it.
...
Scatter1_ZoneConf.json Scatter2_ZoneConf.json Scatter3_ZoneConf.json `}).add({id:304,href:"/docs/excel/metasheet/#option-orderedmap",title:"Metasheet / Option OrderedMap ",description:"Excel metasheet @TABLEAU guide.",content:` [!IMPORTANT] It only applies to each level message’s first map field.
If you set OrderedMap to true, then tableau loader plugins will generate ordered map APIs:
C++: OrderedMap API Go: OrderedMap API `}).add({id:305,href:"/docs/excel/metasheet/#option-index",title:"Metasheet / Option Index ",description:"Excel metasheet @TABLEAU guide.",content:` Option Index can be specified to generate index accessers. There are two kinds of indexes:
...
Examples:
ID ID@Item ID&lt;ID&gt;@Item: sort result array by ID. ID&lt;Type,Priority&gt;@Item: sort result array by Type and Priority. ID, Name@AwardItem ID@Item, Name@AwardItem `}).add({id:307,href:"/docs/excel/metasheet/#multi-column-index",title:"Metasheet / Multi-column Index ",description:"Excel metasheet @TABLEAU guide.",content:` Format: (Column1,Column2,...)&lt;ColumnX,ColumnY,...&gt;@IndexName.
...
Examples:
(ID,Name): index name not set, then determined by parent struct type name. (ID,Name)@AwardItem (ID,Name)&lt;ID&gt;: sort result array by ID. (ID,Type)&lt;Type,Priority&gt;@Item: sort result array by Type and Priority. ID@Item, (ID,Name)@AwardItem: one single-column index and one multi-column index. `}).add({id:308,href:"/docs/excel/metasheet/#option-orderedindex",title:"Metasheet / Option OrderedIndex ",description:"Excel metasheet @TABLEAU guide.",content:` Option OrderedIndex can be specified to generate ordered index accessers. There are two kinds of ordered indexes:
...
C++: Index API Go: Index API Each column type can be:
scalar: numbers, booleans, strings, and bytes. enum: e.g.: enum&lt;.FruitType&gt; incell scalar list: e.g: []int32 incell enum list: e.g: []enum&lt;.FruitType&gt; Example: two worksheets ItemConf and ShopConf in HelloWorld.xlsx:
ItemConf: ordered index on columns of the same struct as map value. ShopConf: ordered index on columns of the same struct as list element. HelloWorld.xlsx&nbsp; ItemConf ShopConf @TABLEAU ID Name Desc map&lt;int32, Item&gt; string string Item&rsquo;s ID Item&rsquo;s name Item&rsquo;s desc 1 Apple A kind of delicious fruit. 2 Orange A kind of sour fruit. 3 Banana A kind of calorie-rich fruit. ID Type Desc [Shop]int32 int32 string Shop&rsquo;s ID Shop&rsquo;s type Shop&rsquo;s desc 1 1 Shoes shop. 2 1 T-Shirt shop. 3 2 Fruite shop. Sheet OrderedIndex ItemConf ID@Item, Name@AwardItem, (ID,Name)@SpecialItem ShopConf ID@Shop, Type@ThemeShop, (ID,Type)@SpecialShop, (ID,Type)@Shop Example with sorting: two worksheets ItemConf and ShopConf in HelloWorld.xlsx, with sorted result arrays using &lt;&gt; syntax:
ItemConf: ordered index sorted by Name column. ShopConf: ordered index sorted by Type and ID columns. HelloWorld.xlsx&nbsp; ItemConf ShopConf @TABLEAU ID Name Desc map&lt;int32, Item&gt; string string Item&rsquo;s ID Item&rsquo;s name Item&rsquo;s desc 1 Apple A kind of delicious fruit. 2 Orange A kind of sour fruit. 3 Banana A kind of calorie-rich fruit. ID Type Desc [Shop]int32 int32 string Shop&rsquo;s ID Shop&rsquo;s type Shop&rsquo;s desc 1 1 Shoes shop. 2 1 T-Shirt shop. 3 2 Fruite shop. Sheet OrderedIndex ItemConf ID@Item, (ID,Name)@SpecialItem ShopConf ID&lt;Type,ID&gt;@Shop, (ID,Type)&lt;Type,ID&gt;@SpecialShop, (ID,Type)@Shop `}).add({id:309,href:"/docs/excel/metasheet/#single-column-orderedindex",title:"Metasheet / Single-column OrderedIndex ",description:"Excel metasheet @TABLEAU guide.",content:` Format: Column&lt;ColumnX,ColumnY,...&gt;@IndexName.
The sign @ is the separator between column name and index name. if IndexName is not set, it will be this column’s parent struct type name. One or more indexes can be specified by comma-separated rule. The columns in the angle brackets &lt;&gt; specify the sorting columns, which the result array of same index key sort by.
Examples:
ID ID@Item ID&lt;ID&gt;@Item: sort result array by ID. ID&lt;Type,Priority&gt;@Item: sort result array by Type and Priority. ID, Name@AwardItem ID@Item, Name@AwardItem Example with sorting: a worksheet ItemConf in HelloWorld.xlsx, with sorted result array using &lt;&gt; syntax:
ID&lt;Name&gt;@Item: single-column ordered index on ID, result array sorted by Name. HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Name Desc map&lt;int32, Item&gt; string string Item&rsquo;s ID Item&rsquo;s name Item&rsquo;s desc 1 Apple A kind of delicious fruit. 2 Orange A kind of sour fruit. 3 Banana A kind of calorie-rich fruit. Sheet OrderedIndex ItemConf ID@Item `}).add({id:310,href:"/docs/excel/metasheet/#multi-column-orderedindex",title:"Metasheet / Multi-column OrderedIndex ",description:"Excel metasheet @TABLEAU guide.",content:` Format: (Column1,Column2,...)&lt;ColumnX,ColumnY,...&gt;@IndexName.
...
The sign @ is the separator between enclosed column names by parentheses and index name. If IndexName is not set, it will be this column&rsquo;s parent struct type name. One or more indexes can be specified by comma-separated rule. The columns in the angle brackets &lt;&gt; specify the sorting columns, which the result array of same index key sort by.
Examples:
(ID,Name): index name not set, then determined by parent struct type name. (ID,Name)@AwardItem (ID,Name)&lt;ID&gt;: sort result array by ID. (ID,Type)&lt;Type&gt;@Shop: sort result array by Type. (ID,Type)&lt;Type,Priority&gt;@Item: sort result array by Type and Priority. ID@Item, (ID,Name)@AwardItem: one single-column ordered index and one multi-column ordered index. Example: Multi-column OrderedIndex # Two worksheets ItemConf and ShopConf in HelloWorld.xlsx:
ItemConf: multi-column ordered index on columns of the same struct as map value. ShopConf: multi-column ordered index on columns of the same struct as list element. HelloWorld.xlsx&nbsp; ItemConf ShopConf @TABLEAU ID Name Desc map&lt;int32, Item&gt; string string Item&rsquo;s ID Item&rsquo;s name Item&rsquo;s desc 1 Apple A kind of delicious fruit. 2 Orange A kind of sour fruit. 3 Banana A kind of calorie-rich fruit. ID Type Desc [Shop]int32 int32 string Shop&rsquo;s ID Shop&rsquo;s type Shop&rsquo;s desc 1 1 Shoes shop. 2 1 T-Shirt shop. 3 2 Fruite shop. Sheet OrderedIndex ItemConf (ID,Name)@SpecialItem ShopConf (ID,Type)@SpecialShop, (ID,Type)@Shop Example: Multi-column OrderedIndex with &lt;&gt; syntax # A worksheet ShopConf in HelloWorld.xlsx, with sorted result array using &lt;&gt; syntax:
(ID,Type)&lt;Type&gt;@Shop: multi-column ordered index on (ID, Type), result array sorted by Type. (ID,Type)&lt;Type,ID&gt;@SpecialShop: multi-column ordered index on (ID, Type), result array sorted by Type then ID. HelloWorld.xlsx&nbsp; ShopConf @TABLEAU ID Type Desc [Shop]int32 int32 string Shop&rsquo;s ID Shop&rsquo;s type Shop&rsquo;s desc 1 1 Shoes shop. 2 1 T-Shirt shop. 3 2 Fruite shop. Sheet OrderedIndex ShopConf (ID,Type)@Shop, (ID,Type)&lt;Type,ID&gt;@SpecialShop `}).add({id:311,href:"/docs/excel/metasheet/#option-patch",title:"Metasheet / Option Patch ",description:"Excel metasheet @TABLEAU guide.",content:`

`}).add({id:312,href:"/docs/excel/metasheet/#option-sep",title:"Metasheet / Option Sep ",description:"Excel metasheet @TABLEAU guide.",content:` Sheet-level separator for separating:
...
`}).add({id:313,href:"/docs/excel/metasheet/#option-subsep",title:"Metasheet / Option Subsep ",description:"Excel metasheet @TABLEAU guide.",content:` Sheet-level subseparator for separating:
...
`}).add({id:314,href:"/docs/basics/",title:"Basics",description:"Basics of Tableau.",content:""}).add({id:315,href:"/docs/basics/concepts/#terminology",title:"Concepts / Terminology ",description:"Core concepts of Tableau.",content:`
`}).add({id:316,href:"/docs/basics/concepts/#basics",title:"Concepts / Basics ",description:"Core concepts of Tableau.",content:` Term Definition Workbook An excel file.
A bundle of CSV files named with the same prefix seperated by #. A XML file. A YAML file. Worksheet A sheet in a excel file. A CSV file. A root node of a XML file. A document in YAML file. Metasheet A worksheet named @TABLEAU to specify tableau parser options. Row The row in a sheet. Column The column in a sheet. Cell The intersection of a row and a column. In-cell The inner-side of a cell. Cross-cell Continuous cells of a row or a column. `}).add({id:317,href:"/docs/basics/concepts/#worksheet",title:"Concepts / Worksheet ",description:"Core concepts of Tableau.",content:` Term Definition Namerow Exact row number of column name definition at a worksheet.
...
Default: false. Layout Incell, vertical(cross-cell) or horizontal(cross-cell). Transpose Interchanging the rows and columns of a given sheet. `}).add({id:318,href:"/docs/basics/concepts/#mappings-to-protoconf",title:"Concepts / Mappings to Protoconf ",description:"Core concepts of Tableau.",content:" Term Protoconf Workbook One protoconf(.proto) file. Worksheet One top-level message in a protoconf file, except the tableau metasheet named @TABLEAU. column One field in a message "}).add({id:319,href:"/docs/basics/concepts/#a-simple-mapping-example",title:"Concepts / A simple mapping example ",description:"Core concepts of Tableau.",content:`
`}).add({id:320,href:"/docs/basics/concepts/#input-an-excel-file",title:"Concepts / Input: an excel file ",description:"Core concepts of Tableau.",content:` A workbook(HelloWorld.xlsx) with two data worksheets(ItemConf and ActivityConf) and an empty tableau metasheet(@TABLEAU).
HelloWorld.xlsx&nbsp; ItemConf ActivityConf @TABLEAU ID Name Type map&lt;uint32, Item&gt; string int32 Item&rsquo;s ID. Item&rsquo;s name. Item&rsquo;s type. 1 item1 100 2 item2 200 3 item3 300 ID Name Open map&lt;uint32, Activity&gt; string bool Activity&rsquo;s ID. Activity&rsquo;s name. Activity is open? 1 activity1 true 2 activity2 false 3 activity3 `}).add({id:321,href:"/docs/basics/concepts/#output-a-protoconf-file",title:"Concepts / Output: a protoconf file ",description:"Core concepts of Tableau.",content:` A protoconf file(hello_world.proto) with two top-level messages(ItemConf and ActivityConf).
hello_world.proto `}).add({id:322,href:"/docs/basics/naming-convention/#enums",title:"Naming convention / Enums ",description:"Naming convention.",content:` Use PascalCase (with an initial capital) for enum type names and CAPITALS_WITH_UNDERSCORES for value names:
See Protobuf style: enums.
`}).add({id:323,href:"/docs/basics/naming-convention/#examples",title:"Naming convention / Examples ",description:"Naming convention.",content:" Name Style Example workbook PascalCase HelloWorld.xlsx worksheet PascalCase HelloWorld struct (message) PascalCase HelloWorld field (column) PascalCase HelloWorld "}).add({id:324,href:"/docs/basics/grammar-and-types/#overview",title:"Grammar and types / Overview ",description:"Grammar and types.",content:` Tableau borrows most of its syntax and types from Protocol Buffers (proto3) and Golang.
`}).add({id:325,href:"/docs/basics/grammar-and-types/#scalar-types",title:"Grammar and types / Scalar types ",description:"Grammar and types.",content:` Details disccused at Protocol Buffers Proto3 Scalar.
Kind Types Default Numbers int32, uint32
int64, uint64
float, double 0
0
0.0 Booleans bool false Strings string &quot;&quot; Bytes bytes &quot;&quot; `}).add({id:326,href:"/docs/basics/grammar-and-types/#composite-types",title:"Grammar and types / Composite types ",description:"Grammar and types.",content:" Type Description struct A struct is mapped to a protobuf message. list A list is mapped to a protobuf repeated field. map A map is mapped to a protobuf map field. "}).add({id:327,href:"/docs/basics/grammar-and-types/#struct",title:"Grammar and types / struct ",description:"Grammar and types.",content:" Feature Description Horizontal layout Each scalar field located in one cell. Simple incell struct Each field must be scalar type. It is a comma-separated list of fields. E.g.: 1,test,3.0. If the data list&rsquo;s size is not same as struct&rsquo;s fields, then fields will be filled in order. Fields not configured will be filled with default values due to its scalar type. "}).add({id:328,href:"/docs/basics/grammar-and-types/#list",title:"Grammar and types / list ",description:"Grammar and types.",content:" Feature Description Horizontal layout This is list&rsquo;s default layout. Element type can be struct or scalar. Vertical layout List&rsquo;s element type should be struct. Simple incell list Element type must be scalar. It is a comma-separated list of elements. E.g.: 1,2,3. Scalable Scalable or dynamic list size. Ignore empty element Smart recognition of empty element at any position. "}).add({id:329,href:"/docs/basics/grammar-and-types/#map",title:"Grammar and types / map ",description:"Grammar and types.",content:` Feature Description Horizontal layout Vertical layout This is map&rsquo;s default layout. Hash map Implemented as unordered map or hash map. Ordered map Supported by tableauio/loader.
- C++ Simple incell map Both key and value must be scalar type. It is a comma-separated list of key:value pairs. E.g.: 1:10,2:20,3:30. Scalable Scalable or dynamic map size. Ignore empty item Smart recognition of empty item at any position. `}).add({id:330,href:"/docs/basics/grammar-and-types/#enumeration",title:"Grammar and types / Enumeration ",description:"Grammar and types.",content:` Feature Description Three forms of enum value 1. Enum value number.
2. Enum value name.
3. Enum value alias name (with EnumValueOptions specified). Validation Auto-check legality of enum values. `}).add({id:331,href:"/docs/basics/grammar-and-types/#empty-value",title:"Grammar and types / Empty value ",description:"Grammar and types.",content:` Type Description scalar Empty scalar will be emplaced with scalar type&rsquo;s default value. struct Empty struct will not be spawned if all fields are empty. list Empty list will not be spawned if list&rsquo;s size is 0.
Empty struct will not be appended if list&rsquo;s element(struct type) is empty. map Empty map will not be spawned if map&rsquo;s size is 0. Empty struct will not be inserted if map&rsquo;s value(struct type) is empty. nesting Recursively empty. `}).add({id:332,href:"/docs/basics/enum/#enum-value",title:"Enum / Enum value ",description:"Enum basics.",content:` The tableau parser accepts three enum value forms:
...
`}).add({id:333,href:"/docs/basics/enum/#validation",title:"Enum / Validation ",description:"Enum basics.",content:` As enum type is predefined, so the tableau parser will auto validate the enum value.
`}).add({id:334,href:"/docs/basics/wellknown-types/#overview",title:"Wellknown types / Overview ",description:"Wellknown types.",content:` For easy use, Wellknown types are built-in types in Tableau. This concept is much like Protocol Buffers Well-Known Types.
You should include the proto files provided by Tableau and Protocol Buffers:
tableau/protobuf/wellknown.proto google/protobuf/timestamp.proto google/protobuf/duration.proto `}).add({id:335,href:"/docs/basics/wellknown-types/#datetime",title:"Wellknown types / Datetime ",description:"Wellknown types.",content:` See Wellknown types: Datetime for use cases.
...
duration is based on google.protobuf.Duration, see JSON mapping. golang duration string form. golang ParseDuration. `}).add({id:337,href:"/docs/basics/wellknown-types/#fraction",title:"Wellknown types / Fraction ",description:"Wellknown types.",content:` See Wellknown types: Fraction for use cases.
...
- N: floating-point numerator, e.g.: 0.01 is same to 1/100 `}).add({id:338,href:"/docs/basics/wellknown-types/#comparator",title:"Wellknown types / Comparator ",description:"Wellknown types.",content:` See Wellknown types: Comparator for use cases.
A comparator holds a sign and a fraction value. Any number or fraction can compare with it.
Type Default Description comparator ==0 Format: &lt;Sign&gt;&lt;Fraction&gt;. e.g.: ==10, !=1/2, &lt;10%, &lt;=10‰, &gt;10%, &gt;=10‱ `}).add({id:339,href:"/docs/basics/wellknown-types/#version",title:"Wellknown types / Version ",description:"Wellknown types.",content:` See Wellknown types: Version for use cases.
...
Type Default Description version &quot;&quot; Format: &lt;MAJOR&gt;.&lt;MINOR&gt;.&lt;PATCH&gt;. e.g.: 1.0.1 version|{pattern:&quot;255.255.255.255&quot;} &quot;&quot; Format: &lt;MAJOR&gt;.&lt;MINOR&gt;.&lt;PATCH&gt;.&lt;OTHER&gt;. e.g.: 1.0.1.1 `}).add({id:340,href:"/docs/basics/predefined-types/#overview",title:"Predefined types / Overview ",description:"Predefined types.",content:` You can define enum, struct, or union types in a protoconf file (such as common.proto) ahead. Then use them to specify the column type or cross-cell type of a worksheet.
`}).add({id:341,href:"/docs/basics/predefined-types/#usage",title:"Predefined types / Usage ",description:"Predefined types.",content:" Syntax: prepend a dot . to predefined CustomType (a.k.a. .CustomType) when you use it in a worksheet. Import: specify the protoFiles option of tableauc config to import the common proto files, where predefined enum, struct, union types are defined. See Tableauc config. "}).add({id:342,href:"/docs/basics/predefined-types/#enum",title:"Predefined types / Enum ",description:"Predefined types.",content:` For example, enum type FruitType in common.proto is defined as:
There are some examples to demonstrate how to use predefined enum types:
Excel/CSV: Use predefined enum type. XML: Use predefined enum type YAML: Use predefined enum type `}).add({id:343,href:"/docs/basics/predefined-types/#struct",title:"Predefined types / Struct ",description:"Predefined types.",content:` For example, struct type Prop in common.proto is defined as:
...
`}).add({id:344,href:"/docs/basics/predefined-types/#union",title:"Predefined types / Union ",description:"Predefined types.",content:` For example, struct type Target in common.proto is defined as:
There are some examples to demonstrate how to use predefined union types:
Excel/CSV list: Predefined union in list map: Predefined union in map XML union: Predefined union list: Predefined union list map: TODO YAML union: Predefined union list: Predefined union list map: TODO `}).add({id:345,href:"/docs/prologue/",title:"Prologue",description:"Prologue of Tableau",content:""}).add({id:346,href:"/docs/prologue/introduction/#tableauc",title:"Introduction / tableauc ",description:"Intro to Tableau.",content:` tableauc is the Tableau Compiler with protogen and confgen inside.
`}).add({id:347,href:"/docs/prologue/introduction/#protogen",title:"Introduction / protogen ",description:"Intro to Tableau.",content:` protogen converts Excel/CSV/XML/YAML files to Protoconf files. Protoconf is a dialect of Protocol Buffers (proto3) extended with tableau options, aimed to define the structure of Excel/CSV/XML/YAML.
`}).add({id:348,href:"/docs/prologue/introduction/#confgen",title:"Introduction / confgen ",description:"Intro to Tableau.",content:` confgen converts Excel/CSV/XML/YAML with Protoconf files to JSON/Text/Bin files.
`}).add({id:349,href:"/docs/prologue/quick-start/#1-download-tableauc",title:"Quick Start / 1. Download tableauc ",description:"Quick Start",content:` Select the appropriate tableauc (aka Tableau Compiler) to download:
Windows x64 Linux x64 macOS x64 arm64 More platforms are available on tableau releases .
`}).add({id:350,href:"/docs/prologue/quick-start/#2-add-a-workbook",title:"Quick Start / 2. Add a workbook ",description:"Quick Start",content:` Add HelloWorld.xlsx with two sheets:
Item: Copy data below to this worksheet. @TABLEAU: Just leave it empty now. It is the tableau metasheet for specifying parser options. HelloWorld.xlsx&nbsp; Item @TABLEAU ID Name Desc map&lt;int32, Item&gt; string string Item’s ID Item’s name Item’s description 1 Apple A kind of delicious fruit. 2 Orange A kind of sour fruit. 3 Banana A kind of calorie-rich fruit. `}).add({id:351,href:"/docs/prologue/quick-start/#3-run-tableauc",title:"Quick Start / 3. Run tableauc ",description:"Quick Start",content:` Run command: ./tableauc HelloWorld.xlsx
...
`}).add({id:352,href:"/docs/prologue/config/#configyaml",title:"Tableauc config / config.yaml ",description:"Tableauc config details",content:` Create a file named config.yaml, and copy configurations below to it:
`}).add({id:353,href:"/docs/prologue/config/#protoinputheaderseq",title:"Tableauc config / proto.input.header.seq ",description:"Tableauc config details",content:` Default: ,
...
Sheet-level separator in metasheet Field-level separator in field property `}).add({id:354,href:"/docs/prologue/config/#protoinputheadersubseq",title:"Tableauc config / proto.input.header.subseq ",description:"Tableauc config details",content:` Default: :
