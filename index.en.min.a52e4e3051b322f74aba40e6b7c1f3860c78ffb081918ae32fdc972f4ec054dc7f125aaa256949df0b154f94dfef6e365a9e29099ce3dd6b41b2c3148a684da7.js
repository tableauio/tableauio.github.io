...
`}).add({id:281,href:"/docs/excel/field-property/#option-range",title:"Field property / Option range ",description:"Tableau field property guide.",content:` [!WARNING] This check option is skipped when cell data is empty (not present). To enforce the check even on empty cells, set option present to true.
Option range can be specified as format: &quot;left,right&quot; (left and right are both inclusive).
Different interpretations of range:
number: value range, e.g.: &quot;1,10&quot;, &quot;1,~&quot;, &quot;~,10&quot;. string: count of utf-8 code point. list: length of list. map: length of map. `}).add({id:282,href:"/docs/excel/field-property/#option-refer",title:"Field property / Option refer ",description:"Tableau field property guide.",content:` Option refer is similar to the FOREIGN KEY constraint in SQL, preventing actions that would break links between tables. Unlike SQL foreign keys, it can reference any sheet&rsquo;s column — not just map key columns — and supports multiple refers (comma-separated). It ensures this field&rsquo;s value exists in at least one of the referenced columns (i.e., the field value space of another sheet).
...
For example:
map&lt;uint32, Item&gt;|{sequence:1}: this map key must follow the sequence rule which begins with value 1. int32|{sequence:1}: the parent list/map elements must follow the sequence rule which begins with value 1. `}).add({id:284,href:"/docs/excel/field-property/#option-default",title:"Field property / Option default ",description:"Tableau field property guide.",content:` If option default is set, then use it as default value if cell is empty (not present).
...
`}).add({id:305,href:"/docs/excel/metasheet/#merging-multiple-workbooks",title:"Metasheet / Merging multiple workbooks ",description:"Excel metasheet @TABLEAU guide.",content:` For example, there are three workbooks, each containing a worksheet ZoneConf with the same schema:
