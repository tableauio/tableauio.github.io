...
hello_world.proto TaskConf.json TaskConf.txt `}).add({id:161,href:"/zh/docs/excel/union/#map-中的-predefined-union",title:"Union（联合体） / Map 中的 predefined union ",description:"Excel union 使用指南。",content:` 基于 predefined union 类型 Target。
HelloWorld.xlsx 中的 worksheet TaskConf：
...
hello_world.proto TaskConf.json TaskConf.txt `}).add({id:162,href:"/zh/docs/excel/union/#map-中的-predefined-incell-union",title:"Union（联合体） / Map 中的 predefined incell union ",description:"Excel union 使用指南。",content:` 基于 predefined union 类型 Target。
HelloWorld.xlsx 中的 worksheet TaskConf：
...
{uint32 ID, int64 Damage}Boss Sheet Mode Target MODE_UNION_TYPE_MULTI 生成结果：
...
hello_world.proto ItemConf.json `}).add({id:176,href:"/zh/docs/excel/list/#纵向-list",title:"List（列表） / 纵向 list ",description:"Excel list 使用指南。",content:`
`}).add({id:177,href:"/zh/docs/excel/list/#纵向-listvertical-list",title:"List（列表） / 纵向 list（Vertical list） ",description:"Excel list 使用指南。",content:` 纵向 list 语法概览：
List 元素类型 语法示例 scalar []uint32 enum []enum&lt;.FruitType&gt; struct [Item]uint32 predefined struct [.Item]int32 incell struct []{int32 ID,int32 Num}Item incell predefined struct []{.Item} `}).add({id:178,href:"/zh/docs/excel/list/#纵向-scalar-list",title:"List（列表） / 纵向 scalar list ",description:"Excel list 使用指南。",content:` 定义方式与 Incell scalar list 相同，但如果提供了多行数据，会聚合多行。
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []uint32 ID 1,2,3 1,2 1 生成结果：
hello_world.proto ItemConf.json `}).add({id:179,href:"/zh/docs/excel/list/#纵向-enum-list",title:"List（列表） / 纵向 enum list ",description:"Excel list 使用指南。",content:` 定义方式与 Incell enum list 相同，但如果提供了多行数据，会聚合多行。
...
hello_world.proto ItemConf.json `}).add({id:180,href:"/zh/docs/excel/list/#纵向-struct-list",title:"List（列表） / 纵向 struct list ",description:"Excel list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:181,href:"/zh/docs/excel/list/#纵向-predefined-struct-list",title:"List（列表） / 纵向 predefined-struct list ",description:"Excel list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Num [.Item]int32 int32 Item&rsquo;s ID Item&rsquo;s num 1 100 2 200 3 300 生成结果：
hello_world.proto ItemConf.json `}).add({id:182,href:"/zh/docs/excel/list/#纵向-incell-struct-list",title:"List（列表） / 纵向 incell-struct list ",description:"Excel list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{int32 ID,int32 Num}Item Item list 1:100 2:200,3:300 生成结果：
hello_world.proto ItemConf.json `}).add({id:183,href:"/zh/docs/excel/list/#纵向-incell-predefined-struct-list",title:"List（列表） / 纵向 incell-predefined-struct list ",description:"Excel list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{.Item} Item&rsquo;s info 1:100 2:200,3:300 生成结果：
hello_world.proto ItemConf.json `}).add({id:184,href:"/zh/docs/excel/list/#incell-list",title:"List（列表） / Incell list ",description:"Excel list 使用指南。",content:` Incell list 语法概览：
List 元素类型 语法示例 scalar []int32 enum []enum&lt;.FruitType&gt; incell struct []{int32 ID,int32 Num}Item incell predefined struct []{.Item} `}).add({id:185,href:"/zh/docs/excel/list/#incell-scalar-list",title:"List（列表） / Incell scalar list ",description:"Excel list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:186,href:"/zh/docs/excel/list/#incell-enum-list",title:"List（列表） / Incell enum list ",description:"Excel list 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xlsx 中的 worksheet ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:187,href:"/zh/docs/excel/list/#incell-struct-list",title:"List（列表） / Incell struct list ",description:"Excel list 使用指南。",content:` 更高级的 incell 数据解析，请参考 高级 predefined incell struct →。
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{int32 ID,int32 Num}Item Item&rsquo;s info 1:100,2:200,3:300 生成结果：
hello_world.proto ItemConf.json `}).add({id:188,href:"/zh/docs/excel/list/#incell-predefined-struct-list",title:"List（列表） / Incell predefined-struct list ",description:"Excel list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{.Item} Item&rsquo;s info 1:100,2:200,3:300 生成结果：
hello_world.proto ItemConf.json `}).add({id:189,href:"/zh/docs/excel/list/#横向-list-大小",title:"List（列表） / 横向 list 大小 ",description:"Excel list 使用指南。",content:`
`}).add({id:190,href:"/zh/docs/excel/list/#动态大小",title:"List（列表） / 动态大小 ",description:"Excel list 使用指南。",content:` 默认情况下，所有 list 都是动态大小类型。List 元素应连续存在，否则如果中间存在空元素会报错。
`}).add({id:191,href:"/zh/docs/excel/list/#固定大小",title:"List（列表） / 固定大小 ",description:"Excel list 使用指南。",content:` 隐式固定大小（Implicit fixed size） # List 大小由名称行中最大存在的 list 元素数量自动确定。
...
hello_world.proto ItemConf.json 显式固定大小 # 显式固定大小（Explicit fixed size） # List 大小由 field property size 显式设置。
...
hello_world.proto ItemConf.json `}).add({id:197,href:"/zh/docs/excel/map/#横向-predefined-struct-map",title:"Map（映射） / 横向 predefined-struct map ",description:"Excel map 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:198,href:"/zh/docs/excel/map/#纵向-mapvertical-map",title:"Map（映射） / 纵向 map（Vertical map） ",description:"Excel map 使用指南。",content:` 纵向 map 有以下几种：
纵向 scalar map，map value 类型为 scalar。例如：map&lt;int32, int32&gt;。 纵向 struct map，map value 类型为 struct。例如：map&lt;int32, Item&gt;。 纵向 predefined-struct map，map value 类型为 predefined struct。例如：map&lt;int32, .Item&gt;。 `}).add({id:199,href:"/zh/docs/excel/map/#纵向-scalar-map",title:"Map（映射） / 纵向 scalar map ",description:"Excel map 使用指南。",content:` 无需单独支持，请使用 map&lt;int32, Item&gt; 代替。
`}).add({id:200,href:"/zh/docs/excel/map/#纵向-struct-map",title:"Map（映射） / 纵向 struct map ",description:"Excel map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:201,href:"/zh/docs/excel/map/#纵向-predefined-struct-map",title:"Map（映射） / 纵向 predefined-struct map ",description:"Excel map 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Num map&lt;int32, .Item&gt; int32 Item&rsquo;s ID Item&rsquo;s num 1 100 2 200 3 300 生成结果：
hello_world.proto ItemConf.json `}).add({id:202,href:"/zh/docs/excel/map/#incell-map",title:"Map（映射） / Incell map ",description:"Excel map 使用指南。",content:` Incell map 有以下几种：
Incell scalar map，map value 类型为 scalar。例如：map&lt;int32, int32&gt;。 Incell struct map，map value 类型为 struct。例如：map&lt;int32, Item&gt;。 `}).add({id:203,href:"/zh/docs/excel/map/#incell-scalar-map",title:"Map（映射） / Incell scalar map ",description:"Excel map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:204,href:"/zh/docs/excel/map/#incell-enum-map",title:"Map（映射） / Incell enum map ",description:"Excel map 使用指南。",content:` 对于 incell map，key 和 value 都可以是枚举类型。
...
hello_world.proto ItemConf.json `}).add({id:205,href:"/zh/docs/excel/map/#空-key-map",title:"Map（映射） / 空 key map ",description:"Excel map 使用指南。",content:` 如果 map key 未配置，则视为 map key 类型的默认值。默认值参见 Scalar types →。
HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:206,href:"/zh/docs/excel/map/#enum-key-map",title:"Map（映射） / Enum key map ",description:"Excel map 使用指南。",content:` 根据 protobuf 文档对 map key type 的限制：
...
hello_world.proto ItemConf.json `}).add({id:207,href:"/zh/docs/excel/map/#横向-map-大小",title:"Map（映射） / 横向 map 大小 ",description:"Excel map 使用指南。",content:`
`}).add({id:208,href:"/zh/docs/excel/map/#动态大小",title:"Map（映射） / 动态大小 ",description:"Excel map 使用指南。",content:` 默认情况下，所有 map 都是动态大小类型。Map 条目应连续存在，否则如果中间存在空条目会报错。
`}).add({id:209,href:"/zh/docs/excel/map/#固定大小",title:"Map（映射） / 固定大小 ",description:"Excel map 使用指南。",content:` 隐式固定大小（Implicit fixed size） # Map 大小由名称行中最大存在的 map 条目数量自动确定。
...
hello_world.proto ItemConf.json `}).add({id:210,href:"/zh/docs/excel/map/#高级特性",title:"Map（映射） / 高级特性 ",description:"Excel map 使用指南。",content:`
`}).add({id:211,href:"/zh/docs/excel/map/#横向跳列-maphorizontal-column-skipped-map",title:"Map（映射） / 横向跳列 map（Horizontal column-skipped map） ",description:"Excel map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:212,href:"/zh/docs/excel/map/#ordered-map",title:"Map（映射） / Ordered-map ",description:"Excel map 使用指南。",content:` 在 metasheet @TABLEAU 中将 OrderedMap 选项设置为 true，则会生成 ordered map 访问器。此功能由 tableauio/loader 提供支持。目前支持的编程语言：
...
`}).add({id:213,href:"/zh/docs/excel/keyedlist/#语法",title:"KeyedList（键控列表） / 语法 ",description:"Excel keyed list 使用指南。",content:` Keyed list 与普通 list 相同，区别在于 ColumnType（第一个字段类型）被尖括号 &lt;&gt; 包围，并作为 map key 处理。
语法：[ElemType]&lt;ColumnType&gt;
`}).add({id:214,href:"/zh/docs/excel/keyedlist/#横向-list",title:"KeyedList（键控列表） / 横向 list ",description:"Excel keyed list 使用指南。",content:` TODO&hellip;
`}).add({id:215,href:"/zh/docs/excel/keyedlist/#纵向-keyedlist",title:"KeyedList（键控列表） / 纵向 KeyedList ",description:"Excel keyed list 使用指南。",content:`
`}).add({id:216,href:"/zh/docs/excel/keyedlist/#纵向-scalar-keyedlist",title:"KeyedList（键控列表） / 纵向 scalar KeyedList ",description:"Excel keyed list 使用指南。",content:` 定义方式与 Incell scalar KeyedList 相同，但如果提供了多行数据，会进行聚合。
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []&lt;uint32&gt; ID 1,2,3 4,5 6 生成结果：
hello_world.proto ItemConf.json `}).add({id:217,href:"/zh/docs/excel/keyedlist/#纵向-enum-keyedlist",title:"KeyedList（键控列表） / 纵向 enum KeyedList ",description:"Excel keyed list 使用指南。",content:` 定义方式与 Incell enum KeyedList 相同，但如果提供了多行数据，会进行聚合。
common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Type []&lt;enum&lt;.FruitType&gt;&gt; Type Apple,Orange FRUIT_TYPE_BANANA 0 生成结果：
hello_world.proto ItemConf.json `}).add({id:218,href:"/zh/docs/excel/keyedlist/#纵向-struct-keyedlist",title:"KeyedList（键控列表） / 纵向 struct KeyedList ",description:"Excel keyed list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:219,href:"/zh/docs/excel/keyedlist/#incell-keyedlist",title:"KeyedList（键控列表） / Incell KeyedList ",description:"Excel keyed list 使用指南。",content:`
`}).add({id:220,href:"/zh/docs/excel/keyedlist/#incell-scalar-keyedlist",title:"KeyedList（键控列表） / Incell scalar KeyedList ",description:"Excel keyed list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []&lt;uint32&gt; ID list 1,2,3 生成结果：
hello_world.proto ItemConf.json `}).add({id:221,href:"/zh/docs/excel/keyedlist/#incell-enum-keyedlist",title:"KeyedList（键控列表） / Incell enum KeyedList ",description:"Excel keyed list 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Param []enum&lt;.FruitType&gt; Param list 1,FRUIT_TYPE_ORANGE,Banana Param 列的类型为 incell list []enum&lt;.FruitType&gt;，list 元素为预定义枚举类型 FruitType。
生成结果：
hello_world.proto ItemConf.json `}).add({id:222,href:"/zh/docs/excel/wellknown-types/#datetime日期时间",title:"Wellknown types（知名类型） / Datetime（日期时间） ",description:"Wellknown types 使用指南。",content:`
`}).add({id:223,href:"/zh/docs/excel/wellknown-types/#datetime日期时间-1",title:"Wellknown types（知名类型） / Datetime（日期时间） ",description:"Wellknown types 使用指南。",content:` 参考 Basics: Datetime →
HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:224,href:"/zh/docs/excel/wellknown-types/#date日期",title:"Wellknown types（知名类型） / Date（日期） ",description:"Wellknown types 使用指南。",content:` 参考 Basics: Datetime →
HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:225,href:"/zh/docs/excel/wellknown-types/#time时间",title:"Wellknown types（知名类型） / Time（时间） ",description:"Wellknown types 使用指南。",content:` 参考 Basics: Datetime →
HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:226,href:"/zh/docs/excel/wellknown-types/#duration时长",title:"Wellknown types（知名类型） / Duration（时长） ",description:"Wellknown types 使用指南。",content:` 参考 Basics: Duration →
HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:227,href:"/zh/docs/excel/wellknown-types/#fraction分数",title:"Wellknown types（知名类型） / Fraction（分数） ",description:"Wellknown types 使用指南。",content:` 参考 Basics: Fraction →
HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:228,href:"/zh/docs/excel/wellknown-types/#comparator比较器",title:"Wellknown types（知名类型） / Comparator（比较器） ",description:"Wellknown types 使用指南。",content:` 参考 Basics: Comparator →
HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:229,href:"/zh/docs/excel/wellknown-types/#version版本号",title:"Wellknown types（知名类型） / Version（版本号） ",description:"Wellknown types 使用指南。",content:` 参考 Basics: Version →
...
hello_world.proto ItemConf.json `}).add({id:230,href:"/zh/docs/excel/struct-in-struct/#struct-in-struct",title:"Struct 嵌套 Struct / Struct in struct ",description:"Excel struct in struct 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:231,href:"/zh/docs/excel/struct-in-struct/#predefined-struct-in-struct",title:"Struct 嵌套 Struct / Predefined-struct in struct ",description:"Excel struct in struct 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU RewardID RewardItemID RewardItemNum {Reward}int32 {.Item}int32 int32 Reward&rsquo;s ID Item&rsquo;s ID Item&rsquo;s num 1 1 10 生成结果：
hello_world.proto ItemConf.json `}).add({id:232,href:"/zh/docs/excel/struct-in-struct/#incell-struct-in-struct",title:"Struct 嵌套 Struct / Incell-struct in struct ",description:"Excel struct in struct 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:233,href:"/zh/docs/excel/struct-in-list/#纵向-list-中的嵌套",title:"List 嵌套 Struct / 纵向 list 中的嵌套 ",description:"Excel struct in list 使用指南。",content:`
`}).add({id:234,href:"/zh/docs/excel/struct-in-list/#纵向-list-中的-struct",title:"List 嵌套 Struct / 纵向 list 中的 struct ",description:"Excel struct in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:235,href:"/zh/docs/excel/struct-in-list/#纵向-list-中的-incell-struct",title:"List 嵌套 Struct / 纵向 list 中的 incell struct ",description:"Excel struct in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:236,href:"/zh/docs/excel/struct-in-list/#横向-list-的第一个字段",title:"List 嵌套 Struct / 横向 list 的第一个字段 ",description:"Excel struct in list 使用指南。",content:`
`}).add({id:237,href:"/zh/docs/excel/struct-in-list/#横向-list-中的-struct",title:"List 嵌套 Struct / 横向 list 中的 struct ",description:"Excel struct in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:238,href:"/zh/docs/excel/struct-in-list/#横向-list-中的-predefined-struct",title:"List 嵌套 Struct / 横向 list 中的 predefined struct ",description:"Excel struct in list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Reward1ItemID Reward1ItemNum Reward1Name Reward2ItemID Reward2ItemNum Reward2Name [Reward]{.Item}int32 int32 string int32 int32 string Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name 1 10 Lotto 10 100 Super Lotto 生成结果：
hello_world.proto ItemConf.json `}).add({id:239,href:"/zh/docs/excel/struct-in-list/#横向-list-中的-incell-struct",title:"List 嵌套 Struct / 横向 list 中的 incell struct ",description:"Excel struct in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
ItemConf.json `}).add({id:240,href:"/zh/docs/excel/struct-in-map/#纵向-map-中的嵌套",title:"Map 嵌套 Struct / 纵向 map 中的嵌套 ",description:"Excel struct in map 使用指南。",content:`
`}).add({id:241,href:"/zh/docs/excel/struct-in-map/#纵向-map-中的-struct",title:"Map 嵌套 Struct / 纵向 map 中的 struct ",description:"Excel struct in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:242,href:"/zh/docs/excel/struct-in-map/#纵向-map-中的-predefined-struct",title:"Map 嵌套 Struct / 纵向 map 中的 predefined struct ",description:"Excel struct in map 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID ItemID ItemNum map&lt;int32, Reward&gt; {.Item}int32 int32 Reward&rsquo;s ID Item&rsquo;s ID Item&rsquo;s Num 1 1 10 2 2 20 3 生成结果：
hello_world.proto ItemConf.json `}).add({id:243,href:"/zh/docs/excel/struct-in-map/#纵向-map-中的-incell-struct",title:"Map 嵌套 Struct / 纵向 map 中的 incell struct ",description:"Excel struct in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:244,href:"/zh/docs/excel/list-in-list/#纵向-list-中的嵌套",title:"List 嵌套 List / 纵向 list 中的嵌套 ",description:"Excel list in list 使用指南。",content:`
`}).add({id:245,href:"/zh/docs/excel/list-in-list/#纵向-list-中的横向-list",title:"List 嵌套 List / 纵向 list 中的横向 list ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:246,href:"/zh/docs/excel/list-in-list/#纵向-keyed-list-中的纵向-list",title:"List 嵌套 List / 纵向 keyed-list 中的纵向 list ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:247,href:"/zh/docs/excel/list-in-list/#纵向-keyed-list-中的纵向-keyed-list",title:"List 嵌套 List / 纵向 keyed-list 中的纵向 keyed-list ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:248,href:"/zh/docs/excel/list-in-list/#纵向-keyed-list-中的-incell-list",title:"List 嵌套 List / 纵向 keyed-list 中的 incell list ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:249,href:"/zh/docs/excel/list-in-list/#纵向-keyed-list-中的-incell-keyed-list",title:"List 嵌套 List / 纵向 keyed-list 中的 incell keyed-list ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:250,href:"/zh/docs/excel/list-in-list/#横向-list-中的嵌套",title:"List 嵌套 List / 横向 list 中的嵌套 ",description:"Excel list in list 使用指南。",content:`
`}).add({id:251,href:"/zh/docs/excel/list-in-list/#横向-list-中的横向-list",title:"List 嵌套 List / 横向 list 中的横向 list ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:252,href:"/zh/docs/excel/list-in-list/#横向-list-中的-predefined-struct-list",title:"List 嵌套 List / 横向 list 中的 predefined struct list ",description:"Excel list in list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Reward1Item1ID Reward1Item1Num Reward1Item2ID Reward1Item2Num Reward1Name Reward2Item1ID Reward2Item1Num Reward2Name [Reward][.Item]int32 int32 int32 int32 string int32 int32 string Item1&rsquo;s ID Item1&rsquo;s num Item2&rsquo;s ID Item2&rsquo;s num Reward&rsquo;s name Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name 1 10 2 20 Lotto 10 100 Super Lotto 生成结果：
hello_world.proto ItemConf.json `}).add({id:253,href:"/zh/docs/excel/list-in-list/#横向-list-中的-incell-list",title:"List 嵌套 List / 横向 list 中的 incell list ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:254,href:"/zh/docs/excel/list-in-map/#纵向-map-中的嵌套",title:"Map 嵌套 List / 纵向 map 中的嵌套 ",description:"Excel list in map 使用指南。",content:`
`}).add({id:255,href:"/zh/docs/excel/list-in-map/#纵向-map-中的横向-list",title:"Map 嵌套 List / 纵向 map 中的横向 list ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:256,href:"/zh/docs/excel/list-in-map/#纵向-map-中的纵向-list",title:"Map 嵌套 List / 纵向 map 中的纵向 list ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:257,href:"/zh/docs/excel/list-in-map/#纵向-map-中的-incell-list",title:"Map 嵌套 List / 纵向 map 中的 incell list ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:258,href:"/zh/docs/excel/list-in-map/#纵向-map-中的-incell-struct-list",title:"Map 嵌套 List / 纵向 map 中的 incell struct list ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:259,href:"/zh/docs/excel/list-in-map/#横向-map-中的嵌套",title:"Map 嵌套 List / 横向 map 中的嵌套 ",description:"Excel list in map 使用指南。",content:`
`}).add({id:260,href:"/zh/docs/excel/list-in-map/#横向-map-中的横向-list",title:"Map 嵌套 List / 横向 map 中的横向 list ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:261,href:"/zh/docs/excel/list-in-map/#横向-map-中的-incell-list",title:"Map 嵌套 List / 横向 map 中的 incell list ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:262,href:"/zh/docs/excel/map-in-list/#纵向-list-中的嵌套",title:"List 嵌套 Map / 纵向 list 中的嵌套 ",description:"Excel map in list 使用指南。",content:`
`}).add({id:263,href:"/zh/docs/excel/map-in-list/#纵向-list-中的横向-map",title:"List 嵌套 Map / 纵向 list 中的横向 map ",description:"Excel map in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:264,href:"/zh/docs/excel/map-in-list/#纵向-keyed-list-中的纵向-map",title:"List 嵌套 Map / 纵向 keyed-list 中的纵向 map ",description:"Excel map in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:265,href:"/zh/docs/excel/map-in-list/#纵向-list-中的-incell-map",title:"List 嵌套 Map / 纵向 list 中的 incell map ",description:"Excel map in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:266,href:"/zh/docs/excel/map-in-list/#横向-list-的第一个字段",title:"List 嵌套 Map / 横向 list 的第一个字段 ",description:"Excel map in list 使用指南。",content:`
`}).add({id:267,href:"/zh/docs/excel/map-in-list/#横向-list-中的横向-map",title:"List 嵌套 Map / 横向 list 中的横向 map ",description:"Excel map in list 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:268,href:"/zh/docs/excel/map-in-list/#横向-list-中的-predefined-struct-map",title:"List 嵌套 Map / 横向 list 中的 predefined struct map ",description:"Excel map in list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Reward1Item1ID Reward1Item1Num Reward1Item2ID Reward1Item2Num Reward1Name Reward2Item1ID Reward2Item1Num Reward2Name [Reward]map&lt;int32, .Item&gt; int32 int32 int32 string int32 int32 string Item1&rsquo;s ID Item1&rsquo;s num Item2&rsquo;s ID Item2&rsquo;s num Reward&rsquo;s name Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name 1 10 2 20 Lotto 10 100 Super Lotto 生成结果：
hello_world.proto ItemConf.json `}).add({id:269,href:"/zh/docs/excel/map-in-map/#纵向-map-中的嵌套",title:"Map 嵌套 Map / 纵向 map 中的嵌套 ",description:"Excel map in map 使用指南。",content:`
`}).add({id:270,href:"/zh/docs/excel/map-in-map/#纵向-map-中的横向-map",title:"Map 嵌套 Map / 纵向 map 中的横向 map ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:271,href:"/zh/docs/excel/map-in-map/#纵向-map-中的纵向-map",title:"Map 嵌套 Map / 纵向 map 中的纵向 map ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:272,href:"/zh/docs/excel/map-in-map/#纵向-map-中的-incell-map",title:"Map 嵌套 Map / 纵向 map 中的 incell map ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:273,href:"/zh/docs/excel/map-in-map/#横向-map-中的嵌套",title:"Map 嵌套 Map / 横向 map 中的嵌套 ",description:"Excel map in map 使用指南。",content:`
`}).add({id:274,href:"/zh/docs/excel/map-in-map/#横向-map-中的横向-map",title:"Map 嵌套 Map / 横向 map 中的横向 map ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:275,href:"/zh/docs/excel/map-in-map/#横向-map-中的-incell-map",title:"Map 嵌套 Map / 横向 map 中的 incell map ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的 worksheet ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:276,href:"/zh/docs/excel/infinite-nesting/#概述",title:"无限嵌套 / 概述 ",description:"Excel 无限嵌套使用指南。",content:` 现在，横向/纵向 list 元素的第一个字段可以是任意类型，包括 struct、list 和 map。
...
`}).add({id:277,href:"/zh/docs/excel/infinite-nesting/#嵌套命名",title:"无限嵌套 / 嵌套命名 ",description:"Excel 无限嵌套使用指南。",content:` common.proto 中的预定义类型：
...
hello_world.proto loader_conf.json `}).add({id:278,href:"/zh/docs/excel/field-property/#概览",title:"Field property（字段属性） / 概览 ",description:"Tableau field property 使用指南。",content:` 选项 类型 说明 unique bool 检查字段唯一性。
...
- PATCH_REPLACE - PATCH_MERGE sep string 字段级分隔符。 subsep string 字段级子分隔符。 cross int32 指定具有基数的复合类型（如 list 和 map）跨越的节点/单元格/字段数量。 pattern string 指定 scalar、list 元素和 map value 的模式。 `}).add({id:279,href:"/zh/docs/excel/field-property/#选项-unique",title:"Field property（字段属性） / 选项 unique ",description:"Tableau field property 使用指南。",content:` 选项 unique 可在 field property 中指定为 true 或 false，用于检查 list/map 元素中任意 scalar 字段的唯一性。
如果显式设置 unique 为 true，当出现重复 key 时 tableau 会报错。 如果显式设置 unique 为 false，则不进行检查。 `}).add({id:280,href:"/zh/docs/excel/field-property/#map或-keyedlist的-key",title:"Field property（字段属性） / Map（或 KeyedList）的 key ",description:"Tableau field property 使用指南。",content:` Tableau 会自动推断 map（或 KeyedList）key 的 unique 是否为 true。
规则是：如果 map 的 value 类型（或 KeyedList 元素类型）没有相同布局（纵向/横向）的子 map/list 字段，则 key 必须唯一。
因此大多数情况下不需要显式配置。
`}).add({id:281,href:"/zh/docs/excel/field-property/#通用-scalar-字段",title:"Field property（字段属性） / 通用 scalar 字段 ",description:"Tableau field property 使用指南。",content:` 如果将通用 scalar 字段的属性 unique 指定为 true，则 tableau 会检查该字段在 map 或 list 中的唯一性。
`}).add({id:282,href:"/zh/docs/excel/field-property/#选项-range",title:"Field property（字段属性） / 选项 range ",description:"Tableau field property 使用指南。",content:" ⚠️️ 如果单元格数据为空（未填写），此检查选项将不会生效。如果希望在单元格为空时也进行检查，请将选项 `present` 设置为 true。 选项 range 的格式为：&quot;left,right&quot;（左右均为闭区间）。\nrange 的不同含义：\n数字：值范围，例如：&quot;1,10&quot;、&quot;1,~&quot;、&quot;~,10&quot;。 字符串：UTF-8 码点数量。 list：list 的长度。 map：map 的长度。 "}).add({id:283,href:"/zh/docs/excel/field-property/#选项-refer",title:"Field property（字段属性） / 选项 refer ",description:"Tableau field property 使用指南。",content:` 选项 refer 类似于 SQL 中的外键（FOREIGN KEY）约束，用于防止破坏表之间关联的操作。但 tableau 的 refer 可以引用任意 sheet 的列（不限于 map key 列），并且支持多个 refer（逗号分隔）。它用于确保该字段的值至少在其他某个 sheet 的列值空间（即 message 的字段值空间）中存在。
格式：&quot;SheetName(SheetAlias).ColumnName[,SheetName(SheetAlias).ColumnName]...&quot;。
示例：
map&lt;uint32, Reward&gt;|{refer:&quot;ItemConf.ID&quot;}：无别名的单 refer，sheet 名即为生成的 protobuf message 名。 map&lt;uint32, Reward&gt;|{refer:&quot;ItemConf.ID,EquipConf.ID&quot;}：无别名的多 refer，sheet 别名即为生成的 protobuf message 名。 map&lt;uint32, Reward&gt;|{refer:&quot;Sheet1(ItemConf).ID&quot;}：有别名的单 refer，sheet 别名即为生成的 protobuf message 名。 `}).add({id:284,href:"/zh/docs/excel/field-property/#选项-sequence",title:"Field property（字段属性） / 选项 sequence ",description:"Tableau field property 使用指南。",content:` 选项 sequence 用于确保该字段的值是一个从指定值开始的序列，可用于任意字段，包括嵌套 list/map 中的字段。
示例：
map&lt;uint32, Item&gt;|{sequence:1}：该 map key 必须遵循从值 1 开始的序列规则。 int32|{sequence:1}：父级 list/map 的元素必须遵循从值 1 开始的序列规则。 `}).add({id:285,href:"/zh/docs/excel/field-property/#选项-default",title:"Field property（字段属性） / 选项 default ",description:"Tableau field property 使用指南。",content:` 如果设置了选项 default，则当单元格为空时使用该值作为默认值。
`}).add({id:286,href:"/zh/docs/excel/field-property/#选项-fixed",title:"Field property（字段属性） / 选项 fixed ",description:"Tableau field property 使用指南。",content:` 如果将选项 fixed 设置为 true，则自动检测横向 list/map 的固定大小。
示例：
List: 隐式固定大小 → Map: 隐式固定大小 → `}).add({id:287,href:"/zh/docs/excel/field-property/#选项-size",title:"Field property（字段属性） / 选项 size ",description:"Tableau field property 使用指南。",content:` 选项 size 用于指定横向 list/map 的固定大小。
示例：
List: 显式固定大小 → Map: 显式固定大小 → `}).add({id:288,href:"/zh/docs/excel/field-property/#选项-form",title:"Field property（字段属性） / 选项 form ",description:"Tableau field property 使用指南。",content:` 选项 form 用于指定 incell struct 的单元格数据格式。
可指定两种格式：
FORM_TEXT：protobuf text format。 FORM_JSON：protobuf JSON format。 详细示例请参考 高级 predefined incell struct →。
`}).add({id:289,href:"/zh/docs/excel/field-property/#选项-json_name",title:"Field property（字段属性） / 选项 json_name ",description:"Tableau field property 使用指南。",content:` 默认情况下，JSON 名称由字段的 proto 名称转换为 camelCase 得到。现在可以通过 json_name prop 选项显式指定。
例如，HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Rarity_1 SpecialEffect_2 map&lt;int32, Item&gt; int32|{json_name:&ldquo;rarity_1&rdquo;} int32|{json_name:&ldquo;specialEffect_2&rdquo;} Item&rsquo;s ID Item&rsquo;s rarity. Item&rsquo;s special effect. 1 10 101 2 20 102 3 30 103 `}).add({id:290,href:"/zh/docs/excel/field-property/#选项-present",title:"Field property（字段属性） / 选项 present ",description:"Tableau field property 使用指南。",content:` 如果将选项 present 设置为 true，则单元格数据不能为空，必须显式填写，否则会报错。
`}).add({id:291,href:"/zh/docs/excel/field-property/#选项-optional",title:"Field property（字段属性） / 选项 optional ",description:"Tableau field property 使用指南。",content:` 指定该字段是否为可选（字段名可缺失）。
如果设置为 true，则：
表格格式（Excel/CSV）：字段的列可以缺失。 文档格式（XML/YAML）：字段名可以缺失。 `}).add({id:292,href:"/zh/docs/excel/field-property/#选项-patch",title:"Field property（字段属性） / 选项 patch ",description:"Tableau field property 使用指南。",content:` 参见 选项 Patch → 中的字段级 patch。
`}).add({id:293,href:"/zh/docs/excel/field-property/#选项-sep",title:"Field property（字段属性） / 选项 sep ",description:"Tableau field property 使用指南。",content:` 字段级分隔符，用于分隔：
incell list 元素（scalar 或 struct）。 incell map 元素 如果未设置，将使用 metasheet 中的sheet 级 sep。
`}).add({id:294,href:"/zh/docs/excel/field-property/#选项-subsep",title:"Field property（字段属性） / 选项 subsep ",description:"Tableau field property 使用指南。",content:` 字段级子分隔符，用于分隔：
每个 incell map 元素的键值对。 每个 incell struct list 元素的结构体字段。 如果未设置，将使用 metasheet 中的sheet 级 subsep。
`}).add({id:295,href:"/zh/docs/excel/field-property/#选项-cross",title:"Field property（字段属性） / 选项 cross ",description:"Tableau field property 使用指南。",content:` 指定具有基数的复合类型（如 list 和 map）跨越的节点/单元格/字段数量。
`}).add({id:296,href:"/zh/docs/excel/field-property/#union-list-字段",title:"Field property（字段属性） / union list 字段 ",description:"Tableau field property 使用指南。",content:` TODO: 示例待补充。
指定 list 将跨越并占用的 union 字段数量（每个字段对应一个 list 元素）。这也会将该 list 字段的布局从 incell 改为横向。
值为 0 表示是 incell list。 值 &gt; 0 表示是占用 N 个字段的横向 list。 值 &lt; 0 表示是占用所有后续字段的横向 list。 `}).add({id:297,href:"/zh/docs/excel/field-property/#选项-pattern",title:"Field property（字段属性） / 选项 pattern ",description:"Tableau field property 使用指南。",content:` 指定 scalar 字段、list 元素和 map value 的模式。
`}).add({id:298,href:"/zh/docs/excel/field-property/#wellknown-version-字段",title:"Field property（字段属性） / Wellknown version 字段 ",description:"Tableau field property 使用指南。",content:` 使用案例请参考 Wellknown types: Version →
指定当前单元格的点分十进制模式。每个十进制数的范围从 0 到模式对应部分的最大值（MAX）。
默认模式：255.255.255。
`}).add({id:299,href:"/zh/docs/excel/metasheet/#概述",title:"Metasheet（元表） / 概述 ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 以下选项可在 metasheet @TABLEAU 中指定，用于影响对应 worksheet 的布局、功能、loader 等。
...
示例：
- OrderedMap:cpp,Index:cpp go // ordered map 仅支持 cpp，index 支持 cpp 和 go - OrderedMap:cpp // ordered map 仅支持 cpp，index 支持所有语言 `}).add({id:300,href:"/zh/docs/excel/metasheet/#空-tableau",title:"Metasheet（元表） / 空 @TABLEAU ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 如果 metasheet @TABLEAU 为空，则同一 workbook 中的所有其他 worksheet 都会被处理。
`}).add({id:301,href:"/zh/docs/excel/metasheet/#简单示例",title:"Metasheet（元表） / 简单示例 ",description:"Excel metasheet @TABLEAU 使用指南。",content:` HelloWorld.xlsx 中有一个 worksheet Sheet1，我们希望将其重命名为 ItemConf，定义自定义分隔符为 |，并生成 ordered map 访问器。
因此，HelloWorld.xlsx 中的 metasheet @TABLEAU 应配置如下：
HelloWorld.xlsx&nbsp; Sheet1 @TABLEAU ID Name map&lt;uint32, Item&gt; string Item&rsquo;s ID Item&rsquo;s Name 1 Apple 2 Orange 3 Banana Sheet Alias Sep OrderedMap Sheet1 ItemConf | true `}).add({id:302,href:"/zh/docs/excel/metasheet/#workbook-alias",title:"Metasheet（元表） / Workbook Alias ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 生成的 proto 文件名是输入文件名的 snake_case 形式。例如，如果 workbook 名为 HelloWorld.xlsx，则生成的 proto 文件名为 hello_world.proto。如果希望手动指定生成的 proto 文件名，可以使用 Alias 选项。在此场景中，# 表示 workbook 名称。
HelloWorld.xlsx 中的 worksheet ItemConf：
...
custom_conf.proto `}).add({id:303,href:"/zh/docs/excel/metasheet/#选项-mode",title:"Metasheet（元表） / 选项 Mode ",description:"Excel metasheet @TABLEAU 使用指南。",content:` Sheet mode 定义了 tableauc（protogen）解析 sheet 的方式：数据或类型。
可用模式：
MODE_DEFAULT：默认模式，定义 sheet 的数据结构。 MODE_ENUM_TYPE：在一个 sheet 中定义单个枚举类型，参见 示例。 MODE_ENUM_TYPE_MULTI：在一个 sheet 中定义多个枚举类型，参见 示例。 MODE_STRUCT_TYPE：在一个 sheet 中定义单个 struct 类型，参见 示例。 MODE_STRUCT_TYPE_MULTI：在一个 sheet 中定义多个 struct 类型，参见 示例。 MODE_UNION_TYPE：在一个 sheet 中定义单个 union 类型，参见 示例。 MODE_UNION_TYPE_MULTI：在一个 sheet 中定义多个 union 类型，参见 示例。 `}).add({id:304,href:"/zh/docs/excel/metasheet/#选项-transpose",title:"Metasheet（元表） / 选项 Transpose ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 在线性代数中，矩阵的转置是将矩阵沿对角线翻转的操作。类似地，sheet（二维矩阵）的转置意味着将行与列互换。
...
Scatter1_ZoneConf.json Scatter2_ZoneConf.json Scatter3_ZoneConf.json `}).add({id:309,href:"/zh/docs/excel/metasheet/#选项-orderedmap",title:"Metasheet（元表） / 选项 OrderedMap ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 📢 仅适用于每个层级 message 的第一个 map 字段。
如果将 OrderedMap 设置为 true，则 tableau loader 插件将生成 ordered map API：
C++: OrderedMap API Go: OrderedMap API `}).add({id:310,href:"/zh/docs/excel/metasheet/#选项-index",title:"Metasheet（元表） / 选项 Index ",description:"Excel metasheet @TABLEAU 使用指南。",content:` Index 选项可用于生成 index 访问器。 有两种 index：
...
示例：
(ID,Name)：未设置 index 名，由父 struct 类型名决定。 (ID,Name)@AwardItem (ID,Name)&lt;ID&gt;：结果数组按 ID 排序。 (ID,Type)&lt;Type,Priority&gt;@Item：结果数组按 Type 和 Priority 排序。 ID@Item, (ID,Name)@AwardItem：一个单列 index 和一个多列 index。 `}).add({id:313,href:"/zh/docs/excel/metasheet/#选项-orderedindex",title:"Metasheet（元表） / 选项 OrderedIndex ",description:"Excel metasheet @TABLEAU 使用指南。",content:` OrderedIndex 选项可用于生成 ordered index 访问器。 有两种 ordered index：
单列 OrderedIndex 多列 OrderedIndex（又称 Composite OrderedIndex） 如果正确设置了 OrderedIndex，则 tableau loader 插件将生成 index API：
C++: Index API Go: Index API 每列类型可以是：
scalar：数值、布尔值、字符串和 bytes。 enum：例如：enum&lt;.FruitType&gt; incell scalar list：例如：[]int32 incell enum list：例如：[]enum&lt;.FruitType&gt; 示例：HelloWorld.xlsx 中的两个 worksheet ItemConf 和 ShopConf：
ItemConf：对 map value 同一 struct 的列建立 ordered index。 ShopConf：对 list element 同一 struct 的列建立 ordered index。 HelloWorld.xlsx&nbsp; ItemConf ShopConf @TABLEAU ID Name Desc map&lt;int32, Item&gt; string string Item&rsquo;s ID Item&rsquo;s name Item&rsquo;s desc 1 Apple A kind of delicious fruit. 2 Orange A kind of sour fruit. 3 Banana A kind of calorie-rich fruit. ID Type Desc [Shop]int32 int32 string Shop&rsquo;s ID Shop&rsquo;s type Shop&rsquo;s desc 1 1 Shoes shop. 2 1 T-Shirt shop. 3 2 Fruite shop. Sheet OrderedIndex ItemConf ID@Item, Name@AwardItem, (ID,Name)@SpecialItem ShopConf ID@Shop, Type@ThemeShop, (ID,Type)@SpecialShop `}).add({id:314,href:"/zh/docs/excel/metasheet/#单列-orderedindex",title:"Metasheet（元表） / 单列 OrderedIndex ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 格式：Column&lt;ColumnX,ColumnY,...&gt;@IndexName。
@ 是列名和 index 名之间的分隔符。如果未设置 IndexName，则使用该列的父 struct 类型名。可以用逗号分隔指定一个或多个 index。尖括号 &lt;&gt; 中的列指定排序列，结果数组按相同 index key 排序。
示例：
ID ID@Item ID&lt;ID&gt;@Item：结果数组按 ID 排序。 ID&lt;Type,Priority&gt;@Item：结果数组按 Type 和 Priority 排序。 ID, Name@AwardItem ID@Item, Name@AwardItem `}).add({id:315,href:"/zh/docs/excel/metasheet/#多列-orderedindex",title:"Metasheet（元表） / 多列 OrderedIndex ",description:"Excel metasheet @TABLEAU 使用指南。",content:` ⚠️ 暂不支持。
`}).add({id:316,href:"/zh/docs/excel/metasheet/#选项-patch",title:"Metasheet（元表） / 选项 Patch ",description:"Excel metasheet @TABLEAU 使用指南。",content:`

`}).add({id:317,href:"/zh/docs/excel/metasheet/#选项-sep",title:"Metasheet（元表） / 选项 Sep ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 工作表的分隔符，用于分隔：
incell list 元素（scalar 或 struct）。 incell map 元素。 如果未设置，将使用 Tableauc 配置 中的全局级别分隔符（默认：,）。
`}).add({id:318,href:"/zh/docs/excel/metasheet/#选项-subsep",title:"Metasheet（元表） / 选项 Subsep ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 工作表的子分隔符，用于分隔：
每个 incell map 元素的键值对。 每个 incell struct list 元素的结构体字段。 如果未设置，将使用 Tableauc 配置 中的全局级别子分隔符（默认：:）。
`}).add({id:319,href:"/zh/docs/basics/",title:"基础",description:"Tableau 基础知识。",content:""}).add({id:320,href:"/zh/docs/basics/concepts/#术语",title:"核心概念 / 术语 ",description:"Tableau 核心概念。",content:`
`}).add({id:321,href:"/zh/docs/basics/concepts/#基础术语",title:"核心概念 / 基础术语 ",description:"Tableau 核心概念。",content:" 术语 定义 Workbook 一个 Excel 文件；或一组以相同前缀命名、用 # 分隔的 CSV 文件；或一个 XML 文件；或一个 YAML 文件。 Worksheet Excel 文件中的一个 sheet；或一个 CSV 文件；或 XML 文件的根节点；或 YAML 文件中的一个文档。 Metasheet 名为 @TABLEAU 的特殊 worksheet，用于指定 tableau 解析器选项。 Row sheet 中的一行。 Column sheet 中的一列。 Cell 行与列的交叉点（单元格）。 In-cell 单元格内部。 Cross-cell 一行或一列中连续的多个单元格。 "}).add({id:322,href:"/zh/docs/basics/concepts/#worksheet-相关术语",title:"核心概念 / Worksheet 相关术语 ",description:"Tableau 核心概念。",content:` 术语 定义 Namerow worksheet 中列名定义所在的行号。
...
默认值：false。 Layout Incell（单元格内）、vertical（跨单元格纵向）或 horizontal（跨单元格横向）。 Transpose 对指定 sheet 进行行列转置。 `}).add({id:323,href:"/zh/docs/basics/concepts/#与-protoconf-的映射关系",title:"核心概念 / 与 Protoconf 的映射关系 ",description:"Tableau 核心概念。",content:" 术语 Protoconf Workbook 一个 protoconf（.proto）文件。 Worksheet protoconf 文件中的一个顶层 message（名为 @TABLEAU 的 metasheet 除外）。 column message 中的一个字段。 "}).add({id:324,href:"/zh/docs/basics/concepts/#简单映射示例",title:"核心概念 / 简单映射示例 ",description:"Tableau 核心概念。",content:`
`}).add({id:325,href:"/zh/docs/basics/concepts/#输入一个-excel-文件",title:"核心概念 / 输入：一个 Excel 文件 ",description:"Tableau 核心概念。",content:` 一个工作簿（HelloWorld.xlsx），包含两个数据 worksheet（ItemConf 和 ActivityConf）以及一个空的 tableau metasheet（@TABLEAU）。
HelloWorld.xlsx&nbsp; ItemConf ActivityConf @TABLEAU ID Name Type map&lt;uint32, Item&gt; string int32 Item&rsquo;s ID. Item&rsquo;s name. Item&rsquo;s type. 1 item1 100 2 item2 200 3 item3 300 ID Name Open map&lt;uint32, Activity&gt; string bool Activity&rsquo;s ID. Activity&rsquo;s name. Activity is open? 1 activity1 true 2 activity2 false 3 activity3 `}).add({id:326,href:"/zh/docs/basics/concepts/#输出一个-protoconf-文件",title:"核心概念 / 输出：一个 protoconf 文件 ",description:"Tableau 核心概念。",content:` 一个 protoconf 文件（hello_world.proto），包含两个顶层 message（ItemConf 和 ActivityConf）。
hello_world.proto `}).add({id:327,href:"/zh/docs/basics/naming-convention/#枚举enums",title:"命名规范 / 枚举（Enums） ",description:"命名规范。",content:` 枚举类型名使用 PascalCase（首字母大写），枚举值名使用 CAPITALS_WITH_UNDERSCORES（全大写加下划线）：
参考 Protobuf 风格：枚举。
`}).add({id:328,href:"/zh/docs/basics/naming-convention/#示例",title:"命名规范 / 示例 ",description:"命名规范。",content:" 名称 风格 示例 workbook PascalCase HelloWorld.xlsx worksheet PascalCase HelloWorld struct (message) PascalCase HelloWorld field (column) PascalCase HelloWorld "}).add({id:329,href:"/zh/docs/basics/grammar-and-types/#概述",title:"语法与类型 / 概述 ",description:"语法与类型。",content:` Tableau 的大部分语法和类型借鉴自 Protocol Buffers (proto3) 和 Golang。
`}).add({id:330,href:"/zh/docs/basics/grammar-and-types/#scalar-types标量类型",title:"语法与类型 / Scalar types（标量类型） ",description:"语法与类型。",content:` 详细说明请参考 Protocol Buffers Proto3 Scalar。
类别 类型 默认值 数值 int32, uint32
int64, uint64
float, double 0
0
0.0 布尔值 bool false 字符串 string &quot;&quot; 字节 bytes &quot;&quot; `}).add({id:331,href:"/zh/docs/basics/grammar-and-types/#composite-types复合类型",title:"语法与类型 / Composite types（复合类型） ",description:"语法与类型。",content:" 类型 说明 struct struct 映射为 protobuf 的 message。 list list 映射为 protobuf 的 repeated 字段。 map map 映射为 protobuf 的 map 字段。 "}).add({id:332,href:"/zh/docs/basics/grammar-and-types/#struct结构体",title:"语法与类型 / Struct（结构体） ",description:"语法与类型。",content:` 特性 说明 横向布局（Horizontal layout） 每个标量字段占一个单元格。 简单 in-cell struct 每个字段必须是标量类型。
以逗号分隔的字段列表，例如：1,test,3.0。
若数据列表长度与 struct 字段数不一致，则按顺序填充，未配置的字段使用标量类型的默认值。 `}).add({id:333,href:"/zh/docs/basics/grammar-and-types/#list列表",title:"语法与类型 / List（列表） ",description:"语法与类型。",content:` 特性 说明 横向布局（Horizontal layout） list 的默认布局。
元素类型可以是 struct 或标量。 纵向布局（Vertical layout） list 的元素类型应为 struct。 简单 in-cell list 元素类型必须是标量。
以逗号分隔的元素列表，例如：1,2,3。 可扩展（Scalable） 支持动态大小的 list。 忽略空元素 智能识别任意位置的空元素。 `}).add({id:334,href:"/zh/docs/basics/grammar-and-types/#map映射",title:"语法与类型 / Map（映射） ",description:"语法与类型。",content:` 特性 说明 横向布局（Horizontal layout） 纵向布局（Vertical layout） map 的默认布局。 Hash map 实现为无序 map 或 hash map。 Ordered map 由 tableauio/loader 支持。
- C++ 简单 in-cell map key 和 value 都必须是标量类型。
以逗号分隔的 key:value 对列表，例如：1:10,2:20,3:30。 可扩展（Scalable） 支持动态大小的 map。 忽略空条目 智能识别任意位置的空条目。 `}).add({id:335,href:"/zh/docs/basics/grammar-and-types/#enumeration枚举",title:"语法与类型 / Enumeration（枚举） ",description:"语法与类型。",content:` 特性 说明 三种枚举值形式 1. 枚举值编号（number）。
2. 枚举值名称（name）。
3. 枚举值别名（alias，通过 EnumValueOptions 指定）。 校验 自动检查枚举值的合法性。 `}).add({id:336,href:"/zh/docs/basics/grammar-and-types/#empty-value空值",title:"语法与类型 / Empty value（空值） ",description:"语法与类型。",content:` 类型 说明 scalar 空标量将使用该标量类型的默认值填充。 struct 若所有字段均为空，则不会生成该 struct。 list 若 list 大小为 0，则不会生成该 list。
若 list 的元素（struct 类型）为空，则不会追加该元素。 map 若 map 大小为 0，则不会生成该 map。若 map 的 value（struct 类型）为空，则不会插入该条目。 nesting 递归地判断是否为空。 `}).add({id:337,href:"/zh/docs/basics/enum/#枚举值",title:"Enum（枚举） / 枚举值 ",description:"枚举基础。",content:` tableau 解析器支持三种枚举值形式：
...
`}).add({id:338,href:"/zh/docs/basics/enum/#校验",title:"Enum（枚举） / 校验 ",description:"枚举基础。",content:` 由于枚举类型是预先定义的，tableau 解析器会自动校验枚举值的合法性。
`}).add({id:339,href:"/zh/docs/basics/wellknown-types/#概述",title:"Wellknown types（知名类型） / 概述 ",description:"Wellknown 类型。",content:` 为方便使用，Wellknown 类型是 Tableau 的内置类型，类似于 Protocol Buffers Well-Known Types。
使用时需要引入 Tableau 和 Protocol Buffers 提供的 proto 文件：
tableau/protobuf/wellknown.proto google/protobuf/timestamp.proto google/protobuf/duration.proto `}).add({id:340,href:"/zh/docs/basics/wellknown-types/#datetime日期时间",title:"Wellknown types（知名类型） / Datetime（日期时间） ",description:"Wellknown 类型。",content:` 使用示例请参考 Wellknown 类型：Datetime →
...
datetime 和 date 基于 google.protobuf.Timestamp，参考 JSON 映射。 time 基于 google.protobuf.Duration，参考 JSON 映射。 RFC 3339: 互联网日期和时间戳 `}).add({id:341,href:"/zh/docs/basics/wellknown-types/#duration时长",title:"Wellknown types（知名类型） / Duration（时长） ",description:"Wellknown 类型。",content:` 使用示例请参考 Wellknown 类型：Duration →
...
duration 基于 google.protobuf.Duration，参考 JSON 映射。 golang duration 字符串格式。 golang ParseDuration。 `}).add({id:342,href:"/zh/docs/basics/wellknown-types/#fraction分数",title:"Wellknown types（知名类型） / Fraction（分数） ",description:"Wellknown 类型。",content:` 使用示例请参考 Wellknown 类型：Fraction →
...
- N/D：简单分数，例如：3/4
- N：仅分子，例如：3 等同于 3/1
- N：浮点分子，例如：0.01 等同于 1/100 `}).add({id:343,href:"/zh/docs/basics/wellknown-types/#comparator比较器",title:"Wellknown types（知名类型） / Comparator（比较器） ",description:"Wellknown 类型。",content:` 使用示例请参考 Wellknown 类型：Comparator →
comparator 包含一个 sign（符号）和一个分数 value（值），任意数字或分数均可与之比较。
类型 默认值 说明 comparator ==0 格式：&lt;Sign&gt;&lt;Fraction&gt;。
例如：==10、!=1/2、&lt;10%、&lt;=10‰、&gt;10%、&gt;=10‱ `}).add({id:344,href:"/zh/docs/basics/wellknown-types/#version版本号",title:"Wellknown types（知名类型） / Version（版本号） ",description:"Wellknown 类型。",content:` 使用示例请参考 Wellknown 类型：Version →
...
例如：1.0.1.1 `}).add({id:345,href:"/zh/docs/basics/predefined-types/#概述",title:"Predefined types（预定义类型） / 概述 ",description:"预定义类型。",content:` 你可以在 protoconf 文件（如 common.proto）中预先定义 enum、struct 或 union 类型，然后在 worksheet 中将其用作列类型或跨单元格类型。
`}).add({id:346,href:"/zh/docs/basics/predefined-types/#使用方式",title:"Predefined types（预定义类型） / 使用方式 ",description:"预定义类型。",content:" 语法：在 worksheet 中使用预定义的 CustomType 时，需在前面加一个点 .（即 .CustomType）。 导入：在 tableauc 配置的 protoFiles 选项中指定公共 proto 文件，这些文件中定义了预定义的 enum、struct、union 类型。参考 Tableauc 配置。 "}).add({id:347,href:"/zh/docs/basics/predefined-types/#enum",title:"Predefined types（预定义类型） / Enum ",description:"预定义类型。",content:` 例如，common.proto 中定义的枚举类型 FruitType：
以下示例说明如何使用预定义枚举类型：
Excel/CSV：使用预定义枚举类型。 XML：使用预定义枚举类型 YAML：使用预定义枚举类型 `}).add({id:348,href:"/zh/docs/basics/predefined-types/#struct",title:"Predefined types（预定义类型） / Struct ",description:"预定义类型。",content:` 例如，common.proto 中定义的 struct 类型 Prop：
