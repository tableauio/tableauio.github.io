...
C++: OrderedMap API Go: OrderedMap API `}).add({id:305,href:"/zh/docs/excel/metasheet/#选项-index",title:"Metasheet（元表） / 选项 Index ",description:"Excel metasheet @TABLEAU 使用指南。",content:` Index 选项可用于生成 index 访问器。 有两种 index：
...
示例：
(ID,Name)：未设置 index 名，由父 struct 类型名决定。 (ID,Name)@AwardItem (ID,Name)&lt;ID&gt;：结果数组按 ID 排序。 (ID,Type)&lt;Type,Priority&gt;@Item：结果数组按 Type 和 Priority 排序。 ID@Item, (ID,Name)@AwardItem：一个单列 index 和一个多列 index。 `}).add({id:308,href:"/zh/docs/excel/metasheet/#选项-orderedindex",title:"Metasheet（元表） / 选项 OrderedIndex ",description:"Excel metasheet @TABLEAU 使用指南。",content:` OrderedIndex 选项可用于生成 ordered index 访问器。 有两种 ordered index：
