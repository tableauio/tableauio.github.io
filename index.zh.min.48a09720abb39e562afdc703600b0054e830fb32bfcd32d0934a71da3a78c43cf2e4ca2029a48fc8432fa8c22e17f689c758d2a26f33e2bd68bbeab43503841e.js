...
默认模式：255.255.255。
`}).add({id:292,href:"/zh/docs/excel/metasheet/#概述",title:"Metasheet（元表） / 概述 ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 以下选项可在 metasheet @TABLEAU 中指定，用于影响对应 worksheet 的布局、功能、loader 等。
...
示例：
- OrderedMap:cpp,Index:cpp go // ordered map 仅支持 cpp，index 支持 cpp 和 go - OrderedMap:cpp // ordered map 仅支持 cpp，index 支持所有语言 `}).add({id:293,href:"/zh/docs/excel/metasheet/#空-tableau",title:"Metasheet（元表） / 空 @TABLEAU ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 如果 metasheet @TABLEAU 为空，则同一 workbook 中的所有其他 worksheet 都会被处理。
`}).add({id:294,href:"/zh/docs/excel/metasheet/#简单示例",title:"Metasheet（元表） / 简单示例 ",description:"Excel metasheet @TABLEAU 使用指南。",content:` HelloWorld.xlsx 中有一个 worksheet Sheet1，我们希望将其重命名为 ItemConf，定义自定义分隔符为 |，并生成 ordered map 访问器。
因此，HelloWorld.xlsx 中的 metasheet @TABLEAU 应配置如下：
HelloWorld.xlsx&nbsp; Sheet1 @TABLEAU ID Name map&lt;uint32, Item&gt; string Item&rsquo;s ID Item&rsquo;s Name 1 Apple 2 Orange 3 Banana Sheet Alias Sep OrderedMap Sheet1 ItemConf | true `}).add({id:295,href:"/zh/docs/excel/metasheet/#workbook-alias",title:"Metasheet（元表） / Workbook Alias ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 生成的 proto 文件名是输入文件名的 snake_case 形式。例如，如果 workbook 名为 HelloWorld.xlsx，则生成的 proto 文件名为 hello_world.proto。如果希望手动指定生成的 proto 文件名，可以使用 Alias 选项。在此场景中，# 表示 workbook 名称。
HelloWorld.xlsx 中的 worksheet ItemConf：
...
custom_conf.proto `}).add({id:296,href:"/zh/docs/excel/metasheet/#选项-mode",title:"Metasheet（元表） / 选项 Mode ",description:"Excel metasheet @TABLEAU 使用指南。",content:` Sheet mode 定义了 tableauc（protogen）解析 sheet 的方式：数据或类型。
可用模式：
MODE_DEFAULT：默认模式，定义 sheet 的数据结构。 MODE_ENUM_TYPE：在一个 sheet 中定义单个枚举类型，参见 示例。 MODE_ENUM_TYPE_MULTI：在一个 sheet 中定义多个枚举类型，参见 示例。 MODE_STRUCT_TYPE：在一个 sheet 中定义单个 struct 类型，参见 示例。 MODE_STRUCT_TYPE_MULTI：在一个 sheet 中定义多个 struct 类型，参见 示例。 MODE_UNION_TYPE：在一个 sheet 中定义单个 union 类型，参见 示例。 MODE_UNION_TYPE_MULTI：在一个 sheet 中定义多个 union 类型，参见 示例。 `}).add({id:297,href:"/zh/docs/excel/metasheet/#选项-transpose",title:"Metasheet（元表） / 选项 Transpose ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 在线性代数中，矩阵的转置是将矩阵沿对角线翻转的操作。类似地，sheet（二维矩阵）的转置意味着将行与列互换。
...
hello_world.proto HeroConf.json `}).add({id:298,href:"/zh/docs/excel/metasheet/#选项-merger",title:"Metasheet（元表） / 选项 Merger ",description:"Excel metasheet @TABLEAU 使用指南。",content:` Merger 选项用于将多个具有相同 schema 的 sheet（逗号分隔）合并为一个。
每个元素可以是：
...
`}).add({id:299,href:"/zh/docs/excel/metasheet/#合并多个-workbook",title:"Metasheet（元表） / 合并多个 workbook ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 例如，有三个 workbook，每个都包含一个具有相同 schema 的 worksheet ZoneConf：
...
merger_main.proto ZoneConf.json `}).add({id:300,href:"/zh/docs/excel/metasheet/#合并同一-workbook-中的多个-sheet",title:"Metasheet（元表） / 合并同一 workbook 中的多个 sheet ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 例如，同一 workbook Merger.xlsx 中有三个具有相同 schema 的 worksheet：
...
生成结果：
merger_same.proto ZoneConf.json `}).add({id:301,href:"/zh/docs/excel/metasheet/#选项-scatter",title:"Metasheet（元表） / 选项 Scatter ",description:"Excel metasheet @TABLEAU 使用指南。",content:` Scatter 选项用于将多个具有相同 schema 的 sheet（逗号分隔）分别转换为不同的配置文件。
每个元素可以是：
仅 workbook 文件路径或 Glob 路径（相对于当前 workbook）：&lt;Workbook&gt;，此时 sheet 名称与当前 sheet 相同。 workbook 文件路径（相对于当前 workbook）加 worksheet 名称：&lt;Workbook&gt;#&lt;Worksheet&gt;。 [!NOTE] Glob 模式通常不应匹配主 workbook。如果匹配，tableauc 会自动排除它。
...
Scatter1_ZoneConf.json Scatter2_ZoneConf.json Scatter3_ZoneConf.json `}).add({id:302,href:"/zh/docs/excel/metasheet/#选项-orderedmap",title:"Metasheet（元表） / 选项 OrderedMap ",description:"Excel metasheet @TABLEAU 使用指南。",content:` [!IMPORTANT] 仅适用于每个层级 message 的第一个 map 字段。
...
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Name PropID PropValue map&lt;uint32, Item&gt; string map&lt;int32, Prop&gt; int64 Item&rsquo;s ID Item&rsquo;s name Prop&rsquo;s ID Prop&rsquo;s value 1 Apple 1 10 2 Orange 1 20 2 Orange 2 30 Sheet OrderedMap ItemConf true `}).add({id:303,href:"/zh/docs/excel/metasheet/#选项-index",title:"Metasheet（元表） / 选项 Index ",description:"Excel metasheet @TABLEAU 使用指南。",content:` [!IMPORTANT] 被索引的列必须是同一 struct（message）内的 scalar 或 enum。
...
ID&lt;Name&gt;@Item：对 ID 列建立单列 ordered index，结果数组按 Name 排序。 HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Name Desc map&lt;int32, Item&gt; string string Item&rsquo;s ID Item&rsquo;s name Item&rsquo;s desc 1 Apple A kind of delicious fruit. 2 Orange A kind of sour fruit. 3 Banana A kind of calorie-rich fruit. Sheet OrderedIndex ItemConf ID@Item `}).add({id:308,href:"/zh/docs/excel/metasheet/#多列-orderedindex",title:"Metasheet（元表） / 多列 OrderedIndex ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 格式：(Column1,Column2,...)&lt;ColumnX,ColumnY,...&gt;@IndexName。
...
(ID,Type)&lt;Type&gt;@Shop：对 (ID, Type) 列建立多列 ordered index，结果数组按 Type 排序。 (ID,Type)&lt;Type,ID&gt;@SpecialShop：对 (ID, Type) 列建立多列 ordered index，结果数组按 Type 然后 ID 排序。 HelloWorld.xlsx&nbsp; ShopConf @TABLEAU ID Type Desc [Shop]int32 int32 string Shop&rsquo;s ID Shop&rsquo;s type Shop&rsquo;s desc 1 1 Shoes shop. 2 1 T-Shirt shop. 3 2 Fruite shop. Sheet OrderedIndex ShopConf (ID,Type)@Shop, (ID,Type)&lt;Type,ID&gt;@SpecialShop `}).add({id:309,href:"/zh/docs/excel/metasheet/#选项-patch",title:"Metasheet（元表） / 选项 Patch ",description:"Excel metasheet @TABLEAU 使用指南。",content:`

`}).add({id:310,href:"/zh/docs/excel/metasheet/#选项-sep",title:"Metasheet（元表） / 选项 Sep ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 工作表的分隔符，用于分隔：
incell list 元素（scalar 或 struct）。 incell map 元素。 如果未设置，将使用 Tableauc 配置 中的全局级别分隔符（默认：,）。
`}).add({id:311,href:"/zh/docs/excel/metasheet/#选项-subsep",title:"Metasheet（元表） / 选项 Subsep ",description:"Excel metasheet @TABLEAU 使用指南。",content:` 工作表的子分隔符，用于分隔：
每个 incell map 元素的键值对。 每个 incell struct list 元素的结构体字段。 如果未设置，将使用 Tableauc 配置 中的全局级别子分隔符（默认：:）。
`}).add({id:312,href:"/zh/docs/basics/",title:"基础",description:"Tableau 基础知识。",content:""}).add({id:313,href:"/zh/docs/basics/concepts/#术语",title:"核心概念 / 术语 ",description:"Tableau 核心概念。",content:`
`}).add({id:314,href:"/zh/docs/basics/concepts/#基础术语",title:"核心概念 / 基础术语 ",description:"Tableau 核心概念。",content:" 术语 定义 Workbook 一个 Excel 文件；或一组以相同前缀命名、用 # 分隔的 CSV 文件；或一个 XML 文件；或一个 YAML 文件。 Worksheet Excel 文件中的一个 sheet；或一个 CSV 文件；或 XML 文件的根节点；或 YAML 文件中的一个文档。 Metasheet 名为 @TABLEAU 的特殊 worksheet，用于指定 tableau 解析器选项。 Row sheet 中的一行。 Column sheet 中的一列。 Cell 行与列的交叉点（单元格）。 In-cell 单元格内部。 Cross-cell 一行或一列中连续的多个单元格。 "}).add({id:315,href:"/zh/docs/basics/concepts/#worksheet-相关术语",title:"核心概念 / Worksheet 相关术语 ",description:"Tableau 核心概念。",content:` 术语 定义 Namerow worksheet 中列名定义所在的行号。
...
默认值：false。 Layout Incell（单元格内）、vertical（跨单元格垂直）或 horizontal（跨单元格水平）。 Transpose 对指定 sheet 进行行列转置。 `}).add({id:316,href:"/zh/docs/basics/concepts/#与-protoconf-的映射关系",title:"核心概念 / 与 Protoconf 的映射关系 ",description:"Tableau 核心概念。",content:" 术语 Protoconf Workbook 一个 protoconf（.proto）文件。 Worksheet protoconf 文件中的一个顶层 message（名为 @TABLEAU 的 metasheet 除外）。 column message 中的一个字段。 "}).add({id:317,href:"/zh/docs/basics/concepts/#简单映射示例",title:"核心概念 / 简单映射示例 ",description:"Tableau 核心概念。",content:`
`}).add({id:318,href:"/zh/docs/basics/concepts/#输入一个-excel-文件",title:"核心概念 / 输入：一个 Excel 文件 ",description:"Tableau 核心概念。",content:` 一个工作簿（HelloWorld.xlsx），包含两个数据 worksheet（ItemConf 和 ActivityConf）以及一个空的 tableau metasheet（@TABLEAU）。
HelloWorld.xlsx&nbsp; ItemConf ActivityConf @TABLEAU ID Name Type map&lt;uint32, Item&gt; string int32 Item&rsquo;s ID. Item&rsquo;s name. Item&rsquo;s type. 1 item1 100 2 item2 200 3 item3 300 ID Name Open map&lt;uint32, Activity&gt; string bool Activity&rsquo;s ID. Activity&rsquo;s name. Activity is open? 1 activity1 true 2 activity2 false 3 activity3 `}).add({id:319,href:"/zh/docs/basics/concepts/#输出一个-protoconf-文件",title:"核心概念 / 输出：一个 protoconf 文件 ",description:"Tableau 核心概念。",content:` 一个 protoconf 文件（hello_world.proto），包含两个顶层 message（ItemConf 和 ActivityConf）。
hello_world.proto `}).add({id:320,href:"/zh/docs/basics/naming-convention/#枚举enums",title:"命名规范 / 枚举（Enums） ",description:"命名规范。",content:` 枚举类型名使用 PascalCase（首字母大写），枚举值名使用 CAPITALS_WITH_UNDERSCORES（全大写加下划线）：
参考 Protobuf 风格：枚举。
`}).add({id:321,href:"/zh/docs/basics/naming-convention/#示例",title:"命名规范 / 示例 ",description:"命名规范。",content:" 名称 风格 示例 workbook PascalCase HelloWorld.xlsx worksheet PascalCase HelloWorld struct (message) PascalCase HelloWorld field (column) PascalCase HelloWorld "}).add({id:322,href:"/zh/docs/basics/grammar-and-types/#概述",title:"语法与类型 / 概述 ",description:"语法与类型。",content:` Tableau 的大部分语法和类型借鉴自 Protocol Buffers (proto3) 和 Golang。
`}).add({id:323,href:"/zh/docs/basics/grammar-and-types/#scalar-types标量类型",title:"语法与类型 / Scalar types（标量类型） ",description:"语法与类型。",content:` 详细说明请参考 Protocol Buffers Proto3 Scalar。
类别 类型 默认值 数值 int32, uint32
int64, uint64
float, double 0
0
0.0 布尔值 bool false 字符串 string &quot;&quot; 字节 bytes &quot;&quot; `}).add({id:324,href:"/zh/docs/basics/grammar-and-types/#composite-types复合类型",title:"语法与类型 / Composite types（复合类型） ",description:"语法与类型。",content:" 类型 说明 struct struct 映射为 protobuf 的 message。 list list 映射为 protobuf 的 repeated 字段。 map map 映射为 protobuf 的 map 字段。 "}).add({id:325,href:"/zh/docs/basics/grammar-and-types/#struct结构体",title:"语法与类型 / Struct（结构体） ",description:"语法与类型。",content:` 特性 说明 水平布局（Horizontal layout） 每个标量字段占一个单元格。 简单 in-cell struct 每个字段必须是标量类型。
以逗号分隔的字段列表，例如：1,test,3.0。
若数据列表长度与 struct 字段数不一致，则按顺序填充，未配置的字段使用标量类型的默认值。 `}).add({id:326,href:"/zh/docs/basics/grammar-and-types/#list列表",title:"语法与类型 / List（列表） ",description:"语法与类型。",content:` 特性 说明 水平布局（Horizontal layout） list 的默认布局。
元素类型可以是 struct 或标量。 垂直布局（Vertical layout） list 的元素类型应为 struct。 简单 in-cell list 元素类型必须是标量。
以逗号分隔的元素列表，例如：1,2,3。 可扩展（Scalable） 支持动态大小的 list。 忽略空元素 智能识别任意位置的空元素。 `}).add({id:327,href:"/zh/docs/basics/grammar-and-types/#map映射",title:"语法与类型 / Map（映射） ",description:"语法与类型。",content:` 特性 说明 水平布局（Horizontal layout） 垂直布局（Vertical layout） map 的默认布局。 Hash map 实现为无序 map 或 hash map。 Ordered map 由 tableauio/loader 支持。
- C++ 简单 in-cell map key 和 value 都必须是标量类型。
以逗号分隔的 key:value 对列表，例如：1:10,2:20,3:30。 可扩展（Scalable） 支持动态大小的 map。 忽略空条目 智能识别任意位置的空条目。 `}).add({id:328,href:"/zh/docs/basics/grammar-and-types/#enumeration枚举",title:"语法与类型 / Enumeration（枚举） ",description:"语法与类型。",content:` 特性 说明 三种枚举值形式 1. 枚举值编号（number）。
2. 枚举值名称（name）。
3. 枚举值别名（alias，通过 EnumValueOptions 指定）。 校验 自动检查枚举值的合法性。 `}).add({id:329,href:"/zh/docs/basics/grammar-and-types/#empty-value空值",title:"语法与类型 / Empty value（空值） ",description:"语法与类型。",content:` 类型 说明 scalar 空标量将使用该标量类型的默认值填充。 struct 若所有字段均为空，则不会生成该 struct。 list 若 list 大小为 0，则不会生成该 list。
若 list 的元素（struct 类型）为空，则不会追加该元素。 map 若 map 大小为 0，则不会生成该 map。若 map 的 value（struct 类型）为空，则不会插入该条目。 nesting 递归地判断是否为空。 `}).add({id:330,href:"/zh/docs/basics/enum/#枚举值",title:"Enum（枚举） / 枚举值 ",description:"枚举基础。",content:` tableau 解析器支持三种枚举值形式：
...
`}).add({id:331,href:"/zh/docs/basics/enum/#校验",title:"Enum（枚举） / 校验 ",description:"枚举基础。",content:` 由于枚举类型是预先定义的，tableau 解析器会自动校验枚举值的合法性。
`}).add({id:332,href:"/zh/docs/basics/wellknown-types/#概述",title:"Wellknown types（知名类型） / 概述 ",description:"Wellknown 类型。",content:` 为方便使用，Wellknown 类型是 Tableau 的内置类型，类似于 Protocol Buffers Well-Known Types。
使用时需要引入 Tableau 和 Protocol Buffers 提供的 proto 文件：
tableau/protobuf/wellknown.proto google/protobuf/timestamp.proto google/protobuf/duration.proto `}).add({id:333,href:"/zh/docs/basics/wellknown-types/#datetime日期时间",title:"Wellknown types（知名类型） / Datetime（日期时间） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Datetime 。
...
datetime 和 date 基于 google.protobuf.Timestamp，参考 JSON 映射。 time 基于 google.protobuf.Duration，参考 JSON 映射。 RFC 3339: 互联网日期和时间戳 `}).add({id:334,href:"/zh/docs/basics/wellknown-types/#duration时长",title:"Wellknown types（知名类型） / Duration（时长） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Duration 。
...
duration 基于 google.protobuf.Duration，参考 JSON 映射。 golang duration 字符串格式。 golang ParseDuration。 `}).add({id:335,href:"/zh/docs/basics/wellknown-types/#fraction分数",title:"Wellknown types（知名类型） / Fraction（分数） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Fraction 。
...
- N/D：简单分数，例如：3/4
- N：仅分子，例如：3 等同于 3/1
- N：浮点分子，例如：0.01 等同于 1/100 `}).add({id:336,href:"/zh/docs/basics/wellknown-types/#comparator比较器",title:"Wellknown types（知名类型） / Comparator（比较器） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Comparator 。
comparator 包含一个 sign（符号）和一个分数 value（值），任意数字或分数均可与之比较。
类型 默认值 说明 comparator ==0 格式：&lt;Sign&gt;&lt;Fraction&gt;。
例如：==10、!=1/2、&lt;10%、&lt;=10‰、&gt;10%、&gt;=10‱ `}).add({id:337,href:"/zh/docs/basics/wellknown-types/#version版本号",title:"Wellknown types（知名类型） / Version（版本号） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Version 。
...
例如：1.0.1.1 `}).add({id:338,href:"/zh/docs/basics/predefined-types/#概述",title:"Predefined types（预定义类型） / 概述 ",description:"预定义类型。",content:` 你可以在 protoconf 文件（如 common.proto）中预先定义 enum、struct 或 union 类型，然后在 worksheet 中将其用作列类型或跨单元格类型。
`}).add({id:339,href:"/zh/docs/basics/predefined-types/#使用方式",title:"Predefined types（预定义类型） / 使用方式 ",description:"预定义类型。",content:" 语法：在 worksheet 中使用预定义的 CustomType 时，需在前面加一个点 .（即 .CustomType）。 导入：在 tableauc 配置的 protoFiles 选项中指定公共 proto 文件，这些文件中定义了预定义的 enum、struct、union 类型。参考 Tableauc 配置。 "}).add({id:340,href:"/zh/docs/basics/predefined-types/#enum",title:"Predefined types（预定义类型） / Enum ",description:"预定义类型。",content:` 例如，common.proto 中定义的枚举类型 FruitType：
以下示例说明如何使用预定义枚举类型：
Excel/CSV：使用预定义枚举类型。 XML：使用预定义枚举类型 YAML：使用预定义枚举类型 `}).add({id:341,href:"/zh/docs/basics/predefined-types/#struct",title:"Predefined types（预定义类型） / Struct ",description:"预定义类型。",content:` 例如，common.proto 中定义的 struct 类型 Prop：
