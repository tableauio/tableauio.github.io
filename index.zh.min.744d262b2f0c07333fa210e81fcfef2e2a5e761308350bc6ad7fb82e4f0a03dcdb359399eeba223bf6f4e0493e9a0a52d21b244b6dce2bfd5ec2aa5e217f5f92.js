...
hello_world.proto loader_conf.json `}).add({id:281,href:"/zh/docs/excel/field-property/#概览",title:"字段属性（Field property） / 概览 ",description:"字段属性使用指南。",content:` 字段属性以管道符 | 紧接在字段类型之后，再加一个 {} 块，块内为 protobuf text format（文本格式）内容，由若干以空白分隔的 选项:值 对组成：
字符串值需要加引号（&quot;1,10&quot;）；数值与布尔值不需要（1、true）。注意 &quot;1,10&quot; 中引号内的逗号属于值本身，并非选项之间的分隔符。
例如：
int32|{range:&quot;1,10&quot;}：带值范围限制的标量字段。 map&lt;uint32, Item&gt;|{sequence:1}：键必须从 1 开始成序列的映射字段。 string|{json_name:&quot;rarity_1&quot;}：带自定义 JSON 名称的字符串字段。 int32|{range:&quot;1,10&quot; unique:true}：在单个字段上同时指定多个选项。 可用选项见下表。
选项 类型 说明 unique bool 检查字段唯一性。
...
默认值：false。 `}).add({id:282,href:"/zh/docs/excel/field-property/#选项-unique",title:"字段属性（Field property） / 选项 unique ",description:"字段属性使用指南。",content:` 选项 unique 可在字段属性中指定为 true 或 false，用于检查列表/映射元素中任意标量字段的唯一性。
如果显式设置 unique 为 true，当出现重复值时 tableau 会报错。 如果显式设置 unique 为 false，则不进行检查。 `}).add({id:283,href:"/zh/docs/excel/field-property/#映射键控列表的键",title:"字段属性（Field property） / 映射/键控列表的键 ",description:"字段属性使用指南。",content:` Tableau 会自动推断映射（或键控列表）键的 unique 是否为 true。
规则是：如果映射的值类型（或键控列表元素类型）没有相同布局（垂直/水平）的子映射/列表字段，则键必须唯一。
因此大多数情况下不需要显式配置。
`}).add({id:284,href:"/zh/docs/excel/field-property/#通用标量字段",title:"字段属性（Field property） / 通用标量字段 ",description:"字段属性使用指南。",content:` 如果将通用标量字段的属性 unique 指定为 true，则 tableau 会检查该字段在映射或列表中的唯一性。
`}).add({id:285,href:"/zh/docs/excel/field-property/#选项-range",title:"字段属性（Field property） / 选项 range ",description:"字段属性使用指南。",content:` [!WARNING] 当单元格数据为空（未填写）时，此检查将被跳过。若需对空单元格也强制执行检查，请将选项 present 设置为 true。
选项 range 的格式为：&quot;left,right&quot;（左右均为闭区间）。
range 的不同含义：
数字：值范围，例如：&quot;1,10&quot;、&quot;1,~&quot;、&quot;~,10&quot;。 `}).add({id:286,href:"/zh/docs/excel/field-property/#选项-refer",title:"字段属性（Field property） / 选项 refer ",description:"字段属性使用指南。",content:` 选项 refer 类似于 SQL 中的外键（FOREIGN KEY）约束，用于防止破坏表之间关联的操作。但 tableau 的 refer 可以引用任意工作表的列（不限于 map key 列），并且支持多个 refer（逗号分隔）。它用于确保该字段的值至少在其他某个工作表的列值空间（即 message 的字段值空间）中存在。
格式：&quot;SheetName(SheetAlias).ColumnName[,SheetName(SheetAlias).ColumnName]...&quot;。
示例：
map&lt;uint32, Reward&gt;|{refer:&quot;ItemConf.ID&quot;}：无别名的单 refer，工作表名即为生成的 protobuf message 名。 map&lt;uint32, Reward&gt;|{refer:&quot;ItemConf.ID,EquipConf.ID&quot;}：无别名的多 refer，工作表别名即为生成的 protobuf message 名。 map&lt;uint32, Reward&gt;|{refer:&quot;Sheet1(ItemConf).ID&quot;}：有别名的单 refer，工作表别名即为生成的 protobuf message 名。 `}).add({id:287,href:"/zh/docs/excel/field-property/#选项-sequence",title:"字段属性（Field property） / 选项 sequence ",description:"字段属性使用指南。",content:` 选项 sequence 用于确保该字段的值是一个从指定值开始的序列，可用于任意字段，包括嵌套列表/映射中的字段。
示例：
map&lt;uint32, Item&gt;|{sequence:1}：该映射键必须遵循从值 1 开始的序列规则。 int32|{sequence:1}：父级列表/映射的元素必须遵循从值 1 开始的序列规则。 `}).add({id:288,href:"/zh/docs/excel/field-property/#选项-default",title:"字段属性（Field property） / 选项 default ",description:"字段属性使用指南。",content:` 如果设置了选项 default，则当单元格为空时使用该值作为默认值。
`}).add({id:289,href:"/zh/docs/excel/field-property/#选项-fixed",title:"字段属性（Field property） / 选项 fixed ",description:"字段属性使用指南。",content:` 如果将选项 fixed 设置为 true，则自动检测水平列表/映射的固定大小。
示例：
列表: 隐式固定大小 映射: 隐式固定大小 `}).add({id:290,href:"/zh/docs/excel/field-property/#选项-size",title:"字段属性（Field property） / 选项 size ",description:"字段属性使用指南。",content:` 选项 size 用于指定水平列表/映射的固定大小。
示例：
列表: 显式固定大小 映射: 显式固定大小 `}).add({id:291,href:"/zh/docs/excel/field-property/#选项-form",title:"字段属性（Field property） / 选项 form ",description:"字段属性使用指南。",content:` 选项 form 用于指定 incell struct 的单元格数据格式。
可指定两种格式：
FORM_TEXT：protobuf text format。 FORM_JSON：protobuf JSON format。 详细示例请参考 高级预定义单元格内结构体。
`}).add({id:292,href:"/zh/docs/excel/field-property/#选项-json_name",title:"字段属性（Field property） / 选项 json_name ",description:"字段属性使用指南。",content:` 默认情况下，JSON 名称由字段的 proto 名称转换为 camelCase 得到。现在可以通过 json_name 选项显式指定。
例如，HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Rarity_1 SpecialEffect_2 map&lt;int32, Item&gt; int32|{json_name:&ldquo;rarity_1&rdquo;} int32|{json_name:&ldquo;specialEffect_2&rdquo;} Item&rsquo;s ID Item&rsquo;s rarity. Item&rsquo;s special effect. 1 10 101 2 20 102 3 30 103 `}).add({id:293,href:"/zh/docs/excel/field-property/#选项-present",title:"字段属性（Field property） / 选项 present ",description:"字段属性使用指南。",content:` 如果将选项 present 设置为 true，则单元格数据不能为空，必须显式填写，否则会报错。
`}).add({id:294,href:"/zh/docs/excel/field-property/#选项-optional",title:"字段属性（Field property） / 选项 optional ",description:"字段属性使用指南。",content:` 指定该字段是否为可选（字段名可缺失）。
如果设置为 true，则：
表格格式（Excel/CSV）：字段的列可以缺失。 文档格式（XML/YAML）：字段名可以缺失。 `}).add({id:295,href:"/zh/docs/excel/field-property/#选项-patch",title:"字段属性（Field property） / 选项 patch ",description:"字段属性使用指南。",content:` 参见 选项 Patch 中的字段级 patch。
`}).add({id:296,href:"/zh/docs/excel/field-property/#选项-sep",title:"字段属性（Field property） / 选项 sep ",description:"字段属性使用指南。",content:` 字段级分隔符，用于分隔：
单元格内映射的键值对。 单元格内列表的元素（标量或结构体）。 如果未设置，将使用 元表 中的工作表级 sep。
`}).add({id:297,href:"/zh/docs/excel/field-property/#选项-subsep",title:"字段属性（Field property） / 选项 subsep ",description:"字段属性使用指南。",content:` 字段级子分隔符，用于分隔：
每个单元格内映射的键和值。 每个单元格内结构体列表元素的结构体各字段。 如果未设置，将使用 元表 中的工作表级 subsep。
`}).add({id:298,href:"/zh/docs/excel/field-property/#选项-pattern",title:"字段属性（Field property） / 选项 pattern ",description:"字段属性使用指南。",content:` 指定标量字段、列表元素和映射值的模式。
`}).add({id:299,href:"/zh/docs/excel/field-property/#版本号字段",title:"字段属性（Field property） / 版本号字段 ",description:"字段属性使用指南。",content:` [!NOTE] 使用案例请参考 知名类型: 版本号
指定当前单元格的点分十进制模式。每个十进制数的范围从 0 到模式对应部分的最大值（MAX）。
默认模式：255.255.255。
`}).add({id:300,href:"/zh/docs/excel/field-property/#选项-order",title:"字段属性（Field property） / 选项 order ",description:"字段属性使用指南。",content:` 选项 order 用于确保字段的值沿解析方向（垂直布局按行、水平布局按列、单元格内列表/映射按元素顺序）满足指定的次序。 若后一个值相对于前一个值违反了所配置的次序，tableau 会报 E2026。
支持的次序：
...
`}).add({id:301,href:"/zh/docs/excel/field-property/#选项-validate",title:"字段属性（Field property） / 选项 validate ",description:"字段属性使用指南。",content:` Tableau 集成了 protovalidate， 可以直接在表格字段属性中声明校验规则。这些规则会被编译为生成 .proto 文件中 字段上的 (buf.validate.field) 选项，并在 tableau 生成配置时强制执行。支持 CEL 表达式以及 protovalidate 提供 的标准规则、predefined 规则和自定义（custom）规则。
...
hello_world.proto `}).add({id:302,href:"/zh/docs/excel/field-property/#选项-validate_complex",title:"字段属性（Field property） / 选项 validate_complex ",description:"字段属性使用指南。",content:` 选项 validate_complex 用于指定复合类型（即 列表/映射）的字段级校验 规则——即对容器本身（而非其元素）施加的规则。其值为 buf.validate.FieldRules （repeated 或 map 字段已设置）的 text format 表示。
示例：
...
`}).add({id:303,href:"/zh/docs/excel/field-property/#选项-validate_message",title:"字段属性（Field property） / 选项 validate_message ",description:"字段属性使用指南。",content:` 选项 validate_message 用于指定字段所嵌套 message 的 message 级校验规则。 通常在字段的值类型为子 message（例如结构体、列表元素或映射值的类型）时使用， 用于通过 CEL 表达式对 message 的多个字段进行交叉校验。其值为 buf.validate.MessageRules 的 text format 表示。
示例：
...
`}).add({id:304,href:"/zh/docs/excel/field-property/#选项-aggregate",title:"字段属性（Field property） / 选项 aggregate ",description:"字段属性使用指南。",content:` 选项 aggregate 作用于单元格内（layout:LAYOUT_INCELL）或水平 （layout:LAYOUT_HORIZONTAL）的列表/映射字段。当同一父记录被多行/多列重复 访问时（同一映射键跨行、同一垂直/水平列表跨多行/多列等），aggregate:true 会把每次访问产生的元素聚合到父记录的同一个集合中，而不要求所有访问填写 完全相同的值。
...
`}).add({id:305,href:"/zh/docs/excel/field-property/#适用场景",title:"字段属性（Field property） / 适用场景 ",description:"字段属性使用指南。",content:" 父级布局 aggregate 作用对象 重复访问场景 垂直映射/垂直键控列表（多行同键） 单元格内列表/映射（LAYOUT_INCELL）、水平列表/映射（LAYOUT_HORIZONTAL） 多行落到同一父记录 水平映射（多列同键） 单元格内列表/映射、水平列表/映射 多列落到同一父记录 垂直/水平列表自身跨多行/多列 兄弟单元格内字段在每行/列被重复访问 多行/多列落到同一父记录 "}).add({id:306,href:"/zh/docs/excel/field-property/#各类型聚合行为",title:"字段属性（Field property） / 各类型聚合行为 ",description:"字段属性使用指南。",content:" 子字段类型 aggregate:true 行为 错误码 单元格内列表 各次访问的元素追加到同一列表 — 单元格内映射 各次访问的键值合并到同一映射；键重复报错 E2005 单元格内键控列表（[]&lt;T&gt;） 各次访问的元素追加并按键去重；元素重复报错 E2028 水平列表 跨行的水平列表元素追加到同一列表 — 水平键控列表 跨行的元素追加并按键去重 E2028 水平映射 跨行的键值合并到同一映射；键重复报错 E2005 "}).add({id:307,href:"/zh/docs/excel/field-property/#垂直聚合",title:"字段属性（Field property） / 垂直聚合 ",description:"字段属性使用指南。",content:` 端到端示例参见 垂直列表（Vertical list） 中的对应小节：
垂直标量列表 —— 跨行聚合的 incell 标量列表。 垂直枚举列表 —— 跨行聚合的 incell 枚举列表。 垂直单元格内结构体列表 —— 跨行聚合的 incell 内联结构体列表。 垂直单元格内预定义结构体列表 —— 跨行聚合的 incell 预定义结构体列表。 `}).add({id:308,href:"/zh/docs/excel/field-property/#水平聚合",title:"字段属性（Field property） / 水平聚合 ",description:"字段属性使用指南。",content:` 端到端示例参见映射嵌套列表/映射嵌套映射中的水平聚合用例：
垂直映射嵌套水平聚合列表 —— 跨行聚合的水平 list（LAYOUT_HORIZONTAL）。 垂直映射嵌套水平聚合映射 —— 跨行聚合的水平 map（LAYOUT_HORIZONTAL）。 `}).add({id:309,href:"/zh/docs/excel/field-property/#aggregate-与-key-组合keyed-list-去重",title:"字段属性（Field property） / aggregate 与 key 组合（keyed-list 去重） ",description:"字段属性使用指南。",content:` 当 aggregate:true 与键控列表（[]&lt;T&gt; 语法，会自动设置字段的 key）一同使用时， tableau 在聚合过程中还会对元素去重：
标量/枚举键控列表：元素本身就是键——跨行/跨列重复的元素会触发 E2028。 结构体键控列表：仅比较通过 key 配置的子字段。 端到端示例参见 垂直键控列表 （垂直聚合）以及 垂直映射嵌套水平聚合列表（水平聚合）。
`}).add({id:310,href:"/zh/docs/excel/field-property/#注意",title:"字段属性（Field property） / 注意 ",description:"字段属性使用指南。",content:" aggregate 仅在单元格内（layout:LAYOUT_INCELL）或水平 （layout:LAYOUT_HORIZONTAL）的列表/映射字段上生效。 父字段键跨行重复时，需要在父字段上同时设置 prop:{unique:false}， 否则会被 E2005 拦截。 聚合后，range、refer、unique 等针对单元素的字段属性仍会逐元素生效。 "}).add({id:311,href:"/zh/docs/excel/metasheet/#概述",title:"元表（Metasheet） / 概述 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 以下选项可在元表 @TABLEAU 中指定，用于影响对应工作表的布局、功能、loader 等。
