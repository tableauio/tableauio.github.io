...
默认模式：255.255.255。
`}).add({id:298,href:"/zh/docs/excel/field-property/#选项-order",title:"字段属性（Field property） / 选项 order ",description:"Tableau field property 使用指南。",content:` 选项 order 用于确保字段的值沿解析方向（垂直布局按行、水平布局按列、单元格内列表/映射按元素顺序）满足指定的次序。 若后一个值相对于前一个值违反了所配置的次序，tableau 会报 E2026。
支持的次序：
...
`}).add({id:299,href:"/zh/docs/excel/field-property/#选项-validate",title:"字段属性（Field property） / 选项 validate ",description:"Tableau field property 使用指南。",content:` Tableau 集成了 protovalidate， 可以直接在表格字段属性中声明校验规则。这些规则会被编译为生成 .proto 文件中 字段上的 (buf.validate.field) 选项，并在 tableau 生成配置时强制执行。支持 CEL 表达式以及 protovalidate 提供 的标准规则、predefined 规则和自定义（custom）规则。
...
hello_world.proto `}).add({id:300,href:"/zh/docs/excel/field-property/#选项-validate_complex",title:"字段属性（Field property） / 选项 validate_complex ",description:"Tableau field property 使用指南。",content:` 选项 validate_complex 用于指定复合类型（即 列表/映射）的字段级校验 规则——即对容器本身（而非其元素）施加的规则。其值为 buf.validate.FieldRules （repeated 或 map 字段已设置）的 text format 表示。
示例：
...
`}).add({id:301,href:"/zh/docs/excel/field-property/#选项-validate_message",title:"字段属性（Field property） / 选项 validate_message ",description:"Tableau field property 使用指南。",content:` 选项 validate_message 用于指定字段所嵌套 message 的 message 级校验规则。 通常在字段的值类型为子 message（例如结构体、列表元素或映射值的类型）时使用， 用于通过 CEL 表达式对 message 的多个字段进行交叉校验。其值为 buf.validate.MessageRules 的 text format 表示。
示例：
...
`}).add({id:302,href:"/zh/docs/excel/field-property/#选项-aggregate",title:"字段属性（Field property） / 选项 aggregate ",description:"Tableau field property 使用指南。",content:` 选项 aggregate 作用于单元格内（layout:LAYOUT_INCELL）或水平 （layout:LAYOUT_HORIZONTAL）的列表/映射字段。当同一父记录被多行/多列重复 访问时（同一映射键跨行、同一垂直/水平列表跨多行/多列等），aggregate:true 会把每次访问产生的元素聚合到父记录的同一个集合中，而不要求所有访问填写 完全相同的值。
...
`}).add({id:303,href:"/zh/docs/excel/field-property/#适用场景",title:"字段属性（Field property） / 适用场景 ",description:"Tableau field property 使用指南。",content:" 父级布局 aggregate 作用对象 重复访问场景 垂直映射/垂直键控列表（多行同键） 单元格内列表/映射（LAYOUT_INCELL）、水平列表/映射（LAYOUT_HORIZONTAL） 多行落到同一父记录 水平映射（多列同键） 单元格内列表/映射、水平列表/映射 多列落到同一父记录 垂直/水平列表自身跨多行/多列 兄弟单元格内字段在每行/列被重复访问 多行/多列落到同一父记录 "}).add({id:304,href:"/zh/docs/excel/field-property/#各类型聚合行为",title:"字段属性（Field property） / 各类型聚合行为 ",description:"Tableau field property 使用指南。",content:" 子字段类型 aggregate:true 行为 错误码 单元格内列表 各次访问的元素追加到同一列表 — 单元格内映射 各次访问的键值合并到同一映射；键重复报错 E2005 单元格内键控列表（[]&lt;T&gt;） 各次访问的元素追加并按键去重；元素重复报错 E2028 水平列表 跨行的水平列表元素追加到同一列表 — 水平键控列表 跨行的元素追加并按键去重 E2028 水平映射 跨行的键值合并到同一映射；键重复报错 E2005 "}).add({id:305,href:"/zh/docs/excel/field-property/#垂直聚合",title:"字段属性（Field property） / 垂直聚合 ",description:"Tableau field property 使用指南。",content:` 端到端示例参见 垂直列表（Vertical list） 中的对应小节：
垂直标量列表 —— 跨行聚合的 incell 标量列表。 垂直枚举列表 —— 跨行聚合的 incell 枚举列表。 垂直单元格内结构体列表 —— 跨行聚合的 incell 内联结构体列表。 垂直单元格内预定义结构体列表 —— 跨行聚合的 incell 预定义结构体列表。 `}).add({id:306,href:"/zh/docs/excel/field-property/#水平聚合",title:"字段属性（Field property） / 水平聚合 ",description:"Tableau field property 使用指南。",content:` 端到端示例参见映射嵌套列表/映射嵌套映射中的水平聚合用例：
垂直映射嵌套水平聚合列表 —— 跨行聚合的水平 list（LAYOUT_HORIZONTAL）。 垂直映射嵌套水平聚合映射 —— 跨行聚合的水平 map（LAYOUT_HORIZONTAL）。 `}).add({id:307,href:"/zh/docs/excel/field-property/#aggregate-与-key-组合keyed-list-去重",title:"字段属性（Field property） / aggregate 与 key 组合（keyed-list 去重） ",description:"Tableau field property 使用指南。",content:` 当 aggregate:true 与键控列表（[]&lt;T&gt; 语法，会自动设置字段的 key）一同使用时， tableau 在聚合过程中还会对元素去重：
标量/枚举键控列表：元素本身就是键——跨行/跨列重复的元素会触发 E2028。 结构体键控列表：仅比较通过 key 配置的子字段。 端到端示例参见 垂直键控列表 （垂直聚合）以及 垂直映射嵌套水平聚合列表（水平聚合）。
`}).add({id:308,href:"/zh/docs/excel/field-property/#注意",title:"字段属性（Field property） / 注意 ",description:"Tableau field property 使用指南。",content:" aggregate 仅在单元格内（layout:LAYOUT_INCELL）或水平 （layout:LAYOUT_HORIZONTAL）的列表/映射字段上生效。 父字段键跨行重复时，需要在父字段上同时设置 prop:{unique:false}， 否则会被 E2005 拦截。 聚合后，range、refer、unique 等针对单元素的字段属性仍会逐元素生效。 "}).add({id:309,href:"/zh/docs/excel/metasheet/#概述",title:"元表（Metasheet） / 概述 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 以下选项可在元表 @TABLEAU 中指定，用于影响对应工作表的布局、功能、loader 等。
...
示例：
- OrderedMap:cpp,Index:cpp go // ordered map 仅支持 cpp，index 支持 cpp 和 go - OrderedMap:cpp // ordered map 仅支持 cpp，index 支持所有语言 Validate string 工作表/messager 级别的 protovalidate message 级校验规则。其值为 buf.validate.MessageRules 的 text format 表示，例如：cel_expression:&quot;this.item_map.size() &gt; 0&quot;。也支持在 MODE_STRUCT_TYPE/MODE_UNION_TYPE 等工作表上使用。 `}).add({id:310,href:"/zh/docs/excel/metasheet/#空-tableau",title:"元表（Metasheet） / 空 @TABLEAU ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 如果元表 @TABLEAU 为空，则同一工作簿中的所有其他工作表都会被处理。
`}).add({id:311,href:"/zh/docs/excel/metasheet/#简单示例",title:"元表（Metasheet） / 简单示例 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` HelloWorld.xlsx 中有一个工作表 Sheet1，我们希望将其重命名为 ItemConf，定义自定义分隔符为 |，并生成 ordered map 访问器。
因此，HelloWorld.xlsx 中的元表 @TABLEAU 应配置如下：
HelloWorld.xlsx&nbsp; Sheet1 @TABLEAU ID Name map&lt;uint32, Item&gt; string Item&rsquo;s ID Item&rsquo;s Name 1 Apple 2 Orange 3 Banana Sheet Alias Sep OrderedMap Sheet1 ItemConf | true `}).add({id:312,href:"/zh/docs/excel/metasheet/#选项-alias",title:"元表（Metasheet） / 选项 Alias ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 生成的 proto 文件名是输入文件名的 snake_case 形式。例如，如果工作簿名为 HelloWorld.xlsx，则生成的 proto 文件名为 hello_world.proto。如果希望手动指定生成的 proto 文件名，可以使用 Alias 选项。在此场景中，# 表示工作簿名称。
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto HeroConf.json `}).add({id:315,href:"/zh/docs/excel/metasheet/#选项-merger",title:"元表（Metasheet） / 选项 Merger ",description:"Excel 元表 @TABLEAU 使用指南。",content:` Merger 选项用于将多个具有相同 schema 的工作表（逗号分隔）合并为一个。
每个元素可以是：
...
`}).add({id:316,href:"/zh/docs/excel/metasheet/#合并多个工作簿",title:"元表（Metasheet） / 合并多个工作簿 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 例如，有三个工作簿，每个都包含一个具有相同 schema 的工作表 ZoneConf：
...
merger_main.proto ZoneConf.json `}).add({id:317,href:"/zh/docs/excel/metasheet/#合并同一工作簿中的多个工作表",title:"元表（Metasheet） / 合并同一工作簿中的多个工作表 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 例如，同一工作簿 Merger.xlsx 中有三个具有相同 schema 的工作表：
...
生成结果：
merger_same.proto ZoneConf.json `}).add({id:318,href:"/zh/docs/excel/metasheet/#选项-scatter",title:"元表（Metasheet） / 选项 Scatter ",description:"Excel 元表 @TABLEAU 使用指南。",content:` Scatter 选项用于将多个具有相同 schema 的工作表（逗号分隔）分别转换为不同的配置文件。
每个元素可以是：
仅工作簿文件路径或 Glob 路径（相对于当前工作簿）：&lt;Workbook&gt;，此时工作表名称与当前工作表相同。 工作簿文件路径（相对于当前工作簿）加工作表名称：&lt;Workbook&gt;#&lt;Worksheet&gt;。 [!NOTE] Glob 模式通常不应匹配主工作簿。如果匹配，tableauc 会自动排除它。
...
Scatter1_ZoneConf.json Scatter2_ZoneConf.json Scatter3_ZoneConf.json `}).add({id:319,href:"/zh/docs/excel/metasheet/#选项-orderedmap",title:"元表（Metasheet） / 选项 OrderedMap ",description:"Excel 元表 @TABLEAU 使用指南。",content:` [!IMPORTANT] 仅适用于每个层级 message 的第一个映射字段。
...
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Name PropID PropValue map&lt;uint32, Item&gt; string map&lt;int32, Prop&gt; int64 Item&rsquo;s ID Item&rsquo;s name Prop&rsquo;s ID Prop&rsquo;s value 1 Apple 1 10 2 Orange 1 20 2 Orange 2 30 Sheet OrderedMap ItemConf true `}).add({id:320,href:"/zh/docs/excel/metasheet/#选项-index",title:"元表（Metasheet） / 选项 Index ",description:"Excel 元表 @TABLEAU 使用指南。",content:` [!IMPORTANT] 被索引的列必须是同一结构体（message）内的标量或枚举。
...
ID&lt;Name&gt;@Item：对 ID 列建立单列有序索引，结果数组按 Name 排序。 HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Name Desc map&lt;int32, Item&gt; string string Item&rsquo;s ID Item&rsquo;s name Item&rsquo;s desc 1 Apple A kind of delicious fruit. 2 Orange A kind of sour fruit. 3 Banana A kind of calorie-rich fruit. Sheet OrderedIndex ItemConf ID@Item `}).add({id:325,href:"/zh/docs/excel/metasheet/#多列有序索引",title:"元表（Metasheet） / 多列有序索引 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 格式：(Column1,Column2,...)&lt;ColumnX,ColumnY,...&gt;@IndexName。
...
(ID,Type)&lt;Type&gt;@Shop：对 (ID, Type) 列建立多列有序索引，结果数组按 Type 排序。 (ID,Type)&lt;Type,ID&gt;@SpecialShop：对 (ID, Type) 列建立多列有序索引，结果数组按 Type 然后 ID 排序。 HelloWorld.xlsx&nbsp; ShopConf @TABLEAU ID Type Desc [Shop]int32 int32 string Shop&rsquo;s ID Shop&rsquo;s type Shop&rsquo;s desc 1 1 Shoes shop. 2 1 T-Shirt shop. 3 2 Fruite shop. Sheet OrderedIndex ShopConf (ID,Type)@Shop, (ID,Type)&lt;Type,ID&gt;@SpecialShop `}).add({id:326,href:"/zh/docs/excel/metasheet/#选项-patch",title:"元表（Metasheet） / 选项 Patch ",description:"Excel 元表 @TABLEAU 使用指南。",content:`

`}).add({id:327,href:"/zh/docs/excel/metasheet/#选项-sep",title:"元表（Metasheet） / 选项 Sep ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 工作表的分隔符，用于分隔：
单元格内列表元素（标量或结构体）。 单元格内映射元素。 如果未设置，将使用 Tableauc 配置 中的全局级别分隔符（默认：,）。
`}).add({id:328,href:"/zh/docs/excel/metasheet/#选项-subsep",title:"元表（Metasheet） / 选项 Subsep ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 工作表的子分隔符，用于分隔：
每个单元格内映射元素的键值对。 每个单元格内结构体列表元素的结构体字段。 如果未设置，将使用 Tableauc 配置 中的全局级别子分隔符（默认：:）。
`}).add({id:329,href:"/zh/docs/excel/metasheet/#选项-validate",title:"元表（Metasheet） / 选项 Validate ",description:"Excel 元表 @TABLEAU 使用指南。",content:` Tableau 集成了 protovalidate， 可以直接在表格中声明校验规则。选项 Validate 是字段级 validate_message 选项在工作表/messager 级别的对应版本。它会被编译为生成工作表对应 proto message 上的 (buf.validate.message) 选项，并在 tableau 生成配置时强制执行。
...
hello_world.proto `}).add({id:330,href:"/zh/docs/basics/",title:"基础",description:"Tableau 基础知识。",content:""}).add({id:331,href:"/zh/docs/basics/concepts/#术语",title:"核心概念 / 术语 ",description:"Tableau 核心概念。",content:`
`}).add({id:332,href:"/zh/docs/basics/concepts/#基础术语",title:"核心概念 / 基础术语 ",description:"Tableau 核心概念。",content:" 术语 定义 Workbook 一个 Excel 文件；或一组以相同前缀命名、用 # 分隔的 CSV 文件；或一个 XML 文件；或一个 YAML 文件。 Worksheet Excel 文件中的一个工作表；或一个 CSV 文件；或 XML 文件的根节点；或 YAML 文件中的一个文档。 Metasheet 名为 @TABLEAU 的特殊工作表，用于指定 tableau 解析器选项。 Row 工作表中的一行。 Column 工作表中的一列。 Cell 行与列的交叉点（单元格）。 In-cell 单元格内部。 Cross-cell 一行或一列中连续的多个单元格。 "}).add({id:333,href:"/zh/docs/basics/concepts/#工作表相关术语",title:"核心概念 / 工作表相关术语 ",description:"Tableau 核心概念。",content:` 术语 定义 Namerow 工作表中列名定义所在的行号。
⚠️ 注意：同一工作表中每个列名必须唯一！
...
默认值：false。 Layout Incell（单元格内）、vertical（跨单元格垂直）或 horizontal（跨单元格水平）。 Transpose 对指定工作表进行行列转置。 `}).add({id:334,href:"/zh/docs/basics/concepts/#与-protoconf-的映射关系",title:"核心概念 / 与 Protoconf 的映射关系 ",description:"Tableau 核心概念。",content:" 术语 Protoconf Workbook 一个 protoconf（.proto）文件。 Worksheet protoconf 文件中的一个顶层 message（名为 @TABLEAU 的元表除外）。 column message 中的一个字段。 "}).add({id:335,href:"/zh/docs/basics/concepts/#简单映射示例",title:"核心概念 / 简单映射示例 ",description:"Tableau 核心概念。",content:`
`}).add({id:336,href:"/zh/docs/basics/concepts/#输入一个-excel-文件",title:"核心概念 / 输入：一个 Excel 文件 ",description:"Tableau 核心概念。",content:` 一个工作簿（HelloWorld.xlsx），包含两个数据工作表（ItemConf 和 ActivityConf）以及一个空的 tableau 元表（@TABLEAU）。
HelloWorld.xlsx&nbsp; ItemConf ActivityConf @TABLEAU ID Name Type map&lt;uint32, Item&gt; string int32 Item&rsquo;s ID. Item&rsquo;s name. Item&rsquo;s type. 1 item1 100 2 item2 200 3 item3 300 ID Name Open map&lt;uint32, Activity&gt; string bool Activity&rsquo;s ID. Activity&rsquo;s name. Activity is open? 1 activity1 true 2 activity2 false 3 activity3 `}).add({id:337,href:"/zh/docs/basics/concepts/#输出一个-protoconf-文件",title:"核心概念 / 输出：一个 protoconf 文件 ",description:"Tableau 核心概念。",content:` 一个 protoconf 文件（hello_world.proto），包含两个顶层 message（ItemConf 和 ActivityConf）。
hello_world.proto `}).add({id:338,href:"/zh/docs/basics/naming-convention/#枚举enums",title:"命名规范 / 枚举（Enums） ",description:"命名规范。",content:` 枚举类型名使用 PascalCase（首字母大写），枚举值名使用 CAPITALS_WITH_UNDERSCORES（全大写加下划线）：
参考 Protobuf 风格：枚举。
`}).add({id:339,href:"/zh/docs/basics/naming-convention/#示例",title:"命名规范 / 示例 ",description:"命名规范。",content:" 名称 风格 示例 workbook PascalCase HelloWorld.xlsx worksheet PascalCase HelloWorld struct (message) PascalCase HelloWorld field (column) PascalCase HelloWorld "}).add({id:340,href:"/zh/docs/basics/grammar-and-types/#概述",title:"语法与类型 / 概述 ",description:"语法与类型。",content:` Tableau 的大部分语法和类型借鉴自 Protocol Buffers (proto3) 和 Golang。
`}).add({id:341,href:"/zh/docs/basics/grammar-and-types/#scalar-types",title:"语法与类型 / 标量类型（Scalar types） ",description:"语法与类型。",content:` 详细说明请参考 Protocol Buffers Proto3 Scalar。
类别 类型 默认值 数值 int32, uint32
int64, uint64
float, double 0
0
0.0 布尔值 bool false 字符串 string &quot;&quot; 字节 bytes &quot;&quot; `}).add({id:342,href:"/zh/docs/basics/grammar-and-types/#复合类型composite-types",title:"语法与类型 / 复合类型（Composite types） ",description:"语法与类型。",content:" 类型 说明 struct struct 映射为 protobuf 的 message。 list list 映射为 protobuf 的 repeated 字段。 map map 映射为 protobuf 的 map 字段。 "}).add({id:343,href:"/zh/docs/basics/grammar-and-types/#结构体struct",title:"语法与类型 / 结构体（Struct） ",description:"语法与类型。",content:` 特性 说明 水平布局（Horizontal layout） 每个标量字段占一个单元格。 简单 in-cell struct 每个字段必须是标量类型。
以逗号分隔的字段列表，例如：1,test,3.0。
若数据列表长度与 struct 字段数不一致，则按顺序填充，未配置的字段使用标量类型的默认值。 `}).add({id:344,href:"/zh/docs/basics/grammar-and-types/#列表list",title:"语法与类型 / 列表（List） ",description:"语法与类型。",content:` 特性 说明 水平布局（Horizontal layout） list 的默认布局。
元素类型可以是 struct 或标量。 垂直布局（Vertical layout） list 的元素类型应为 struct。 简单 in-cell list 元素类型必须是标量。
以逗号分隔的元素列表，例如：1,2,3。 可扩展（Scalable） 支持动态大小的 list。 忽略空元素 智能识别任意位置的空元素。 `}).add({id:345,href:"/zh/docs/basics/grammar-and-types/#映射map",title:"语法与类型 / 映射（Map） ",description:"语法与类型。",content:` 特性 说明 水平布局（Horizontal layout） 垂直布局（Vertical layout） map 的默认布局。 Hash map 实现为无序 map 或 hash map。 Ordered map 由 tableauio/loader 支持。
- C++ 简单 in-cell map key 和 value 都必须是标量类型。
以逗号分隔的 key:value 对列表，例如：1:10,2:20,3:30。 可扩展（Scalable） 支持动态大小的 map。 忽略空条目 智能识别任意位置的空条目。 `}).add({id:346,href:"/zh/docs/basics/grammar-and-types/#枚举enumeration",title:"语法与类型 / 枚举（Enumeration） ",description:"语法与类型。",content:` 特性 说明 三种枚举值形式 1. 枚举值编号（number）。
2. 枚举值名称（name）。
3. 枚举值别名（alias，通过 EnumValueOptions 指定）。 校验 自动检查枚举值的合法性。 `}).add({id:347,href:"/zh/docs/basics/grammar-and-types/#空值empty-value",title:"语法与类型 / 空值（Empty value） ",description:"语法与类型。",content:` 类型 说明 scalar 空标量将使用该标量类型的默认值填充。 struct 若所有字段均为空，则不会生成该 struct。 list 若 list 大小为 0，则不会生成该 list。
若 list 的元素（struct 类型）为空，则不会追加该元素。 map 若 map 大小为 0，则不会生成该 map。若 map 的 value（struct 类型）为空，则不会插入该条目。 nesting 递归地判断是否为空。 `}).add({id:348,href:"/zh/docs/basics/enum/#枚举值enum-value",title:"枚举（Enum） / 枚举值（Enum value） ",description:"枚举基础。",content:` tableau 解析器支持三种枚举值形式：
...
`}).add({id:349,href:"/zh/docs/basics/enum/#校验validation",title:"枚举（Enum） / 校验（Validation） ",description:"枚举基础。",content:` 由于枚举类型是预先定义的，tableau 解析器会自动校验枚举值的合法性。
`}).add({id:350,href:"/zh/docs/basics/wellknown-types/#概述",title:"知名类型（Wellknown types） / 概述 ",description:"Wellknown 类型。",content:` 为方便使用，Wellknown 类型是 Tableau 的内置类型，类似于 Protocol Buffers Well-Known Types。
使用时需要引入 Tableau 和 Protocol Buffers 提供的 proto 文件：
tableau/protobuf/wellknown.proto google/protobuf/timestamp.proto google/protobuf/duration.proto `}).add({id:351,href:"/zh/docs/basics/wellknown-types/#datetime",title:"知名类型（Wellknown types） / 日期时间（Datetime） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Datetime 。
...
datetime 和 date 基于 google.protobuf.Timestamp，参考 JSON 映射。 time 基于 google.protobuf.Duration，参考 JSON 映射。 RFC 3339: 互联网日期和时间戳 `}).add({id:352,href:"/zh/docs/basics/wellknown-types/#duration",title:"知名类型（Wellknown types） / 时长（Duration） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Duration 。
...
duration 基于 google.protobuf.Duration，参考 JSON 映射。 golang duration 字符串格式。 golang ParseDuration。 `}).add({id:353,href:"/zh/docs/basics/wellknown-types/#fraction",title:"知名类型（Wellknown types） / 分数（Fraction） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Fraction 。
...
- N/D：简单分数，例如：3/4
- N：仅分子，例如：3 等同于 3/1
- N：浮点分子，例如：0.01 等同于 1/100 `}).add({id:354,href:"/zh/docs/basics/wellknown-types/#comparator",title:"知名类型（Wellknown types） / 比较器（Comparator） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Comparator 。
comparator 包含一个 sign（符号）和一个分数 value（值），任意数字或分数均可与之比较。
类型 默认值 说明 comparator ==0 格式：&lt;Sign&gt;&lt;Fraction&gt;。
例如：==10、!=1/2、&lt;10%、&lt;=10‰、&gt;10%、&gt;=10‱ `}).add({id:355,href:"/zh/docs/basics/wellknown-types/#version",title:"知名类型（Wellknown types） / 版本号（Version） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Version 。
...
例如：1.0.1.1 `}).add({id:356,href:"/zh/docs/basics/predefined-types/#概述",title:"预定义类型（Predefined types） / 概述 ",description:"预定义类型。",content:` 你可以在 protoconf 文件（如 common.proto）中预先定义 enum、struct 或 union 类型，然后在工作表中将其用作列类型或跨单元格类型。
`}).add({id:357,href:"/zh/docs/basics/predefined-types/#使用方式",title:"预定义类型（Predefined types） / 使用方式 ",description:"预定义类型。",content:" 语法：在工作表中使用预定义的 CustomType 时，需在前面加一个点 .（即 .CustomType）。 导入：在 tableauc 配置的 protoFiles 选项中指定公共 proto 文件，这些文件中定义了预定义的 enum、struct、union 类型。参考 Tableauc 配置。 "}).add({id:358,href:"/zh/docs/basics/predefined-types/#枚举enum",title:"预定义类型（Predefined types） / 枚举（Enum） ",description:"预定义类型。",content:` 例如，common.proto 中定义的枚举类型 FruitType：
以下示例说明如何使用预定义枚举类型：
Excel/CSV：使用预定义枚举类型。 XML：使用预定义枚举类型 YAML：使用预定义枚举类型 `}).add({id:359,href:"/zh/docs/basics/predefined-types/#结构体struct",title:"预定义类型（Predefined types） / 结构体（Struct） ",description:"预定义类型。",content:` 例如，common.proto 中定义的 struct 类型 Prop：
