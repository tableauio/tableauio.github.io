...
生成结果：
hello_world.proto ItemConf.json `}).add({id:77,href:"/zh/docs/yaml/enum/#使用预定义枚举类型",title:"枚举（Enum） / 使用预定义枚举类型 ",description:"YAML enum 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:78,href:"/zh/docs/api/tableau/guide/#前置条件",title:"指南 / 前置条件 ",description:"Tableau Go API 使用指南。",content:` Go，任意最新三个主要版本之一，参考 Go 发布记录。 安装说明请参考 Go 的 Getting Started 指南。 Protocol buffer 编译器 protoc，版本 3。 安装说明请参考 Protocol Buffer Compiler Installation。 协议编译器的 Go 插件： 使用以下命令安装 Go 的协议编译器插件：
更新 PATH，使 protoc 编译器能找到插件：
`}).add({id:79,href:"/zh/docs/api/tableau/guide/#获取示例代码",title:"指南 / 获取示例代码 ",description:"Tableau Go API 使用指南。",content:` 示例代码位于 tableau/demo 仓库。
下载仓库 zip 包 并解压，或克隆仓库：
切换到快速开始示例目录：
`}).add({id:80,href:"/zh/docs/api/tableau/guide/#运行示例",title:"指南 / 运行示例 ",description:"Tableau Go API 使用指南。",content:` 在 examples/helloworld 目录下：
...
`}).add({id:81,href:"/zh/docs/yaml/struct/#通用结构体",title:"结构体（Struct） / 通用结构体 ",description:"YAML struct 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
提示
Wellknown 类型：datetime Wellknown 类型：duration 生成结果：
hello_world.proto ItemConf.json `}).add({id:82,href:"/zh/docs/yaml/struct/#复用同级结构体",title:"结构体（Struct） / 复用同级结构体 ",description:"YAML struct 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:83,href:"/zh/docs/yaml/struct/#predefined-struct",title:"结构体（Struct） / 预定义结构体（Predefined struct） ",description:"YAML struct 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:84,href:"/zh/docs/yaml/struct/#单元格内结构体incell-struct",title:"结构体（Struct） / 单元格内结构体（Incell struct） ",description:"YAML struct 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:85,href:"/zh/docs/yaml/struct/#单元格内通用结构体incell-general-struct",title:"结构体（Struct） / 单元格内通用结构体（Incell general struct） ",description:"YAML struct 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:86,href:"/zh/docs/yaml/struct/#单元格内预定义结构体incell-predefined-struct",title:"结构体（Struct） / 单元格内预定义结构体（Incell predefined struct） ",description:"YAML struct 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:87,href:"/zh/docs/yaml/union/#联合体定义union-definition",title:"联合体（Union） / 联合体定义（Union definition） ",description:"YAML union 使用指南。",content:` 例如，common.proto 中预定义的联合体类型 Target：
`}).add({id:88,href:"/zh/docs/yaml/union/#predefined-union",title:"联合体（Union） / 预定义联合体（Predefined union） ",description:"YAML union 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:89,href:"/zh/docs/yaml/union/#预定义单元格内联合体predefined-incell-union",title:"联合体（Union） / 预定义单元格内联合体（Predefined incell union） ",description:"YAML union 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:90,href:"/zh/docs/yaml/union/#predefined-union-list",title:"联合体（Union） / 预定义联合体列表（Predefined union list） ",description:"YAML union 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:91,href:"/zh/docs/yaml/list/#标量列表scalar-list",title:"列表（List） / 标量列表（Scalar list） ",description:"YAML list 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:92,href:"/zh/docs/yaml/list/#枚举列表enum-list",title:"列表（List） / 枚举列表（Enum list） ",description:"YAML list 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:93,href:"/zh/docs/yaml/list/#单元格内标量列表incell-scalar-list",title:"列表（List） / 单元格内标量列表（Incell scalar list） ",description:"YAML list 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:94,href:"/zh/docs/yaml/list/#单元格内枚举列表incell-enum-list",title:"列表（List） / 单元格内枚举列表（Incell enum list） ",description:"YAML list 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:95,href:"/zh/docs/yaml/list/#结构体列表struct-list",title:"列表（List） / 结构体列表（Struct list） ",description:"YAML list 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:96,href:"/zh/docs/yaml/list/#predefined-struct-list",title:"列表（List） / 预定义结构体列表（Predefined struct list） ",description:"YAML list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:97,href:"/zh/docs/yaml/list/#列表嵌套列表list-in-list",title:"列表（List） / 列表嵌套列表（List in list） ",description:"YAML list 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:98,href:"/zh/docs/yaml/list/#列表嵌套映射map-in-list",title:"列表（List） / 列表嵌套映射（Map in list） ",description:"YAML list 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:99,href:"/zh/docs/yaml/map/#标量映射scalar-map",title:"映射（Map） / 标量映射（Scalar map） ",description:"YAML map 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:100,href:"/zh/docs/yaml/map/#枚举键映射enum-key-map",title:"映射（Map） / 枚举键映射（Enum key map） ",description:"YAML map 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:101,href:"/zh/docs/yaml/map/#枚举键值映射enum-key-value-map",title:"映射（Map） / 枚举键值映射（Enum key-value map） ",description:"YAML map 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType 和 FruitFlavor：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:102,href:"/zh/docs/yaml/map/#单元格内标量映射incell-scalar-map",title:"映射（Map） / 单元格内标量映射（Incell scalar map） ",description:"YAML map 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:103,href:"/zh/docs/yaml/map/#单元格内枚举映射incell-enum-map",title:"映射（Map） / 单元格内枚举映射（Incell enum map） ",description:"YAML map 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType 和 FruitFlavor：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:104,href:"/zh/docs/yaml/map/#结构体映射struct-map",title:"映射（Map） / 结构体映射（Struct map） ",description:"YAML map 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:105,href:"/zh/docs/yaml/map/#枚举键结构体映射enum-key-struct-map",title:"映射（Map） / 枚举键结构体映射（Enum key struct map） ",description:"YAML map 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:106,href:"/zh/docs/yaml/map/#自定义键结构体映射custom-key-struct-map",title:"映射（Map） / 自定义键结构体映射（Custom key struct map） ",description:"YAML map 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:107,href:"/zh/docs/yaml/map/#映射嵌套列表list-in-map",title:"映射（Map） / 映射嵌套列表（List in map） ",description:"YAML map 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:108,href:"/zh/docs/yaml/map/#映射嵌套映射map-in-map",title:"映射（Map） / 映射嵌套映射（Map in map） ",description:"YAML map 使用指南。",content:` HelloWorld.yaml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:109,href:"/zh/docs/yaml/metasheet/#概述",title:"元表（Metasheet） / 概述 ",description:"YAML 元表 @TABLEAU 使用指南。",content:` 名为 &ldquo;@TABLEAU&rdquo; 的元表用于指定 tableau 解析器选项。 详细信息请参考 元表。
YAML 元表示例：
`}).add({id:110,href:"/zh/docs/yaml/metasheet/#todo",title:"元表（Metasheet） / TODO ",description:"YAML 元表 @TABLEAU 使用指南。",content:` 更多详情&hellip;
`}).add({id:111,href:"/zh/docs/xml/scalar/#标量scalar",title:"标量（Scalar） / 标量（Scalar） ",description:"XML scalar 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:112,href:"/zh/docs/xml/",title:"XML",description:"XML 使用指南。",content:""}).add({id:113,href:"/zh/docs/xml/enum/#使用预定义枚举类型",title:"枚举（Enum） / 使用预定义枚举类型 ",description:"XML enum 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:114,href:"/zh/docs/xml/struct/#通用结构体",title:"结构体（Struct） / 通用结构体 ",description:"XML struct 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
提示
Wellknown 类型：datetime Wellknown 类型：duration 生成结果：
hello_world.proto ItemConf.json `}).add({id:115,href:"/zh/docs/xml/struct/#复用同级结构体",title:"结构体（Struct） / 复用同级结构体 ",description:"XML struct 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:116,href:"/zh/docs/xml/struct/#predefined-struct",title:"结构体（Struct） / 预定义结构体（Predefined struct） ",description:"XML struct 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:117,href:"/zh/docs/xml/struct/#单元格内结构体incell-struct",title:"结构体（Struct） / 单元格内结构体（Incell struct） ",description:"XML struct 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:118,href:"/zh/docs/xml/struct/#单元格内预定义结构体incell-predefined-struct",title:"结构体（Struct） / 单元格内预定义结构体（Incell predefined struct） ",description:"XML struct 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:119,href:"/zh/docs/xml/union/#联合体定义",title:"联合体（Union） / 联合体定义 ",description:"XML union 使用指南。",content:` 例如，common.proto 中预定义的 union 类型 Target：
`}).add({id:120,href:"/zh/docs/xml/union/#predefined-union",title:"联合体（Union） / 预定义联合体（Predefined union） ",description:"XML union 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:121,href:"/zh/docs/xml/union/#预定义单元格内联合体predefined-incell-union",title:"联合体（Union） / 预定义单元格内联合体（Predefined incell union） ",description:"XML union 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:122,href:"/zh/docs/xml/union/#predefined-union-list",title:"联合体（Union） / 预定义联合体列表（Predefined union list） ",description:"XML union 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:123,href:"/zh/docs/xml/list/#标量列表scalar-list",title:"列表（List） / 标量列表（Scalar list） ",description:"XML list 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:124,href:"/zh/docs/xml/list/#枚举列表enum-list",title:"列表（List） / 枚举列表（Enum list） ",description:"XML list 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:125,href:"/zh/docs/xml/list/#单元格内标量列表incell-scalar-list",title:"列表（List） / 单元格内标量列表（Incell scalar list） ",description:"XML list 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:126,href:"/zh/docs/xml/list/#单元格内枚举列表incell-enum-list",title:"列表（List） / 单元格内枚举列表（Incell enum list） ",description:"XML list 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:127,href:"/zh/docs/xml/list/#结构体列表struct-list",title:"列表（List） / 结构体列表（Struct list） ",description:"XML list 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:128,href:"/zh/docs/xml/list/#predefined-struct-list",title:"列表（List） / 预定义结构体列表（Predefined struct list） ",description:"XML list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:129,href:"/zh/docs/xml/list/#列表嵌套列表list-in-list",title:"列表（List） / 列表嵌套列表（List in list） ",description:"XML list 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:130,href:"/zh/docs/xml/list/#列表嵌套映射map-in-list",title:"列表（List） / 列表嵌套映射（Map in list） ",description:"XML list 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:131,href:"/zh/docs/xml/map/#单元格内标量映射incell-scalar-map",title:"映射（Map） / 单元格内标量映射（Incell scalar map） ",description:"XML map 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:132,href:"/zh/docs/xml/map/#单元格内枚举映射incell-enum-map",title:"映射（Map） / 单元格内枚举映射（Incell enum map） ",description:"XML map 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType 和 FruitFlavor：
HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:133,href:"/zh/docs/xml/map/#结构体映射struct-map",title:"映射（Map） / 结构体映射（Struct map） ",description:"XML map 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:134,href:"/zh/docs/xml/map/#枚举键结构体映射enum-key-struct-map",title:"映射（Map） / 枚举键结构体映射（Enum key struct map） ",description:"XML map 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:135,href:"/zh/docs/xml/map/#映射嵌套列表list-in-map",title:"映射（Map） / 映射嵌套列表（List in map） ",description:"XML map 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:136,href:"/zh/docs/xml/map/#映射嵌套映射map-in-map",title:"映射（Map） / 映射嵌套映射（Map in map） ",description:"XML map 使用指南。",content:` HelloWorld.xml 中的工作表 ItemConf：
生成结果：
hello_world.proto ItemConf.json `}).add({id:137,href:"/zh/docs/xml/metasheet/#xml-中的元表是什么",title:"元表（Metasheet） / XML 中的元表是什么？ ",description:"XML 元表 @TABLEAU 使用指南。",content:` 元表 是通常写在 XML 文件开头的注释块，必须以关键字 @TABLEAU 开头，并在后续行中定义节点属性的类型。例如：
`}).add({id:138,href:"/zh/docs/xml/metasheet/#没有元表的情况",title:"元表（Metasheet） / 没有元表的情况 ",description:"XML 元表 @TABLEAU 使用指南。",content:` 如果一个 XML 文件没有元表（即没有以 @TABLEAU 开头的注释块），则不会生成任何 protobuf 和 JSON 文件。
`}).add({id:139,href:"/zh/docs/csv/",title:"CSV",description:"CSV 使用指南。",content:""}).add({id:140,href:"/zh/docs/csv/overview/#概念",title:"概览 / 概念 ",description:"CSV 概览。",content:` Tableau 识别 &lt;BookName&gt;#&lt;SheetName&gt;.csv 模式，因此一个 CSV 工作簿（Glob 模式）&lt;BookName&gt;#*.csv 由同一目录下的多个 CSV 工作表（文件）组成。
示例：
CSV 工作簿 HelloWorld#*.csv 由三个 CSV 工作表组成：
工作表 Item：HelloWorld#Item.csv 工作表 Activity：HelloWorld#Activity.csv 工作表 @TABLEAU：HelloWorld#@TABLEAU.csv `}).add({id:141,href:"/zh/docs/csv/overview/#指南",title:"概览 / 指南 ",description:"CSV 概览。",content:` 由于 CSV 工作表与 Excel 工作表相同，请直接阅读 Excel 指南
`}).add({id:142,href:"/zh/docs/excel/",title:"Excel",description:"Excel 使用指南。",content:""}).add({id:143,href:"/zh/docs/excel/scalar/#标量scalar",title:"标量（Scalar） / 标量（Scalar） ",description:"Excel scalar 使用指南。",content:` HelloWorld.xlsx 中的工作表 Apple：
...
hello_world.proto Apple.json `}).add({id:144,href:"/zh/docs/excel/scalar/#说明",title:"标量（Scalar） / 说明 ",description:"Excel scalar 使用指南。",content:` 标量类型通常用于定义结构体类型的字段。
`}).add({id:145,href:"/zh/docs/excel/enum/#使用预定义枚举类型",title:"枚举（Enum） / 使用预定义枚举类型 ",description:"Excel enum 使用指南。",content:` [!NOTE] 枚举基础知识请参考 枚举 。
...
hello_world.proto ItemConf.json `}).add({id:146,href:"/zh/docs/excel/enum/#在工作表中定义枚举类型",title:"枚举（Enum） / 在工作表中定义枚举类型 ",description:"Excel enum 使用指南。",content:` 在元表 @TABLEAU 中有两种 Mode 可用于在工作表中定义枚举类型：
MODE_ENUM_TYPE：在一个工作表中定义单个枚举类型。 MODE_ENUM_TYPE_MULTI：在一个工作表中定义多个枚举类型。 `}).add({id:147,href:"/zh/docs/excel/enum/#单个枚举类型single-enum-type-in-sheet",title:"枚举（Enum） / 单个枚举类型（Single enum type in sheet） ",description:"Excel enum 使用指南。",content:` [!Note]
...
hello_world.proto `}).add({id:148,href:"/zh/docs/excel/enum/#多个枚举类型",title:"枚举（Enum） / 多个枚举类型 ",description:"Excel enum 使用指南。",content:` [!IMPORTANT] 每个枚举类型由一个 block 定义，即一系列连续的非空行。不同的 block 之间由一行或多行空行分隔。
...
hello_world.proto `}).add({id:149,href:"/zh/docs/excel/enum/#在工作表中定义并使用枚举类型",title:"枚举（Enum） / 在工作表中定义并使用枚举类型 ",description:"Excel enum 使用指南。",content:` 例如，HelloWorld.xlsx 中的两个工作表 ItemType 和 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:150,href:"/zh/docs/excel/struct/#跨单元格结构体cross-cell-struct",title:"结构体（Struct） / 跨单元格结构体（Cross-cell struct） ",description:"Excel struct 使用指南。",content:` 语法：&lt;StructType&gt;ColumnType。
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:151,href:"/zh/docs/excel/struct/#说明",title:"结构体（Struct） / 说明 ",description:"Excel struct 使用指南。",content:` 跨单元格结构体通常与以下类型配合使用：
跨单元格水平/垂直映射，作为映射的值类型。Map 跨单元格水平/垂直列表，作为列表的元素类型。List `}).add({id:152,href:"/zh/docs/excel/struct/#incell-struct",title:"结构体（Struct） / 单元格内结构体（Incell struct） ",description:"Excel struct 使用指南。",content:` 结构体的每个字段类型应为标量类型。
例如，HelloWorld.xlsx 中的工作表 ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:153,href:"/zh/docs/excel/struct/#predefined-struct",title:"结构体（Struct） / 预定义结构体（Predefined struct） ",description:"Excel struct 使用指南。",content:` 例如，common.proto 中定义的结构体类型 Prop：
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:154,href:"/zh/docs/excel/struct/#预定义单元格内结构体predefined-incell-struct",title:"结构体（Struct） / 预定义单元格内结构体（Predefined incell struct） ",description:"Excel struct 使用指南。",content:` 预定义结构体的每个字段类型应为标量类型。
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:155,href:"/zh/docs/excel/struct/#custom-named-struct",title:"结构体（Struct） / 自定义命名结构体（Custom named struct） ",description:"Excel struct 使用指南。",content:` 默认情况下，结构体变量名与结构体类型名相同，但你可以指定不同的结构体变量名。自定义命名结构体主要用于在 tableau（protogen）无法自动识别变量名时，标识 name row 中连续单元格的名称前缀。
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:156,href:"/zh/docs/excel/struct/#advanced-predefined-incell-struct",title:"结构体（Struct） / 高级预定义单元格内结构体（Advanced predefined incell struct） ",description:"Excel struct 使用指南。",content:` 在某些情况下，你可能希望在单元格中配置任意复杂的结构体，因此 tableau 支持两种 protobuf 序列化格式：text format 和 JSON format。
...
hello_world.proto ItemConf.json `}).add({id:157,href:"/zh/docs/excel/struct/#在工作表中定义结构体类型define-struct-type-in-sheet",title:"结构体（Struct） / 在工作表中定义结构体类型（Define struct type in sheet） ",description:"Excel struct 使用指南。",content:` 在元表 @TABLEAU 中有两种 Mode 可用于在工作表中定义结构体类型：
MODE_STRUCT_TYPE：在一个工作表中定义单个结构体类型。 MODE_STRUCT_TYPE_MULTI：在一个工作表中定义多个结构体类型。 `}).add({id:158,href:"/zh/docs/excel/struct/#单个结构体类型single-struct-type-in-sheet",title:"结构体（Struct） / 单个结构体类型（Single struct type in sheet） ",description:"Excel struct 使用指南。",content:` [!Note] Number 列是可选的，用于指定字段编号。省略时从 1 开始自动递增。
...
hello_world.proto `}).add({id:159,href:"/zh/docs/excel/struct/#多个结构体类型multiple-struct-types-in-sheet",title:"结构体（Struct） / 多个结构体类型（Multiple struct types in sheet） ",description:"Excel struct 使用指南。",content:` [!IMPORTANT] 每个结构体类型由一个 block 定义，即一系列连续的非空行。不同的 block 之间由一行或多行空行分隔。
...
hello_world.proto `}).add({id:160,href:"/zh/docs/excel/union/#原理",title:"联合体（Union） / 原理 ",description:"Excel union 使用指南。",content:` 在 protoconf 中，union 类型是指带标签的联合体（tagged union）：一种用于保存可以取多种不同但固定类型值的数据结构。任意时刻只有一种类型在使用，且一个 tag 字段明确指示当前使用的是哪种类型。更多详情请参考维基百科 Tagged union。
...
`}).add({id:161,href:"/zh/docs/excel/union/#union-definition",title:"联合体（Union） / 联合体定义（Union definition） ",description:"Excel union 使用指南。",content:` 例如，common.proto 中预定义的联合体类型 Target：
`}).add({id:162,href:"/zh/docs/excel/union/#predefined-union-in-list",title:"联合体（Union） / 列表嵌套预定义联合体（Predefined-union in list） ",description:"Excel union 使用指南。",content:` [!NOTE] 基于 predefined union 类型 Target。
...
hello_world.proto TaskConf.json TaskConf.txt `}).add({id:163,href:"/zh/docs/excel/union/#predefined-union-in-map",title:"联合体（Union） / 映射嵌套预定义联合体（Predefined-union in map） ",description:"Excel union 使用指南。",content:` [!NOTE] 基于 predefined union 类型 Target。
HelloWorld.xlsx 中的工作表 TaskConf：
...
hello_world.proto TaskConf.json TaskConf.txt `}).add({id:164,href:"/zh/docs/excel/union/#predefined-incell-union-in-map",title:"联合体（Union） / 映射嵌套预定义单元格内联合体（Predefined-incell-union in map） ",description:"Excel union 使用指南。",content:` [!NOTE] 基于 predefined union 类型 Target。
HelloWorld.xlsx 中的工作表 TaskConf：
...
hello_world.proto TaskConf.json `}).add({id:165,href:"/zh/docs/excel/union/#在工作表中定义联合体类型define-union-type-in-sheet",title:"联合体（Union） / 在工作表中定义联合体类型（Define union type in sheet） ",description:"Excel union 使用指南。",content:` 在元表 @TABLEAU 中有两种 Mode 可用于在工作表中定义联合体类型：
MODE_UNION_TYPE：在一个工作表中定义单个联合体类型。 MODE_UNION_TYPE_MULTI：在一个工作表中定义多个联合体类型。 每个联合体字段可以使用以下类型定义：
Scalar Enum Wellknown types Incell struct Incell list Incell map `}).add({id:166,href:"/zh/docs/excel/union/#单个联合体类型single-union-type-in-sheet",title:"联合体（Union） / 单个联合体类型（Single union type in sheet） ",description:"Excel union 使用指南。",content:` [!Note] Number 列是可选的，用于指定字段编号及对应的枚举值编号。省略时从 1 开始自动递增。
...
hello_world.proto `}).add({id:167,href:"/zh/docs/excel/union/#多个联合体类型multiple-union-types-in-sheet",title:"联合体（Union） / 多个联合体类型（Multiple union types in sheet） ",description:"Excel union 使用指南。",content:` [!IMPORTANT] 每个 union 类型由一个 block 定义，即一系列连续的非空行。不同的 block 之间由一行或多行空行分隔。
...
hello_world.proto `}).add({id:168,href:"/zh/docs/excel/union/#指定-type-列specify-type-column",title:"联合体（Union） / 指定 Type 列（Specify Type column） ",description:"Excel union 使用指南。",content:` 默认情况下，每个联合体的 oneof 字段是一个以 Name 列指定名称的 message 类型。 现在，你可以添加 Type 列并指定自定义的 oneof 字段类型：
...
hello_world.proto `}).add({id:169,href:"/zh/docs/excel/union/#复杂联合体类型complex-union-type-in-sheet",title:"联合体（Union） / 复杂联合体类型（Complex union type in sheet） ",description:"Excel union 使用指南。",content:` 例如，HelloWorld.xlsx 中的两个工作表 Target 和 TaskConf：
...
hello_world.proto TaskConf.json `}).add({id:170,href:"/zh/docs/excel/list/#水平列表horizontal-list",title:"列表（List） / 水平列表（Horizontal list） ",description:"列表使用指南。",content:` [!IMPORTANT] 水平列表的列名中，列表元素名必须带有从 1 开始的数字后缀。
例如：Item1ID、Item2ID、Item3ID（结构体列表，元素名：Item）；ID1、ID2、ID3（标量列表，元素名：ID）。
水平列表语法概览：
List 元素类型 语法示例 scalar []uint32 enum []enum&lt;.FruitType&gt; struct [Item]uint32 predefined struct [.Item]uint32 incell struct []{uint32 ID, string Num}Item incell predefined struct []{.Item} `}).add({id:171,href:"/zh/docs/excel/list/#水平标量列表horizontal-scalar-list",title:"列表（List） / 水平标量列表（Horizontal scalar list） ",description:"列表使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:172,href:"/zh/docs/excel/list/#水平枚举列表horizontal-enum-list",title:"列表（List） / 水平枚举列表（Horizontal enum list） ",description:"列表使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:173,href:"/zh/docs/excel/list/#水平结构体列表horizontal-struct-list",title:"列表（List） / 水平结构体列表（Horizontal struct list） ",description:"列表使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:174,href:"/zh/docs/excel/list/#水平预定义结构体列表horizontal-predefined-struct-list",title:"列表（List） / 水平预定义结构体列表（Horizontal predefined-struct list） ",description:"列表使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:175,href:"/zh/docs/excel/list/#水平单元格内结构体列表horizontal-incell-struct-list",title:"列表（List） / 水平单元格内结构体列表（Horizontal incell-struct list） ",description:"列表使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:176,href:"/zh/docs/excel/list/#水平单元格内预定义结构体列表horizontal-incell-predefined-struct-list",title:"列表（List） / 水平单元格内预定义结构体列表（Horizontal incell-predefined-struct list） ",description:"列表使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:177,href:"/zh/docs/excel/list/#垂直列表vertical-list",title:"列表（List） / 垂直列表（Vertical list） ",description:"列表使用指南。",content:` 垂直列表语法概览：
List 元素类型 语法示例 标量 []uint32 枚举 []enum&lt;.FruitType&gt; 结构体 [Item]uint32 预定义结构体 [.Item]int32 单元格内结构体 []{int32 ID,int32 Num}Item 单元格内预定义结构体 []{.Item} `}).add({id:178,href:"/zh/docs/excel/list/#垂直标量列表vertical-scalar-list",title:"列表（List） / 垂直标量列表（Vertical scalar list） ",description:"列表使用指南。",content:` [!NOTE] 定义方式与 单元格内标量列表 相同，但需要显式开启字段属性 aggregate:true， 才会把每一行的 incell list 合并到同一条记录的扁平列表中。 如果未设置 aggregate:true，同一父记录的多行数据会因一致性校验失败而报 E2023。
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []uint32|{aggregate:true} ID 1,2,3 1,2 1 生成结果：
hello_world.proto ItemConf.json `}).add({id:179,href:"/zh/docs/excel/list/#垂直枚举列表vertical-enum-list",title:"列表（List） / 垂直枚举列表（Vertical enum list） ",description:"列表使用指南。",content:` [!NOTE] 定义方式与 单元格内枚举列表 相同，但需要显式开启字段属性 aggregate:true， 才会把每一行的 incell list 合并到同一条记录的扁平列表中。 如果未设置 aggregate:true，同一父记录的多行数据会因一致性校验失败而报 E2023。
...
hello_world.proto ItemConf.json `}).add({id:180,href:"/zh/docs/excel/list/#垂直结构体列表vertical-struct-list",title:"列表（List） / 垂直结构体列表（Vertical struct list） ",description:"列表使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:184,href:"/zh/docs/excel/list/#incell-list",title:"列表（List） / 单元格内列表（Incell list） ",description:"列表使用指南。",content:` 单元格内列表语法概览：
List 元素类型 语法示例 标量 []int32 枚举 []enum&lt;.FruitType&gt; 结构体 []{int32 ID,int32 Num}Item 预定义结构体 []{.Item} `}).add({id:185,href:"/zh/docs/excel/list/#单元格内标量列表incell-scalar-list",title:"列表（List） / 单元格内标量列表（Incell scalar list） ",description:"列表使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:186,href:"/zh/docs/excel/list/#单元格内枚举列表incell-enum-list",title:"列表（List） / 单元格内枚举列表（Incell enum list） ",description:"列表使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xlsx 中的工作表 ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:187,href:"/zh/docs/excel/list/#单元格内结构体列表incell-struct-list",title:"列表（List） / 单元格内结构体列表（Incell struct list） ",description:"列表使用指南。",content:` [!NOTE] 更高级的 incell 数据解析，请参考 高级 predefined incell struct。
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{int32 ID,int32 Num}Item Item&rsquo;s info 1:100,2:200,3:300 生成结果：
hello_world.proto ItemConf.json `}).add({id:188,href:"/zh/docs/excel/list/#单元格内预定义结构体列表incell-predefined-struct-list",title:"列表（List） / 单元格内预定义结构体列表（Incell predefined-struct list） ",description:"列表使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Item []{.Item} Item&rsquo;s info 1:100,2:200,3:300 生成结果：
hello_world.proto ItemConf.json `}).add({id:189,href:"/zh/docs/excel/list/#水平列表大小",title:"列表（List） / 水平列表大小 ",description:"列表使用指南。",content:`
`}).add({id:190,href:"/zh/docs/excel/list/#动态大小",title:"列表（List） / 动态大小 ",description:"列表使用指南。",content:` 默认情况下，所有列表都是动态大小类型。列表元素应连续存在，否则如果中间存在空元素会报错。
`}).add({id:191,href:"/zh/docs/excel/list/#固定大小",title:"列表（List） / 固定大小 ",description:"列表使用指南。",content:` 隐式固定大小 # 列表大小由名称行中最大存在的列表元素数量自动确定。
...
hello_world.proto ItemConf.json `}).add({id:192,href:"/zh/docs/excel/list/#高级特性",title:"列表（List） / 高级特性 ",description:"列表使用指南。",content:`
`}).add({id:193,href:"/zh/docs/excel/list/#水平跳列列表horizontal-column-skipped-list",title:"列表（List） / 水平跳列列表（Horizontal column-skipped list） ",description:"列表使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:194,href:"/zh/docs/excel/map/#水平映射horizontal-map",title:"映射（Map） / 水平映射（Horizontal map） ",description:"映射使用指南。",content:` [!IMPORTANT] 水平映射的列名中，映射值名必须带有从 1 开始的数字后缀。
例如：Item1ID、Item1Name、Item2ID、Item2Name（结构体映射，值名：Item）。
水平映射有以下几种：
水平标量映射，映射值类型为标量。例如：map&lt;int32, int32&gt;。 水平结构体映射，映射值类型为结构体。例如：map&lt;int32, Item&gt;。 水平预定义结构体映射，映射值类型为预定义结构体。例如：map&lt;int32, .Item&gt;。 `}).add({id:195,href:"/zh/docs/excel/map/#水平标量映射horizontal-scalar-map",title:"映射（Map） / 水平标量映射（Horizontal scalar map） ",description:"映射使用指南。",content:` 无需单独支持，请使用 map&lt;int32, Item&gt; 代替。
`}).add({id:196,href:"/zh/docs/excel/map/#水平结构体映射horizontal-struct-map",title:"映射（Map） / 水平结构体映射（Horizontal struct map） ",description:"映射使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:197,href:"/zh/docs/excel/map/#水平预定义结构体映射horizontal-predefined-struct-map",title:"映射（Map） / 水平预定义结构体映射（Horizontal predefined-struct map） ",description:"映射使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:198,href:"/zh/docs/excel/map/#垂直映射vertical-map",title:"映射（Map） / 垂直映射（Vertical map） ",description:"映射使用指南。",content:` 垂直映射有以下几种：
垂直标量映射，映射值类型为标量。例如：map&lt;int32, int32&gt;。 垂直结构体映射，映射值类型为结构体。例如：map&lt;int32, Item&gt;。 垂直预定义结构体映射，映射值类型为预定义结构体。例如：map&lt;int32, .Item&gt;。 `}).add({id:199,href:"/zh/docs/excel/map/#垂直标量映射vertical-scalar-map",title:"映射（Map） / 垂直标量映射（Vertical scalar map） ",description:"映射使用指南。",content:` 无需单独支持，请使用 map&lt;int32, Item&gt; 代替。
`}).add({id:200,href:"/zh/docs/excel/map/#垂直结构体映射vertical-struct-map",title:"映射（Map） / 垂直结构体映射（Vertical struct map） ",description:"映射使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:201,href:"/zh/docs/excel/map/#垂直预定义结构体映射vertical-predefined-struct-map",title:"映射（Map） / 垂直预定义结构体映射（Vertical predefined-struct map） ",description:"映射使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Num map&lt;int32, .Item&gt; int32 Item&rsquo;s ID Item&rsquo;s num 1 100 2 200 3 300 生成结果：
hello_world.proto ItemConf.json `}).add({id:202,href:"/zh/docs/excel/map/#incell-map",title:"映射（Map） / 单元格内映射（Incell map） ",description:"映射使用指南。",content:` 单元格内映射有以下几种：
单元格内标量映射，映射值类型为标量。例如：map&lt;int32, int32&gt;。 单元格内结构体映射，映射值类型为结构体。例如：map&lt;int32, Item&gt;。 `}).add({id:203,href:"/zh/docs/excel/map/#单元格内标量映射incell-scalar-map",title:"映射（Map） / 单元格内标量映射（Incell scalar map） ",description:"映射使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:204,href:"/zh/docs/excel/map/#单元格内枚举映射incell-enum-map",title:"映射（Map） / 单元格内枚举映射（Incell enum map） ",description:"映射使用指南。",content:` 对于单元格内映射，键和值都可以是枚举类型。
...
hello_world.proto ItemConf.json `}).add({id:205,href:"/zh/docs/excel/map/#空键映射empty-key-map",title:"映射（Map） / 空键映射（Empty key map） ",description:"映射使用指南。",content:` 如果映射的键未配置，则视为键类型的默认值。默认值参见 标量类型。
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:206,href:"/zh/docs/excel/map/#枚举键映射enum-key-map",title:"映射（Map） / 枚举键映射（Enum key map） ",description:"映射使用指南。",content:` 根据 protobuf 文档对 映射键的类型 的限制：
...
hello_world.proto ItemConf.json `}).add({id:207,href:"/zh/docs/excel/map/#水平映射大小",title:"映射（Map） / 水平映射大小 ",description:"映射使用指南。",content:`
`}).add({id:208,href:"/zh/docs/excel/map/#动态大小",title:"映射（Map） / 动态大小 ",description:"映射使用指南。",content:` 默认情况下，所有映射都是动态大小类型。Map 条目应连续存在，否则如果中间存在空条目会报错。
`}).add({id:209,href:"/zh/docs/excel/map/#固定大小",title:"映射（Map） / 固定大小 ",description:"映射使用指南。",content:` 隐式固定大小 # Map 大小由名称行中最大存在的映射条目数量自动确定。
...
hello_world.proto ItemConf.json `}).add({id:210,href:"/zh/docs/excel/map/#高级特性",title:"映射（Map） / 高级特性 ",description:"映射使用指南。",content:`
`}).add({id:211,href:"/zh/docs/excel/map/#水平跳列映射horizontal-column-skipped-map",title:"映射（Map） / 水平跳列映射（Horizontal column-skipped map） ",description:"映射使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:212,href:"/zh/docs/excel/keyedlist/#语法",title:"键控列表（KeyedList） / 语法 ",description:"Excel keyed list 使用指南。",content:` 键控列表与普通列表相同，区别在于 ColumnType（第一个字段类型）被尖括号 &lt;&gt; 包围，并作为键处理。
语法：[ElemType]&lt;ColumnType&gt;
`}).add({id:213,href:"/zh/docs/excel/keyedlist/#水平键控列表horizontal-keyedlist",title:"键控列表（KeyedList） / 水平键控列表（Horizontal KeyedList） ",description:"Excel keyed list 使用指南。",content:` TODO&hellip;
`}).add({id:214,href:"/zh/docs/excel/keyedlist/#vertical-keyedlist",title:"键控列表（KeyedList） / 垂直键控列表（Vertical KeyedList） ",description:"Excel keyed list 使用指南。",content:`
`}).add({id:215,href:"/zh/docs/excel/keyedlist/#垂直标量键控列表vertical-scalar-keyedlist",title:"键控列表（KeyedList） / 垂直标量键控列表（Vertical scalar KeyedList） ",description:"Excel keyed list 使用指南。",content:` [!NOTE] 定义方式与 单元格内标量键控列表 相同，但如果提供了多行数据，会进行聚合。
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []&lt;uint32&gt; ID 1,2,3 4,5 6 生成结果：
hello_world.proto ItemConf.json `}).add({id:216,href:"/zh/docs/excel/keyedlist/#垂直枚举键控列表vertical-enum-keyedlist",title:"键控列表（KeyedList） / 垂直枚举键控列表（Vertical enum KeyedList） ",description:"Excel keyed list 使用指南。",content:` [!NOTE] 定义方式与 单元格内枚举键控列表 相同，但如果提供了多行数据，会进行聚合。
common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Type []&lt;enum&lt;.FruitType&gt;&gt; Type Apple,Orange FRUIT_TYPE_BANANA 0 生成结果：
hello_world.proto ItemConf.json `}).add({id:217,href:"/zh/docs/excel/keyedlist/#垂直结构体键控列表vertical-struct-keyedlist",title:"键控列表（KeyedList） / 垂直结构体键控列表（Vertical struct KeyedList） ",description:"Excel keyed list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:218,href:"/zh/docs/excel/keyedlist/#单元格内键控列表incell-keyedlist",title:"键控列表（KeyedList） / 单元格内键控列表（Incell KeyedList） ",description:"Excel keyed list 使用指南。",content:`
`}).add({id:219,href:"/zh/docs/excel/keyedlist/#单元格内标量键控列表",title:"键控列表（KeyedList） / 单元格内标量键控列表（Incell scalar KeyedList） ",description:"Excel keyed list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []&lt;uint32&gt; ID list 1,2,3 生成结果：
hello_world.proto ItemConf.json `}).add({id:220,href:"/zh/docs/excel/keyedlist/#单元格内枚举键控列表",title:"键控列表（KeyedList） / 单元格内枚举键控列表（Incell enum KeyedList） ",description:"Excel keyed list 使用指南。",content:` common.proto 中预定义的枚举类型 FruitType：
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Param []enum&lt;.FruitType&gt; Param list 1,FRUIT_TYPE_ORANGE,Banana Param 列的类型为 incell list []enum&lt;.FruitType&gt;，list 元素为预定义枚举类型 FruitType。
生成结果：
hello_world.proto ItemConf.json `}).add({id:221,href:"/zh/docs/excel/wellknown-types/#datetime",title:"知名类型（Wellknown types） / 日期时间（Datetime） ",description:"Wellknown types 使用指南。",content:`
`}).add({id:222,href:"/zh/docs/excel/wellknown-types/#datetime-1",title:"知名类型（Wellknown types） / 日期时间（Datetime） ",description:"Wellknown types 使用指南。",content:` [!NOTE] 参考 Basics: 日期时间
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:223,href:"/zh/docs/excel/wellknown-types/#date",title:"知名类型（Wellknown types） / 日期（Date） ",description:"Wellknown types 使用指南。",content:` [!NOTE] 参考 Basics: 日期时间
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:224,href:"/zh/docs/excel/wellknown-types/#time",title:"知名类型（Wellknown types） / 时间（Time） ",description:"Wellknown types 使用指南。",content:` [!NOTE] 参考 Basics: 日期时间
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:225,href:"/zh/docs/excel/wellknown-types/#duration",title:"知名类型（Wellknown types） / 时长（Duration） ",description:"Wellknown types 使用指南。",content:` [!NOTE] 参考 Basics: 时长
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:226,href:"/zh/docs/excel/wellknown-types/#fraction",title:"知名类型（Wellknown types） / 分数（Fraction） ",description:"Wellknown types 使用指南。",content:` [!NOTE] 参考 Basics: 分数
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:227,href:"/zh/docs/excel/wellknown-types/#comparator",title:"知名类型（Wellknown types） / 比较器（Comparator） ",description:"Wellknown types 使用指南。",content:` [!NOTE] 参考 Basics: 比较器
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:228,href:"/zh/docs/excel/wellknown-types/#version",title:"知名类型（Wellknown types） / 版本号（Version） ",description:"Wellknown types 使用指南。",content:` [!NOTE] 参考 Basics: 版本号
...
hello_world.proto ItemConf.json `}).add({id:229,href:"/zh/docs/excel/struct-in-struct/#结构体嵌套结构体struct-in-struct",title:"结构体嵌套结构体（Struct in struct） / 结构体嵌套结构体（Struct in struct） ",description:"Excel struct in struct 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:230,href:"/zh/docs/excel/struct-in-struct/#结构体嵌套预定义结构体predefined-struct-in-struct",title:"结构体嵌套结构体（Struct in struct） / 结构体嵌套预定义结构体（Predefined-struct in struct） ",description:"Excel struct in struct 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU RewardID RewardItemID RewardItemNum {Reward}int32 {.Item}int32 int32 Reward&rsquo;s ID Item&rsquo;s ID Item&rsquo;s num 1 1 10 生成结果：
hello_world.proto ItemConf.json `}).add({id:231,href:"/zh/docs/excel/struct-in-struct/#结构体嵌套单元格内结构体incell-struct-in-struct",title:"结构体嵌套结构体（Struct in struct） / 结构体嵌套单元格内结构体（Incell-struct in struct） ",description:"Excel struct in struct 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:232,href:"/zh/docs/excel/struct-in-list/#垂直列表中的嵌套",title:"列表嵌套结构体（Struct in list） / 垂直列表中的嵌套 ",description:"Excel struct in list 使用指南。",content:`
`}).add({id:233,href:"/zh/docs/excel/struct-in-list/#垂直列表嵌套结构体struct-in-vertical-list",title:"列表嵌套结构体（Struct in list） / 垂直列表嵌套结构体（Struct in vertical-list） ",description:"Excel struct in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:234,href:"/zh/docs/excel/struct-in-list/#垂直列表嵌套单元格内结构体incell-struct-in-vertical-list",title:"列表嵌套结构体（Struct in list） / 垂直列表嵌套单元格内结构体（Incell-struct in vertical-list） ",description:"Excel struct in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:235,href:"/zh/docs/excel/struct-in-list/#水平列表的第一个字段",title:"列表嵌套结构体（Struct in list） / 水平列表的第一个字段 ",description:"Excel struct in list 使用指南。",content:`
`}).add({id:236,href:"/zh/docs/excel/struct-in-list/#水平列表嵌套结构体struct-in-horizontal-list",title:"列表嵌套结构体（Struct in list） / 水平列表嵌套结构体（Struct in horizontal-list） ",description:"Excel struct in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:237,href:"/zh/docs/excel/struct-in-list/#水平列表嵌套预定义结构体predefined-struct-in-horizontal-list",title:"列表嵌套结构体（Struct in list） / 水平列表嵌套预定义结构体（Predefined-struct in horizontal-list） ",description:"Excel struct in list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Reward1ItemID Reward1ItemNum Reward1Name Reward2ItemID Reward2ItemNum Reward2Name [Reward]{.Item}int32 int32 string int32 int32 string Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name 1 10 Lotto 10 100 Super Lotto 生成结果：
hello_world.proto ItemConf.json `}).add({id:238,href:"/zh/docs/excel/struct-in-list/#水平列表嵌套单元格内结构体incell-struct-in-horizontal-list",title:"列表嵌套结构体（Struct in list） / 水平列表嵌套单元格内结构体（Incell-struct in horizontal-list） ",description:"Excel struct in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
ItemConf.json `}).add({id:239,href:"/zh/docs/excel/struct-in-map/#垂直映射中的嵌套",title:"映射嵌套结构体（Struct in map） / 垂直映射中的嵌套 ",description:"Excel struct in map 使用指南。",content:`
`}).add({id:240,href:"/zh/docs/excel/struct-in-map/#垂直映射嵌套结构体struct-in-vertical-map",title:"映射嵌套结构体（Struct in map） / 垂直映射嵌套结构体（Struct in vertical-map） ",description:"Excel struct in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:241,href:"/zh/docs/excel/struct-in-map/#垂直映射嵌套预定义结构体predefined-struct-in-vertical-map",title:"映射嵌套结构体（Struct in map） / 垂直映射嵌套预定义结构体（Predefined-struct in vertical-map） ",description:"Excel struct in map 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID ItemID ItemNum map&lt;int32, Reward&gt; {.Item}int32 int32 Reward&rsquo;s ID Item&rsquo;s ID Item&rsquo;s Num 1 1 10 2 2 20 3 生成结果：
hello_world.proto ItemConf.json `}).add({id:242,href:"/zh/docs/excel/struct-in-map/#垂直映射嵌套单元格内结构体incell-struct-in-vertical-map",title:"映射嵌套结构体（Struct in map） / 垂直映射嵌套单元格内结构体（Incell-struct in vertical-map） ",description:"Excel struct in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:243,href:"/zh/docs/excel/list-in-list/#垂直列表中的嵌套",title:"列表嵌套列表（List in list） / 垂直列表中的嵌套 ",description:"Excel list in list 使用指南。",content:`
`}).add({id:244,href:"/zh/docs/excel/list-in-list/#垂直列表嵌套水平列表horizontal-list-in-vertical-list",title:"列表嵌套列表（List in list） / 垂直列表嵌套水平列表（Horizontal-list in vertical-list） ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:245,href:"/zh/docs/excel/list-in-list/#垂直键控列表嵌套垂直列表vertical-list-in-vertical-keyed-list",title:"列表嵌套列表（List in list） / 垂直键控列表嵌套垂直列表（Vertical-list in vertical-keyed-list） ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:246,href:"/zh/docs/excel/list-in-list/#垂直键控列表嵌套垂直键控列表vertical-keyed-list-in-vertical-keyed-list",title:"列表嵌套列表（List in list） / 垂直键控列表嵌套垂直键控列表（Vertical-keyed-list in vertical-keyed-list） ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:247,href:"/zh/docs/excel/list-in-list/#垂直键控列表嵌套单元格内列表incell-list-in-vertical-keyed-list",title:"列表嵌套列表（List in list） / 垂直键控列表嵌套单元格内列表（Incell-list in vertical-keyed-list） ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:248,href:"/zh/docs/excel/list-in-list/#垂直键控列表嵌套单元格内键控列表incell-keyed-list-in-vertical-keyed-list",title:"列表嵌套列表（List in list） / 垂直键控列表嵌套单元格内键控列表（Incell-keyed-list in vertical-keyed-list） ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:249,href:"/zh/docs/excel/list-in-list/#水平列表中的嵌套",title:"列表嵌套列表（List in list） / 水平列表中的嵌套 ",description:"Excel list in list 使用指南。",content:`
`}).add({id:250,href:"/zh/docs/excel/list-in-list/#水平列表嵌套水平列表horizontal-list-in-horizontal-list",title:"列表嵌套列表（List in list） / 水平列表嵌套水平列表（Horizontal-list in horizontal-list） ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:251,href:"/zh/docs/excel/list-in-list/#水平列表嵌套预定义结构体列表predefined-struct-list-in-horizontal-list",title:"列表嵌套列表（List in list） / 水平列表嵌套预定义结构体列表（Predefined-struct-list in horizontal-list） ",description:"Excel list in list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Reward1Item1ID Reward1Item1Num Reward1Item2ID Reward1Item2Num Reward1Name Reward2Item1ID Reward2Item1Num Reward2Name [Reward][.Item]int32 int32 int32 int32 string int32 int32 string Item1&rsquo;s ID Item1&rsquo;s num Item2&rsquo;s ID Item2&rsquo;s num Reward&rsquo;s name Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name 1 10 2 20 Lotto 10 100 Super Lotto 生成结果：
hello_world.proto ItemConf.json `}).add({id:252,href:"/zh/docs/excel/list-in-list/#水平列表嵌套单元格内列表incell-list-in-horizontal-list",title:"列表嵌套列表（List in list） / 水平列表嵌套单元格内列表（Incell-list in horizontal-list） ",description:"Excel list in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:253,href:"/zh/docs/excel/list-in-map/#垂直映射中的嵌套",title:"映射嵌套列表（List in map） / 垂直映射中的嵌套 ",description:"Excel list in map 使用指南。",content:`
`}).add({id:254,href:"/zh/docs/excel/list-in-map/#垂直映射嵌套水平列表horizontal-list-in-vertical-map",title:"映射嵌套列表（List in map） / 垂直映射嵌套水平列表（Horizontal-list in vertical-map） ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:255,href:"/zh/docs/excel/list-in-map/#horizontal-aggregate-list-in-vertical-map",title:"映射嵌套列表（List in map） / 垂直映射嵌套水平聚合列表（Horizontal-aggregate-list in vertical-map） ",description:"Excel list in map 使用指南。",content:` [!NOTE] 当键相同的多行需要把水平展开的键控列表元素跨行追加到同一个父记录时， 在水平列表字段上设置 prop:{aggregate:true}， 父映射的键字段同时设置 prop:{unique:false} 允许重复键。
重复键控列表素会触发 E2028。
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:256,href:"/zh/docs/excel/list-in-map/#垂直映射嵌套垂直列表vertical-list-in-vertical-map",title:"映射嵌套列表（List in map） / 垂直映射嵌套垂直列表（Vertical-list in vertical-map） ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:257,href:"/zh/docs/excel/list-in-map/#垂直映射嵌套单元格内列表incell-list-in-vertical-map",title:"映射嵌套列表（List in map） / 垂直映射嵌套单元格内列表（Incell-list in vertical-map） ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:258,href:"/zh/docs/excel/list-in-map/#垂直映射嵌套单元格内结构体列表incell-struct-list-in-vertical-map",title:"映射嵌套列表（List in map） / 垂直映射嵌套单元格内结构体列表（Incell-struct-list in vertical-map） ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:259,href:"/zh/docs/excel/list-in-map/#水平映射中的嵌套",title:"映射嵌套列表（List in map） / 水平映射中的嵌套 ",description:"Excel list in map 使用指南。",content:`
`}).add({id:260,href:"/zh/docs/excel/list-in-map/#水平映射嵌套水平列表horizontal-list-in-horizontal-map",title:"映射嵌套列表（List in map） / 水平映射嵌套水平列表（Horizontal-list in horizontal-map） ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:261,href:"/zh/docs/excel/list-in-map/#水平映射嵌套单元格内列表incell-list-in-horizontal-map",title:"映射嵌套列表（List in map） / 水平映射嵌套单元格内列表（Incell-list in horizontal-map） ",description:"Excel list in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
生成结果：
hello_world.proto ItemConf.json `}).add({id:262,href:"/zh/docs/excel/map-in-list/#垂直列表中的嵌套",title:"列表嵌套映射（Map in list） / 垂直列表中的嵌套 ",description:"Excel map in list 使用指南。",content:`
`}).add({id:263,href:"/zh/docs/excel/map-in-list/#垂直列表嵌套水平映射horizontal-map-in-vertical-list",title:"列表嵌套映射（Map in list） / 垂直列表嵌套水平映射（Horizontal-map in vertical-list） ",description:"Excel map in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:264,href:"/zh/docs/excel/map-in-list/#垂直键控列表嵌套垂直映射vertical-map-in-vertical-keyed-list",title:"列表嵌套映射（Map in list） / 垂直键控列表嵌套垂直映射（Vertical-map in vertical-keyed-list） ",description:"Excel map in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:265,href:"/zh/docs/excel/map-in-list/#垂直列表嵌套单元格内映射incell-map-in-vertical-list",title:"列表嵌套映射（Map in list） / 垂直列表嵌套单元格内映射（Incell-map in vertical-list） ",description:"Excel map in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:266,href:"/zh/docs/excel/map-in-list/#水平列表的第一个字段",title:"列表嵌套映射（Map in list） / 水平列表的第一个字段 ",description:"Excel map in list 使用指南。",content:`
`}).add({id:267,href:"/zh/docs/excel/map-in-list/#水平列表嵌套水平映射horizontal-map-in-horizontal-list",title:"列表嵌套映射（Map in list） / 水平列表嵌套水平映射（Horizontal-map in horizontal-list） ",description:"Excel map in list 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:268,href:"/zh/docs/excel/map-in-list/#水平列表嵌套预定义结构体映射predefined-struct-map-in-horizontal-list",title:"列表嵌套映射（Map in list） / 水平列表嵌套预定义结构体映射（Predefined-struct-map in horizontal-list） ",description:"Excel map in list 使用指南。",content:` common.proto 中预定义的 Item：
HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU Reward1Item1ID Reward1Item1Num Reward1Item2ID Reward1Item2Num Reward1Name Reward2Item1ID Reward2Item1Num Reward2Name [Reward]map&lt;int32, .Item&gt; int32 int32 int32 string int32 int32 string Item1&rsquo;s ID Item1&rsquo;s num Item2&rsquo;s ID Item2&rsquo;s num Reward&rsquo;s name Item1&rsquo;s ID Item1&rsquo;s num Reward&rsquo;s name 1 10 2 20 Lotto 10 100 Super Lotto 生成结果：
hello_world.proto ItemConf.json `}).add({id:269,href:"/zh/docs/excel/map-in-map/#垂直映射中的嵌套",title:"映射嵌套映射（Map in map） / 垂直映射中的嵌套 ",description:"Excel map in map 使用指南。",content:`
`}).add({id:270,href:"/zh/docs/excel/map-in-map/#垂直映射嵌套水平映射horizontal-map-in-vertical-map",title:"映射嵌套映射（Map in map） / 垂直映射嵌套水平映射（Horizontal-map in vertical-map） ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:271,href:"/zh/docs/excel/map-in-map/#horizontal-aggregate-map-in-vertical-map",title:"映射嵌套映射（Map in map） / 垂直映射嵌套水平聚合映射（Horizontal-aggregate-map in vertical-map） ",description:"Excel map in map 使用指南。",content:` [!NOTE] 当键相同的多行需要把水平展开的子映射元素跨行合并到同一个父记录时， 在水平映射字段上设置 prop:{aggregate:true}， 父映射的键字段同时设置 prop:{unique:false} 允许重复键。
子映射的键跨行重复会触发 E2005。
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:272,href:"/zh/docs/excel/map-in-map/#垂直映射嵌套垂直映射vertical-map-in-vertical-map",title:"映射嵌套映射（Map in map） / 垂直映射嵌套垂直映射（Vertical-map in vertical-map） ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:273,href:"/zh/docs/excel/map-in-map/#垂直映射嵌套单元格内映射incell-map-in-vertical-map",title:"映射嵌套映射（Map in map） / 垂直映射嵌套单元格内映射（Incell-map in vertical-map） ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:274,href:"/zh/docs/excel/map-in-map/#水平映射中的嵌套",title:"映射嵌套映射（Map in map） / 水平映射中的嵌套 ",description:"Excel map in map 使用指南。",content:`
`}).add({id:275,href:"/zh/docs/excel/map-in-map/#水平映射嵌套水平映射horizontal-map-in-horizontal-map",title:"映射嵌套映射（Map in map） / 水平映射嵌套水平映射（Horizontal-map in horizontal-map） ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:276,href:"/zh/docs/excel/map-in-map/#水平映射嵌套单元格内映射incell-map-in-horizontal-map",title:"映射嵌套映射（Map in map） / 水平映射嵌套单元格内映射（Incell-map in horizontal-map） ",description:"Excel map in map 使用指南。",content:` HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto ItemConf.json `}).add({id:277,href:"/zh/docs/excel/infinite-nesting/#概述",title:"无限嵌套 / 概述 ",description:"Excel 无限嵌套使用指南。",content:` 现在，水平/垂直列表元素的第一个字段可以是任意类型，包括结构体、列表和映射。
...
`}).add({id:278,href:"/zh/docs/excel/infinite-nesting/#嵌套命名",title:"无限嵌套 / 嵌套命名 ",description:"Excel 无限嵌套使用指南。",content:` common.proto 中的预定义类型：
...
hello_world.proto loader_conf.json `}).add({id:279,href:"/zh/docs/excel/field-property/#概览",title:"字段属性（Field property） / 概览 ",description:"Tableau field property 使用指南。",content:` 选项 类型 说明 unique bool 检查字段唯一性。
...
默认值：false。 `}).add({id:280,href:"/zh/docs/excel/field-property/#选项-unique",title:"字段属性（Field property） / 选项 unique ",description:"Tableau field property 使用指南。",content:` 选项 unique 可在字段属性中指定为 true 或 false，用于检查列表/映射元素中任意标量字段的唯一性。
如果显式设置 unique 为 true，当出现重复值时 tableau 会报错。 如果显式设置 unique 为 false，则不进行检查。 `}).add({id:281,href:"/zh/docs/excel/field-property/#映射键控列表的键",title:"字段属性（Field property） / 映射/键控列表的键 ",description:"Tableau field property 使用指南。",content:` Tableau 会自动推断映射（或键控列表）键的 unique 是否为 true。
规则是：如果映射的值类型（或键控列表元素类型）没有相同布局（垂直/水平）的子映射/列表字段，则键必须唯一。
因此大多数情况下不需要显式配置。
`}).add({id:282,href:"/zh/docs/excel/field-property/#通用标量字段",title:"字段属性（Field property） / 通用标量字段 ",description:"Tableau field property 使用指南。",content:` 如果将通用标量字段的属性 unique 指定为 true，则 tableau 会检查该字段在映射或列表中的唯一性。
`}).add({id:283,href:"/zh/docs/excel/field-property/#选项-range",title:"字段属性（Field property） / 选项 range ",description:"Tableau field property 使用指南。",content:` [!WARNING] 当单元格数据为空（未填写）时，此检查将被跳过。若需对空单元格也强制执行检查，请将选项 present 设置为 true。
选项 range 的格式为：&quot;left,right&quot;（左右均为闭区间）。
range 的不同含义：
数字：值范围，例如：&quot;1,10&quot;、&quot;1,~&quot;、&quot;~,10&quot;。 `}).add({id:284,href:"/zh/docs/excel/field-property/#选项-refer",title:"字段属性（Field property） / 选项 refer ",description:"Tableau field property 使用指南。",content:` 选项 refer 类似于 SQL 中的外键（FOREIGN KEY）约束，用于防止破坏表之间关联的操作。但 tableau 的 refer 可以引用任意工作表的列（不限于 map key 列），并且支持多个 refer（逗号分隔）。它用于确保该字段的值至少在其他某个工作表的列值空间（即 message 的字段值空间）中存在。
格式：&quot;SheetName(SheetAlias).ColumnName[,SheetName(SheetAlias).ColumnName]...&quot;。
示例：
map&lt;uint32, Reward&gt;|{refer:&quot;ItemConf.ID&quot;}：无别名的单 refer，工作表名即为生成的 protobuf message 名。 map&lt;uint32, Reward&gt;|{refer:&quot;ItemConf.ID,EquipConf.ID&quot;}：无别名的多 refer，工作表别名即为生成的 protobuf message 名。 map&lt;uint32, Reward&gt;|{refer:&quot;Sheet1(ItemConf).ID&quot;}：有别名的单 refer，工作表别名即为生成的 protobuf message 名。 `}).add({id:285,href:"/zh/docs/excel/field-property/#选项-sequence",title:"字段属性（Field property） / 选项 sequence ",description:"Tableau field property 使用指南。",content:` 选项 sequence 用于确保该字段的值是一个从指定值开始的序列，可用于任意字段，包括嵌套列表/映射中的字段。
示例：
map&lt;uint32, Item&gt;|{sequence:1}：该映射键必须遵循从值 1 开始的序列规则。 int32|{sequence:1}：父级列表/映射的元素必须遵循从值 1 开始的序列规则。 `}).add({id:286,href:"/zh/docs/excel/field-property/#选项-default",title:"字段属性（Field property） / 选项 default ",description:"Tableau field property 使用指南。",content:` 如果设置了选项 default，则当单元格为空时使用该值作为默认值。
`}).add({id:287,href:"/zh/docs/excel/field-property/#选项-fixed",title:"字段属性（Field property） / 选项 fixed ",description:"Tableau field property 使用指南。",content:` 如果将选项 fixed 设置为 true，则自动检测水平列表/映射的固定大小。
示例：
列表: 隐式固定大小 映射: 隐式固定大小 `}).add({id:288,href:"/zh/docs/excel/field-property/#选项-size",title:"字段属性（Field property） / 选项 size ",description:"Tableau field property 使用指南。",content:` 选项 size 用于指定水平列表/映射的固定大小。
示例：
列表: 显式固定大小 映射: 显式固定大小 `}).add({id:289,href:"/zh/docs/excel/field-property/#选项-form",title:"字段属性（Field property） / 选项 form ",description:"Tableau field property 使用指南。",content:` 选项 form 用于指定 incell struct 的单元格数据格式。
可指定两种格式：
FORM_TEXT：protobuf text format。 FORM_JSON：protobuf JSON format。 详细示例请参考 高级预定义单元格内结构体。
`}).add({id:290,href:"/zh/docs/excel/field-property/#选项-json_name",title:"字段属性（Field property） / 选项 json_name ",description:"Tableau field property 使用指南。",content:` 默认情况下，JSON 名称由字段的 proto 名称转换为 camelCase 得到。现在可以通过 json_name 选项显式指定。
例如，HelloWorld.xlsx 中的工作表 ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Rarity_1 SpecialEffect_2 map&lt;int32, Item&gt; int32|{json_name:&ldquo;rarity_1&rdquo;} int32|{json_name:&ldquo;specialEffect_2&rdquo;} Item&rsquo;s ID Item&rsquo;s rarity. Item&rsquo;s special effect. 1 10 101 2 20 102 3 30 103 `}).add({id:291,href:"/zh/docs/excel/field-property/#选项-present",title:"字段属性（Field property） / 选项 present ",description:"Tableau field property 使用指南。",content:` 如果将选项 present 设置为 true，则单元格数据不能为空，必须显式填写，否则会报错。
`}).add({id:292,href:"/zh/docs/excel/field-property/#选项-optional",title:"字段属性（Field property） / 选项 optional ",description:"Tableau field property 使用指南。",content:` 指定该字段是否为可选（字段名可缺失）。
如果设置为 true，则：
表格格式（Excel/CSV）：字段的列可以缺失。 文档格式（XML/YAML）：字段名可以缺失。 `}).add({id:293,href:"/zh/docs/excel/field-property/#选项-patch",title:"字段属性（Field property） / 选项 patch ",description:"Tableau field property 使用指南。",content:` 参见 选项 Patch 中的字段级 patch。
`}).add({id:294,href:"/zh/docs/excel/field-property/#选项-sep",title:"字段属性（Field property） / 选项 sep ",description:"Tableau field property 使用指南。",content:` 字段级分隔符，用于分隔：
单元格内映射的键值对。 单元格内列表的元素（标量或结构体）。 如果未设置，将使用 元表 中的工作表级 sep。
`}).add({id:295,href:"/zh/docs/excel/field-property/#选项-subsep",title:"字段属性（Field property） / 选项 subsep ",description:"Tableau field property 使用指南。",content:` 字段级子分隔符，用于分隔：
每个单元格内映射的键和值。 每个单元格内结构体列表元素的结构体各字段。 如果未设置，将使用 元表 中的工作表级 subsep。
`}).add({id:296,href:"/zh/docs/excel/field-property/#选项-pattern",title:"字段属性（Field property） / 选项 pattern ",description:"Tableau field property 使用指南。",content:` 指定标量字段、列表元素和映射值的模式。
`}).add({id:297,href:"/zh/docs/excel/field-property/#版本号字段",title:"字段属性（Field property） / 版本号字段 ",description:"Tableau field property 使用指南。",content:` [!NOTE] 使用案例请参考 知名类型: 版本号
指定当前单元格的点分十进制模式。每个十进制数的范围从 0 到模式对应部分的最大值（MAX）。
默认模式：255.255.255。
`}).add({id:298,href:"/zh/docs/excel/field-property/#选项-validate",title:"字段属性（Field property） / 选项 validate ",description:"Tableau field property 使用指南。",content:` Tableau 集成了 protovalidate， 可以直接在表格字段属性中声明校验规则。这些规则会被编译为生成 .proto 文件中 字段上的 (buf.validate.field) 选项，并在 tableau 生成配置时强制执行。支持 CEL 表达式以及 protovalidate 提供 的标准规则、predefined 规则和自定义（custom）规则。
...
hello_world.proto `}).add({id:299,href:"/zh/docs/excel/field-property/#选项-validate_complex",title:"字段属性（Field property） / 选项 validate_complex ",description:"Tableau field property 使用指南。",content:` 选项 validate_complex 用于指定复合类型（即 列表/映射）的字段级校验 规则——即对容器本身（而非其元素）施加的规则。其值为 buf.validate.FieldRules （repeated 或 map 字段已设置）的 text format 表示。
示例：
...
`}).add({id:300,href:"/zh/docs/excel/field-property/#选项-validate_message",title:"字段属性（Field property） / 选项 validate_message ",description:"Tableau field property 使用指南。",content:` 选项 validate_message 用于指定字段所嵌套 message 的 message 级校验规则。 通常在字段的值类型为子 message（例如结构体、列表元素或映射值的类型）时使用， 用于通过 CEL 表达式对 message 的多个字段进行交叉校验。其值为 buf.validate.MessageRules 的 text format 表示。
示例：
...
`}).add({id:301,href:"/zh/docs/excel/field-property/#选项-aggregate",title:"字段属性（Field property） / 选项 aggregate ",description:"Tableau field property 使用指南。",content:` 选项 aggregate 作用于单元格内（layout:LAYOUT_INCELL）或水平 （layout:LAYOUT_HORIZONTAL）的列表/映射字段。当同一父记录被多行/多列重复 访问时（同一映射键跨行、同一垂直/水平列表跨多行/多列等），aggregate:true 会把每次访问产生的元素聚合到父记录的同一个集合中，而不要求所有访问填写 完全相同的值。
...
`}).add({id:302,href:"/zh/docs/excel/field-property/#适用场景",title:"字段属性（Field property） / 适用场景 ",description:"Tableau field property 使用指南。",content:" 父级布局 aggregate 作用对象 重复访问场景 垂直映射/垂直键控列表（多行同键） 单元格内列表/映射（LAYOUT_INCELL）、水平列表/映射（LAYOUT_HORIZONTAL） 多行落到同一父记录 水平映射（多列同键） 单元格内列表/映射、水平列表/映射 多列落到同一父记录 垂直/水平列表自身跨多行/多列 兄弟单元格内字段在每行/列被重复访问 多行/多列落到同一父记录 "}).add({id:303,href:"/zh/docs/excel/field-property/#各类型聚合行为",title:"字段属性（Field property） / 各类型聚合行为 ",description:"Tableau field property 使用指南。",content:" 子字段类型 aggregate:true 行为 错误码 单元格内列表 各次访问的元素追加到同一列表 — 单元格内映射 各次访问的键值合并到同一映射；键重复报错 E2005 单元格内键控列表（[]&lt;T&gt;） 各次访问的元素追加并按键去重；元素重复报错 E2028 水平列表 跨行的水平列表元素追加到同一列表 — 水平键控列表 跨行的元素追加并按键去重 E2028 水平映射 跨行的键值合并到同一映射；键重复报错 E2005 "}).add({id:304,href:"/zh/docs/excel/field-property/#垂直聚合",title:"字段属性（Field property） / 垂直聚合 ",description:"Tableau field property 使用指南。",content:` 端到端示例参见 垂直列表（Vertical list） 中的对应小节：
垂直标量列表 —— 跨行聚合的 incell 标量列表。 垂直枚举列表 —— 跨行聚合的 incell 枚举列表。 垂直单元格内结构体列表 —— 跨行聚合的 incell 内联结构体列表。 垂直单元格内预定义结构体列表 —— 跨行聚合的 incell 预定义结构体列表。 `}).add({id:305,href:"/zh/docs/excel/field-property/#水平聚合",title:"字段属性（Field property） / 水平聚合 ",description:"Tableau field property 使用指南。",content:` 端到端示例参见映射嵌套列表/映射嵌套映射中的水平聚合用例：
垂直映射嵌套水平聚合列表 —— 跨行聚合的水平 list（LAYOUT_HORIZONTAL）。 垂直映射嵌套水平聚合映射 —— 跨行聚合的水平 map（LAYOUT_HORIZONTAL）。 `}).add({id:306,href:"/zh/docs/excel/field-property/#aggregate-与-key-组合keyed-list-去重",title:"字段属性（Field property） / aggregate 与 key 组合（keyed-list 去重） ",description:"Tableau field property 使用指南。",content:` 当 aggregate:true 与键控列表（[]&lt;T&gt; 语法，会自动设置字段的 key）一同使用时， tableau 在聚合过程中还会对元素去重：
标量/枚举键控列表：元素本身就是键——跨行/跨列重复的元素会触发 E2028。 结构体键控列表：仅比较通过 key 配置的子字段。 端到端示例参见 垂直键控列表 （垂直聚合）以及 垂直映射嵌套水平聚合列表（水平聚合）。
`}).add({id:307,href:"/zh/docs/excel/field-property/#注意",title:"字段属性（Field property） / 注意 ",description:"Tableau field property 使用指南。",content:" aggregate 仅在单元格内（layout:LAYOUT_INCELL）或水平 （layout:LAYOUT_HORIZONTAL）的列表/映射字段上生效。 父字段键跨行重复时，需要在父字段上同时设置 prop:{unique:false}， 否则会被 E2005 拦截。 聚合后，range、refer、unique 等针对单元素的字段属性仍会逐元素生效。 "}).add({id:308,href:"/zh/docs/excel/metasheet/#概述",title:"元表（Metasheet） / 概述 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 以下选项可在元表 @TABLEAU 中指定，用于影响对应工作表的布局、功能、loader 等。
...
示例：
- OrderedMap:cpp,Index:cpp go // ordered map 仅支持 cpp，index 支持 cpp 和 go - OrderedMap:cpp // ordered map 仅支持 cpp，index 支持所有语言 Validate string 工作表/messager 级别的 protovalidate message 级校验规则。其值为 buf.validate.MessageRules 的 text format 表示，例如：cel_expression:&quot;this.item_map.size() &gt; 0&quot;。也支持在 MODE_STRUCT_TYPE/MODE_UNION_TYPE 等工作表上使用。 `}).add({id:309,href:"/zh/docs/excel/metasheet/#空-tableau",title:"元表（Metasheet） / 空 @TABLEAU ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 如果元表 @TABLEAU 为空，则同一工作簿中的所有其他工作表都会被处理。
`}).add({id:310,href:"/zh/docs/excel/metasheet/#简单示例",title:"元表（Metasheet） / 简单示例 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` HelloWorld.xlsx 中有一个工作表 Sheet1，我们希望将其重命名为 ItemConf，定义自定义分隔符为 |，并生成 ordered map 访问器。
因此，HelloWorld.xlsx 中的元表 @TABLEAU 应配置如下：
HelloWorld.xlsx&nbsp; Sheet1 @TABLEAU ID Name map&lt;uint32, Item&gt; string Item&rsquo;s ID Item&rsquo;s Name 1 Apple 2 Orange 3 Banana Sheet Alias Sep OrderedMap Sheet1 ItemConf | true `}).add({id:311,href:"/zh/docs/excel/metasheet/#选项-alias",title:"元表（Metasheet） / 选项 Alias ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 生成的 proto 文件名是输入文件名的 snake_case 形式。例如，如果工作簿名为 HelloWorld.xlsx，则生成的 proto 文件名为 hello_world.proto。如果希望手动指定生成的 proto 文件名，可以使用 Alias 选项。在此场景中，# 表示工作簿名称。
HelloWorld.xlsx 中的工作表 ItemConf：
...
hello_world.proto HeroConf.json `}).add({id:314,href:"/zh/docs/excel/metasheet/#选项-merger",title:"元表（Metasheet） / 选项 Merger ",description:"Excel 元表 @TABLEAU 使用指南。",content:` Merger 选项用于将多个具有相同 schema 的工作表（逗号分隔）合并为一个。
每个元素可以是：
...
`}).add({id:315,href:"/zh/docs/excel/metasheet/#合并多个工作簿",title:"元表（Metasheet） / 合并多个工作簿 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 例如，有三个工作簿，每个都包含一个具有相同 schema 的工作表 ZoneConf：
...
merger_main.proto ZoneConf.json `}).add({id:316,href:"/zh/docs/excel/metasheet/#合并同一工作簿中的多个工作表",title:"元表（Metasheet） / 合并同一工作簿中的多个工作表 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 例如，同一工作簿 Merger.xlsx 中有三个具有相同 schema 的工作表：
...
生成结果：
merger_same.proto ZoneConf.json `}).add({id:317,href:"/zh/docs/excel/metasheet/#选项-scatter",title:"元表（Metasheet） / 选项 Scatter ",description:"Excel 元表 @TABLEAU 使用指南。",content:` Scatter 选项用于将多个具有相同 schema 的工作表（逗号分隔）分别转换为不同的配置文件。
每个元素可以是：
仅工作簿文件路径或 Glob 路径（相对于当前工作簿）：&lt;Workbook&gt;，此时工作表名称与当前工作表相同。 工作簿文件路径（相对于当前工作簿）加工作表名称：&lt;Workbook&gt;#&lt;Worksheet&gt;。 [!NOTE] Glob 模式通常不应匹配主工作簿。如果匹配，tableauc 会自动排除它。
...
Scatter1_ZoneConf.json Scatter2_ZoneConf.json Scatter3_ZoneConf.json `}).add({id:318,href:"/zh/docs/excel/metasheet/#选项-orderedmap",title:"元表（Metasheet） / 选项 OrderedMap ",description:"Excel 元表 @TABLEAU 使用指南。",content:` [!IMPORTANT] 仅适用于每个层级 message 的第一个映射字段。
...
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Name PropID PropValue map&lt;uint32, Item&gt; string map&lt;int32, Prop&gt; int64 Item&rsquo;s ID Item&rsquo;s name Prop&rsquo;s ID Prop&rsquo;s value 1 Apple 1 10 2 Orange 1 20 2 Orange 2 30 Sheet OrderedMap ItemConf true `}).add({id:319,href:"/zh/docs/excel/metasheet/#选项-index",title:"元表（Metasheet） / 选项 Index ",description:"Excel 元表 @TABLEAU 使用指南。",content:` [!IMPORTANT] 被索引的列必须是同一结构体（message）内的标量或枚举。
...
ID&lt;Name&gt;@Item：对 ID 列建立单列有序索引，结果数组按 Name 排序。 HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID Name Desc map&lt;int32, Item&gt; string string Item&rsquo;s ID Item&rsquo;s name Item&rsquo;s desc 1 Apple A kind of delicious fruit. 2 Orange A kind of sour fruit. 3 Banana A kind of calorie-rich fruit. Sheet OrderedIndex ItemConf ID@Item `}).add({id:324,href:"/zh/docs/excel/metasheet/#多列有序索引",title:"元表（Metasheet） / 多列有序索引 ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 格式：(Column1,Column2,...)&lt;ColumnX,ColumnY,...&gt;@IndexName。
...
(ID,Type)&lt;Type&gt;@Shop：对 (ID, Type) 列建立多列有序索引，结果数组按 Type 排序。 (ID,Type)&lt;Type,ID&gt;@SpecialShop：对 (ID, Type) 列建立多列有序索引，结果数组按 Type 然后 ID 排序。 HelloWorld.xlsx&nbsp; ShopConf @TABLEAU ID Type Desc [Shop]int32 int32 string Shop&rsquo;s ID Shop&rsquo;s type Shop&rsquo;s desc 1 1 Shoes shop. 2 1 T-Shirt shop. 3 2 Fruite shop. Sheet OrderedIndex ShopConf (ID,Type)@Shop, (ID,Type)&lt;Type,ID&gt;@SpecialShop `}).add({id:325,href:"/zh/docs/excel/metasheet/#选项-patch",title:"元表（Metasheet） / 选项 Patch ",description:"Excel 元表 @TABLEAU 使用指南。",content:`

`}).add({id:326,href:"/zh/docs/excel/metasheet/#选项-sep",title:"元表（Metasheet） / 选项 Sep ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 工作表的分隔符，用于分隔：
单元格内列表元素（标量或结构体）。 单元格内映射元素。 如果未设置，将使用 Tableauc 配置 中的全局级别分隔符（默认：,）。
`}).add({id:327,href:"/zh/docs/excel/metasheet/#选项-subsep",title:"元表（Metasheet） / 选项 Subsep ",description:"Excel 元表 @TABLEAU 使用指南。",content:` 工作表的子分隔符，用于分隔：
每个单元格内映射元素的键值对。 每个单元格内结构体列表元素的结构体字段。 如果未设置，将使用 Tableauc 配置 中的全局级别子分隔符（默认：:）。
`}).add({id:328,href:"/zh/docs/excel/metasheet/#选项-validate",title:"元表（Metasheet） / 选项 Validate ",description:"Excel 元表 @TABLEAU 使用指南。",content:` Tableau 集成了 protovalidate， 可以直接在表格中声明校验规则。选项 Validate 是字段级 validate_message 选项在工作表/messager 级别的对应版本。它会被编译为生成工作表对应 proto message 上的 (buf.validate.message) 选项，并在 tableau 生成配置时强制执行。
...
hello_world.proto `}).add({id:329,href:"/zh/docs/basics/",title:"基础",description:"Tableau 基础知识。",content:""}).add({id:330,href:"/zh/docs/basics/concepts/#术语",title:"核心概念 / 术语 ",description:"Tableau 核心概念。",content:`
`}).add({id:331,href:"/zh/docs/basics/concepts/#基础术语",title:"核心概念 / 基础术语 ",description:"Tableau 核心概念。",content:" 术语 定义 Workbook 一个 Excel 文件；或一组以相同前缀命名、用 # 分隔的 CSV 文件；或一个 XML 文件；或一个 YAML 文件。 Worksheet Excel 文件中的一个工作表；或一个 CSV 文件；或 XML 文件的根节点；或 YAML 文件中的一个文档。 Metasheet 名为 @TABLEAU 的特殊工作表，用于指定 tableau 解析器选项。 Row 工作表中的一行。 Column 工作表中的一列。 Cell 行与列的交叉点（单元格）。 In-cell 单元格内部。 Cross-cell 一行或一列中连续的多个单元格。 "}).add({id:332,href:"/zh/docs/basics/concepts/#工作表相关术语",title:"核心概念 / 工作表相关术语 ",description:"Tableau 核心概念。",content:` 术语 定义 Namerow 工作表中列名定义所在的行号。
⚠️ 注意：同一工作表中每个列名必须唯一！
...
默认值：false。 Layout Incell（单元格内）、vertical（跨单元格垂直）或 horizontal（跨单元格水平）。 Transpose 对指定工作表进行行列转置。 `}).add({id:333,href:"/zh/docs/basics/concepts/#与-protoconf-的映射关系",title:"核心概念 / 与 Protoconf 的映射关系 ",description:"Tableau 核心概念。",content:" 术语 Protoconf Workbook 一个 protoconf（.proto）文件。 Worksheet protoconf 文件中的一个顶层 message（名为 @TABLEAU 的元表除外）。 column message 中的一个字段。 "}).add({id:334,href:"/zh/docs/basics/concepts/#简单映射示例",title:"核心概念 / 简单映射示例 ",description:"Tableau 核心概念。",content:`
`}).add({id:335,href:"/zh/docs/basics/concepts/#输入一个-excel-文件",title:"核心概念 / 输入：一个 Excel 文件 ",description:"Tableau 核心概念。",content:` 一个工作簿（HelloWorld.xlsx），包含两个数据工作表（ItemConf 和 ActivityConf）以及一个空的 tableau 元表（@TABLEAU）。
HelloWorld.xlsx&nbsp; ItemConf ActivityConf @TABLEAU ID Name Type map&lt;uint32, Item&gt; string int32 Item&rsquo;s ID. Item&rsquo;s name. Item&rsquo;s type. 1 item1 100 2 item2 200 3 item3 300 ID Name Open map&lt;uint32, Activity&gt; string bool Activity&rsquo;s ID. Activity&rsquo;s name. Activity is open? 1 activity1 true 2 activity2 false 3 activity3 `}).add({id:336,href:"/zh/docs/basics/concepts/#输出一个-protoconf-文件",title:"核心概念 / 输出：一个 protoconf 文件 ",description:"Tableau 核心概念。",content:` 一个 protoconf 文件（hello_world.proto），包含两个顶层 message（ItemConf 和 ActivityConf）。
hello_world.proto `}).add({id:337,href:"/zh/docs/basics/naming-convention/#枚举enums",title:"命名规范 / 枚举（Enums） ",description:"命名规范。",content:` 枚举类型名使用 PascalCase（首字母大写），枚举值名使用 CAPITALS_WITH_UNDERSCORES（全大写加下划线）：
参考 Protobuf 风格：枚举。
`}).add({id:338,href:"/zh/docs/basics/naming-convention/#示例",title:"命名规范 / 示例 ",description:"命名规范。",content:" 名称 风格 示例 workbook PascalCase HelloWorld.xlsx worksheet PascalCase HelloWorld struct (message) PascalCase HelloWorld field (column) PascalCase HelloWorld "}).add({id:339,href:"/zh/docs/basics/grammar-and-types/#概述",title:"语法与类型 / 概述 ",description:"语法与类型。",content:` Tableau 的大部分语法和类型借鉴自 Protocol Buffers (proto3) 和 Golang。
`}).add({id:340,href:"/zh/docs/basics/grammar-and-types/#scalar-types",title:"语法与类型 / 标量类型（Scalar types） ",description:"语法与类型。",content:` 详细说明请参考 Protocol Buffers Proto3 Scalar。
类别 类型 默认值 数值 int32, uint32
int64, uint64
float, double 0
0
0.0 布尔值 bool false 字符串 string &quot;&quot; 字节 bytes &quot;&quot; `}).add({id:341,href:"/zh/docs/basics/grammar-and-types/#复合类型composite-types",title:"语法与类型 / 复合类型（Composite types） ",description:"语法与类型。",content:" 类型 说明 struct struct 映射为 protobuf 的 message。 list list 映射为 protobuf 的 repeated 字段。 map map 映射为 protobuf 的 map 字段。 "}).add({id:342,href:"/zh/docs/basics/grammar-and-types/#结构体struct",title:"语法与类型 / 结构体（Struct） ",description:"语法与类型。",content:` 特性 说明 水平布局（Horizontal layout） 每个标量字段占一个单元格。 简单 in-cell struct 每个字段必须是标量类型。
以逗号分隔的字段列表，例如：1,test,3.0。
若数据列表长度与 struct 字段数不一致，则按顺序填充，未配置的字段使用标量类型的默认值。 `}).add({id:343,href:"/zh/docs/basics/grammar-and-types/#列表list",title:"语法与类型 / 列表（List） ",description:"语法与类型。",content:` 特性 说明 水平布局（Horizontal layout） list 的默认布局。
元素类型可以是 struct 或标量。 垂直布局（Vertical layout） list 的元素类型应为 struct。 简单 in-cell list 元素类型必须是标量。
以逗号分隔的元素列表，例如：1,2,3。 可扩展（Scalable） 支持动态大小的 list。 忽略空元素 智能识别任意位置的空元素。 `}).add({id:344,href:"/zh/docs/basics/grammar-and-types/#映射map",title:"语法与类型 / 映射（Map） ",description:"语法与类型。",content:` 特性 说明 水平布局（Horizontal layout） 垂直布局（Vertical layout） map 的默认布局。 Hash map 实现为无序 map 或 hash map。 Ordered map 由 tableauio/loader 支持。
- C++ 简单 in-cell map key 和 value 都必须是标量类型。
以逗号分隔的 key:value 对列表，例如：1:10,2:20,3:30。 可扩展（Scalable） 支持动态大小的 map。 忽略空条目 智能识别任意位置的空条目。 `}).add({id:345,href:"/zh/docs/basics/grammar-and-types/#枚举enumeration",title:"语法与类型 / 枚举（Enumeration） ",description:"语法与类型。",content:` 特性 说明 三种枚举值形式 1. 枚举值编号（number）。
2. 枚举值名称（name）。
3. 枚举值别名（alias，通过 EnumValueOptions 指定）。 校验 自动检查枚举值的合法性。 `}).add({id:346,href:"/zh/docs/basics/grammar-and-types/#空值empty-value",title:"语法与类型 / 空值（Empty value） ",description:"语法与类型。",content:` 类型 说明 scalar 空标量将使用该标量类型的默认值填充。 struct 若所有字段均为空，则不会生成该 struct。 list 若 list 大小为 0，则不会生成该 list。
若 list 的元素（struct 类型）为空，则不会追加该元素。 map 若 map 大小为 0，则不会生成该 map。若 map 的 value（struct 类型）为空，则不会插入该条目。 nesting 递归地判断是否为空。 `}).add({id:347,href:"/zh/docs/basics/enum/#枚举值enum-value",title:"枚举（Enum） / 枚举值（Enum value） ",description:"枚举基础。",content:` tableau 解析器支持三种枚举值形式：
...
`}).add({id:348,href:"/zh/docs/basics/enum/#校验validation",title:"枚举（Enum） / 校验（Validation） ",description:"枚举基础。",content:` 由于枚举类型是预先定义的，tableau 解析器会自动校验枚举值的合法性。
`}).add({id:349,href:"/zh/docs/basics/wellknown-types/#概述",title:"知名类型（Wellknown types） / 概述 ",description:"Wellknown 类型。",content:` 为方便使用，Wellknown 类型是 Tableau 的内置类型，类似于 Protocol Buffers Well-Known Types。
使用时需要引入 Tableau 和 Protocol Buffers 提供的 proto 文件：
tableau/protobuf/wellknown.proto google/protobuf/timestamp.proto google/protobuf/duration.proto `}).add({id:350,href:"/zh/docs/basics/wellknown-types/#datetime",title:"知名类型（Wellknown types） / 日期时间（Datetime） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Datetime 。
...
datetime 和 date 基于 google.protobuf.Timestamp，参考 JSON 映射。 time 基于 google.protobuf.Duration，参考 JSON 映射。 RFC 3339: 互联网日期和时间戳 `}).add({id:351,href:"/zh/docs/basics/wellknown-types/#duration",title:"知名类型（Wellknown types） / 时长（Duration） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Duration 。
...
duration 基于 google.protobuf.Duration，参考 JSON 映射。 golang duration 字符串格式。 golang ParseDuration。 `}).add({id:352,href:"/zh/docs/basics/wellknown-types/#fraction",title:"知名类型（Wellknown types） / 分数（Fraction） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Fraction 。
...
- N/D：简单分数，例如：3/4
- N：仅分子，例如：3 等同于 3/1
- N：浮点分子，例如：0.01 等同于 1/100 `}).add({id:353,href:"/zh/docs/basics/wellknown-types/#comparator",title:"知名类型（Wellknown types） / 比较器（Comparator） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Comparator 。
comparator 包含一个 sign（符号）和一个分数 value（值），任意数字或分数均可与之比较。
类型 默认值 说明 comparator ==0 格式：&lt;Sign&gt;&lt;Fraction&gt;。
例如：==10、!=1/2、&lt;10%、&lt;=10‰、&gt;10%、&gt;=10‱ `}).add({id:354,href:"/zh/docs/basics/wellknown-types/#version",title:"知名类型（Wellknown types） / 版本号（Version） ",description:"Wellknown 类型。",content:` [!NOTE] 使用示例请参考 Wellknown 类型：Version 。
...
例如：1.0.1.1 `}).add({id:355,href:"/zh/docs/basics/predefined-types/#概述",title:"预定义类型（Predefined types） / 概述 ",description:"预定义类型。",content:` 你可以在 protoconf 文件（如 common.proto）中预先定义 enum、struct 或 union 类型，然后在工作表中将其用作列类型或跨单元格类型。
`}).add({id:356,href:"/zh/docs/basics/predefined-types/#使用方式",title:"预定义类型（Predefined types） / 使用方式 ",description:"预定义类型。",content:" 语法：在工作表中使用预定义的 CustomType 时，需在前面加一个点 .（即 .CustomType）。 导入：在 tableauc 配置的 protoFiles 选项中指定公共 proto 文件，这些文件中定义了预定义的 enum、struct、union 类型。参考 Tableauc 配置。 "}).add({id:357,href:"/zh/docs/basics/predefined-types/#枚举enum",title:"预定义类型（Predefined types） / 枚举（Enum） ",description:"预定义类型。",content:` 例如，common.proto 中定义的枚举类型 FruitType：
以下示例说明如何使用预定义枚举类型：
Excel/CSV：使用预定义枚举类型。 XML：使用预定义枚举类型 YAML：使用预定义枚举类型 `}).add({id:358,href:"/zh/docs/basics/predefined-types/#结构体struct",title:"预定义类型（Predefined types） / 结构体（Struct） ",description:"预定义类型。",content:` 例如，common.proto 中定义的 struct 类型 Prop：
