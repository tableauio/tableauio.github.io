...
List 元素类型 语法示例 scalar []uint32 enum []enum&lt;.FruitType&gt; struct [Item]uint32 predefined struct [.Item]int32 incell struct []{int32 ID,int32 Num}Item incell predefined struct []{.Item} `}).add({id:174,href:"/zh/docs/excel/list/#垂直标量列表",title:"List（列表） / 垂直标量列表 ",description:"列表使用指南。",content:` [!NOTE] 定义方式与 Incell scalar list 相同，但如果提供了多行数据，会聚合多行。
HelloWorld.xlsx 中的 worksheet ItemConf：
HelloWorld.xlsx&nbsp; ItemConf @TABLEAU ID []uint32 ID 1,2,3 1,2 1 生成结果：
hello_world.proto ItemConf.json `}).add({id:175,href:"/zh/docs/excel/list/#垂直枚举列表",title:"List（列表） / 垂直枚举列表 ",description:"列表使用指南。",content:` [!NOTE] 定义方式与 Incell enum list 相同，但如果提供了多行数据，会聚合多行。
