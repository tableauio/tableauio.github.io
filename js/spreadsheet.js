(() => {
  ...
  for (let i = 0; i < spreadsheets.length; i++) {
    ...
    spreadsheet.querySelector("#download-btn").addEventListener("click", function() {
      ...
      if (tables.length != 0) {
        ...
        for (var i2 = 0; i2 < tables.length; ++i2) {
          let ws = utils.table_to_sheet(tables[i2]);
          utils.book_append_sheet(wb, ws, sheets[i2].textContent);
        }
        let bookName = spreadsheet.querySelector("#download-btn span").textContent;
        writeFileSync(wb, bookName);
      }
    });
  }
})();
